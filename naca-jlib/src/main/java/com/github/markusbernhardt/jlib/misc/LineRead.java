...
public class LineRead {
  ...
  int readAndConvertHeaderVHToVBMode() {
    // Header read in VH mode are hh ll 00 00; and the length 0xhhll includes the
    // header itself
    // Header read in VB mode are hh xx yy ll; and the length 0xhhxxyyll does not
  ...
  public boolean isTrailingLF() {
    ...
    // the -1 is wrong as manageTrailingLF() must have already been called
    // previously on this LineReadObject, so m_nTotalLength was decremented.
