...
public class PoolOfThreads {
  ...
  /*
   * ! Enqueue \retval bool: return true if the enqueue has been correctly done;
   * that is if a free thread has been instructed to process the request, or is
   * all threads are occupied to put the request in the queue. It returns false if
   * Terminate() has been called (no more enqueing available) \param IN CRequest
   * *pRequest: Request to be treated by one of the threads as soon as one will
   * become available \note Enqueue a request and a type of request; it will be
   * precced asynchronously by a thread allocated in the pool, as soon as one is
   * available
   */
  ...
  /*
   * ! Terminate \retval void \note Reqests all threads to terminate their Run()
   * method i.e. to shutdown their processing As soon as this method is lauched,
   * no more enqueing is possible; that is Enqueue will return false.
   */
  ...
  /*
   * ! Join \retval void \note Wait until all threads in the pool have been
   * destroyed
   */
  private void join() {
    terminate();
