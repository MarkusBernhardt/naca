...
public class ClassDynLoader extends ClassLoader {
  ...
  // This is the required version of loadClass which is called both from loadClass
  // above and from the internal function FindClassFromClass.
  ...
  private Hashtable<String, byte[]> loadJarFileData(String csJarFile, Hashtable<String, Integer> hashFileSize) {
    ...
    try {
      ...
      while ((entry = zis.getNextEntry()) != null) {
        ...
        if (csEntryName.toLowerCase()
                       .endsWith(".class")) {
          int nSize = (int) entry.getSize(); // -1 means unknown size.
          if (nSize == -1) {
            nSize = hashFileSize.get(entry.getName())
                                .intValue();
          }
