 ...
 * @version $Id: LogicalFileDescriptor.java,v 1.16 2007/10/25 15:13:11 u930di
 *          Exp $
 */
public class LogicalFileDescriptor {
  ...
  public void fill(String csPhysicalDesc) {
    ...
    if (nIndex >= 0) {
      m_csPath = csPhysicalDesc.substring(0, nIndex)
                               .trim();
      csPhysicalDesc = csPhysicalDesc.substring(nIndex + 1);
      nIndex = csPhysicalDesc.indexOf(",");
      while (nIndex != -1) {
        String csWord = csPhysicalDesc.substring(0, nIndex)
                                      .trim();
  ...
  private boolean tryDetermineVariableLengthRecord(BaseDataFile dataFile) {
    ...
    try {
      ...
      if (nNbRecordHeaderOk == nNbRecordControled && nNbRecordHeaderOk > 0 && nNbRecordHeaderNotOk == 0) // All record cheked are ok, even if less than 3
                                                                                                         // records in the file !
      {
