...
public class StringUtil {
  ...
  public static String decodeSignComp3String(String s, int nNbDigitInteger) {
    ...
    if (bEvenNumberOfDigits) {
      // Remove leading 0 that was there as a placeholder due to the even number of
      // digits + sign -> implies an odd number of nibbles; the leading compensated
      // that odd number
  ...
  /*
   * private static int indexOfNoCase(char[] source, int sourceOffset, int
   * sourceCount, char[] target, int targetOffset, int targetCount, int fromIndex)
   * { if (fromIndex >= sourceCount) { return (targetCount == 0 ? sourceCount :
   * -1); } if (fromIndex < 0) { fromIndex = 0; } if (targetCount == 0) { return
   * fromIndex; }
   * 
   * char first = target[targetOffset]; int max = sourceOffset + (sourceCount -
   * targetCount);
   * 
   * int i = sourceOffset + fromIndex; char cSource = source[i]; if(cSource >= 'a'
   * && cSource <= 'Z') cSource -= 'a' + 'A'; for (; i <= max; i++) { // Look for
   * first character. if (cSource != first) { while (++i <= max) { char cSource =
   * source[i]; if(cSource >= 'a' && cSource <= 'Z') cSource -= 'a' + 'A';
   * if(cSource == first) break; }
   * 
   * // Found first character, now look at the rest of v2 if (i <= max) { int j =
   * i + 1; int end = j + targetCount - 1; for (int k = targetOffset + 1; j < end
   * && source[j] == target[k]; j++, k++);
   * 
   * if (j == end) { // Found whole string. return i - sourceOffset; } } } return
   * -1; }
   */
   ...
   *          What to look for.
   * @param s
   *          What has to replace the needle.
   */
   ...
   * @param csInput
   *          inout String
   * @return encoded string Example: Input = "ha<toto>&tutu" Output=
   *         "ha&lt;toto&gt;&amp;tutu"
   */
   ...
   * @param csInput
   *          string to decode
   * @return decoded string Example: Input = "ha&lt;toto&gt;&amp;tutu" Output =
   *         "ha<toto>&tutu"
   */
  ...
  // Returns the number of line of the text. A null string is 0 line length; an
  // empty one is 1 line length a\nb is 2 lines length, a\nb\c\n is 4 line length
