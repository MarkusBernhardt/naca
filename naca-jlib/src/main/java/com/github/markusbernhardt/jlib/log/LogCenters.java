...
/**
 * Class containing a collection of {@link LogCenterLoader} instances, and
 * populating it based on the content [Organisation] tag in a JLib logger
 * configuration file. <i>LogCenterLoader</i> instances can be individually
 * accessed through the {@link #getLogCenterloader} method. One
 * <i>LogCenterLoaded</i> instance is created for each [LogCenter] tag present
 * in the [Organisation] tag provided to the class constructor.
 * <p/>
 * Each <i>LogCenterLoader</i> will instantiate a new {@link LogCenter} object.
 * Those objects will be registered to the {@link Log} singleton, and will
 * eventually change their configuration. The new configuration can be retrieve
 * in the form of a {@link Tag} instance through {@link #saveDefinition}.
 * 
 * @author PJD
 *
 */
public class LogCenters {
  ...
  /**
   * Reads the [Organisation] tag of a JLib.log configuration file and
   * instantiates one {@link LogCenterLoader} for each [LogCenter] tag in it. The
   * <i>LogCenterLoader</i> reads the [LogCenter] tag and builds a new
   * {@link LogCenter} instance with it, and then registers the new instance to
   * the {@link Log} singleton.
   * <p/>
   * 
   * The general effect of calling this method is that all log centers described
   * in the specified configuration are registered to the {@link Log} singleton.
   * 
   * @param csChannelRestriction
   *          If not <i>null</i>, the method will only load [LogCenter] belonging
   *          to the specified channel.
   * @param tagOrganisation
   *          The [Organisation] tag to be read (comming from a JLib.log xml
   *          configuration file loaded through a {@link Tag} class).
   * @param arrIncludePath
   *          If additional [LogCenter] can be found in external files, a "File"
   *          attribute is placed in the [LogCenters] tag. This attribute contains
   *          the path to another xml file. The external file will be searched in
   *          the list of path specified in <b>arrIncludePath</b>.
  ...
  /**
   * Saves the configuration of all {@link LogCenterLoader} instances into the
   * provided {@link Tag} object.
   * 
   * @param tagOrganisation
   *          The <i>Tag</i> where save the <i>LogCenterLoader</i> configuration.
   ...
   * 
   * @param n
   *          The item ordinal to retrieve. Has to be smaller than the maximum
   *          number of items (see {@link #getNbLogCenterloader}).
