 ...
 * @version $Id: SqlMapperRecordsCollection.java,v 1.2 2007/12/04 14:00:23
 *          u930di Exp $
 */
public class SqlMapperRecordsCollection {
  ...
  boolean executeSelect(DbAccessor dbAccessor, String csTableName, RecordId recordId) throws TechnicalException {
    ...
    try {
      ...
      // for(int nCol=0; nCol<recordId.getNbColValues(); nCol++) // Enum all cols of
      // the record
      // {
      ...
      SqlMapperManagedRecord recordColsTypeMaster = null; // Just to identify the types of all columns; It's done only for the 1st record
                                                          // of the result set.
      // A performance enhancement would be to have this recordColsTypeMaster created
      // for only the 1st record of all select sharing the same statement
      ...
      // Callback to call to inform application code of the records read; it can
      // filter; It must return only 1 record
      // RecordsCollection arrRecordsSelected =
      // sqlMapperSelectFilterIntf.filterRecordsSelected(arrRecordsRead);
      // if(arrRecordsSelected != null)
      // replaceInternalContainer(arrRecordsSelected); // "this" contains now only the
      // selected record; it can be null if the filter decided so
