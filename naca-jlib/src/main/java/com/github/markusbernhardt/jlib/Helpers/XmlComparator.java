...
public class XmlComparator {
  ...
  // ***************************
  /**
   * Returns the list of differences between the last two compared nodes. If the
   * compared nodes are identical, the list is empty (not null).
   */
  ...
  // *****************************************************************************************
  /**
   * Compares two Xml nodes and checks if they contain the same information. This
   * method doesn't check the attributes of upper level. (upper level attributes
   * are attributes present directly in <i>x1</i> and <i>x2</i>; attributes in
   * child nodes are always compared).
   * 
   * @param x1
   *          One of the nodes to be compared.
   * @param x2
   *          The other of the nodes to be compared.
   ...
   *          When <i>true</i> the method checks the upper level attributes (upper
   *          level attributes are attributes present directly in <i>x1</i> and
   *          <i>x2</i>; attributes in child nodes are always compared)
   * @return <i>true</i> If both nodes contain the same information.
   */
   ...
   * @return <i>true</i> if all information contained in <b>x1</b> is present in
   *         <b>x2</b>. <b>x2</b> may contain more information.
   */
  private boolean IsAllInfoPresent(Node x1, Node x2, boolean ignoreBlankSpaces, boolean checkAttributes, boolean ignoreEmptyNodes) throws Exception {
    ...
    try {
      ...
      // *****************
      if (checkAttributes)
        for (n = 0; n < x1.getAttributes()
                          .getLength(); n++) {
          x1Attr = x1.getAttributes()
                     .item(n);
      ...
      m = 0;
      for (n = 0; n < x1.getChildNodes()
                        .getLength(); n++) {

        // .................... On cherche le suivant node texte de x1
        // .............................
        x1Node = x1.getChildNodes()
                   .item(n);
      ...
      for (n = 0; n < x1.getChildNodes()
        ...
        if (m == x2Nodes.getLength())
          return false; // Si il n'y en a aucun, x1<>x2.
      }
