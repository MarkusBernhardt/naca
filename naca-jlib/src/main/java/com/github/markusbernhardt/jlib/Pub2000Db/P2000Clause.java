 ...
 * 
 */
package com.github.markusbernhardt.jlib.Pub2000Db;

import java.util.Calendar;
import java.util.Date;

import com.github.markusbernhardt.jlib.languageUtil.LanguageId;
import com.github.markusbernhardt.jlib.misc.DateUtil;
import com.github.markusbernhardt.jlib.misc.NumberParser;
import com.github.markusbernhardt.jlib.sql.DbAccessor;
import com.github.markusbernhardt.jlib.sql.DbConnectionBase;
import com.github.markusbernhardt.jlib.sql.SQLClause;
 ...
 */
/*
 * Sample usage of P2000Clause with a main and alternate connection: P2000Clause
 * clause = new P2000Clause();
 * 
 * DbConnectionBase mainConnection = clause.getAlternateConnection();
 * Asserter.assertIfNotNull(mainConnection); // mainConnection is null as it's
 * not an alternate connection // L a connection principale est g�r�e dans le
 * TLS. Il n'y a pas besoin de la passer explicitement d'une m�thode � l'autre;
 * elle n'est pas accessible publiquement
 * 
 * clause.set("SELECT * FROM RSGV43"); clause.prepareAndExecute();
 * clause.next();
 * 
 * P2000Clause clauseAlternate = new P2000Clause(P2000Accessor.accessor, null);
 * // 1st call to establish an alternate connection; it's identified by the null
 * parameter DbConnectionBase alternateConnection =
 * clauseAlternate.getAlternateConnection(); // Access to the alternate
 * connection Asserter.assertIfNull(alternateConnection); // alternateConnection
 * is not null as it's an alternate connection
 * 
 * // Do some actions on the alternate connection
 * clauseAlternate.set("SELECT count(*) FROM RSGV00");
 * clauseAlternate.prepareAndExecute(); clauseAlternate.next();
 * clauseAlternate.close();
 * 
 * // Open another clause with the same alternate connection P2000Clause
 * clauseAlternate2 = new P2000Clause(P2000Accessor.accessor,
 * alternateConnection); // The previously allocated alternate connection is
 * given in second parameter // Do some actions on the seond clause, using the
 * alternate connection clauseAlternate2.set("SELECT count(*) FROM RSGV01");
 * clauseAlternate2.prepareAndExecute(); clauseAlternate2.next();
 * clauseAlternate2.close();
 * 
 * // alternateConnection.commitWithException(); // If we wanted to commit
 * operations done on alternate connection //
 * alternateConnection.rollbackWithException(); // Or roolback operations done
 * on alternate connection
 * 
 * alternateConnection.returnConnectionToPool(); // Return alternate connection
 * to pool
 * 
 * clause.close(); // Close 1st clause, P2000Accessor.returnConnectionToPool();
 * // Return main connection to pool
 */

public class P2000Clause extends SQLClause {
  /**
   * Use this ctor to have the connection to DB automatically established. The
   * clause uses automatically the DB defined under the PUB2000Db section of the
   * app.properties file. This is defined by the accessor used (@see
   * P2000Accessor.accessor)
   */
  public P2000Clause() {
  ...
  /**
   * @param connection:
   *          Valid conection already established Use this ctor to create a new
   *          specific SQLClause, without automatic connection allocation. The TLS
  ...
  // Call with connection filled to reused an already established alternate
  // connection
   ...
   * 
   * @param languageId
   *          Valid languageId identifying a language in the Pub2000 system
   */
  ...
  // **************************************************************************
  /**
   * Handles <code>Date</code> parameters in the P2000 database. In the P2000
   * database, <code>Date</code> parameters are stored as integer with the
   * following format:
   * 
   * <pre>
   * YYYYMMDD
  ...
  /**
   * Retrieves <code>Date</code> parameters from the P2000 database. In the P2000
   * database, <code>Date</code> parameters are stored as integer with the
   * following format:
   * 
   * <pre>
  ...
  // **************************************************************************
  /**
   * Handles <code>Time</code> parameters in the MYPPUM database. In the MYPPUM
   * database, <code>Time</code> parameters are stored as integer with the
   * following format:
   * 
   * <pre>
   * HHMMSS
   ...
   * 
   * @param bVal
   *          A <code>Date</code> parameter, where only the <code>Time</code> is
   *          taken (see {@link #paramDate}).
   * @return See {@link #param(int)}.
   */
   ...
   * 
   * @param csName
   *          Column's name; it must be typed as an int
   * @return
   */
