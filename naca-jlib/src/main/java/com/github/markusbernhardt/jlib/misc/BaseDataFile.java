...
public abstract class BaseDataFile {
  ...
  public static boolean isNullFile(String csFilePhysicalName) {
    ...
    if (csFilePhysicalName.equalsIgnoreCase("wrk/nullfile"))
      return true;
    if (csFilePhysicalName.toUpperCase()
                          .indexOf("NULLFILE") >= 0)
      return true;
    return false;
  }
  ...
  public abstract LineRead readVariableLengthLine(boolean bTryReadNextLF, boolean bHeaderIsInt, LineRead lineOut); // Read a vairable length line (length is
                                                                                                                   // given in record header 4 bytes)
