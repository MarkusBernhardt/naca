...
public class DbPreparedStatement {
  ...
  public boolean prepare(DbConnectionBase dbConnection, String csQuery, boolean bHoldability) {
    ...
    if (dbConnection != null) {
      try {
        if (bHoldability)
          m_PreparedStatement = dbConnection.getDbConnection()
                                            .prepareStatement(csQuery, ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY,
                                                ResultSet.HOLD_CURSORS_OVER_COMMIT);
        else
          m_PreparedStatement = dbConnection.getDbConnection()
                                            .prepareStatement(csQuery);
        return true;
      } catch (SQLException e) {
  ...
  public boolean prepareWithException(DbConnectionBase dbConnection, String csQuery, boolean bHoldability) throws TechnicalException {
    m_csQueryString = csQuery;
    if (dbConnection != null) {
      try {
        if (bHoldability)
  ...
  int executeWithException(SQLTypeOperation typeOperation, SQLClause sqlClause) throws TechnicalException {
    if (typeOperation == SQLTypeOperation.Insert)
      ...
      // Should never happen: This case should be handled only by
      // DbConnectionBase.prepareAndExecuteWithException
  ...
  private int executeWithException(SQLClause sqlClause) throws TechnicalException {
    if (m_PreparedStatement != null) {
      try {
  ...
  public int executeInsertWithException(SQLClause sqlClause) throws TechnicalException {
    if (m_PreparedStatement != null) {
      try {
        int n = m_PreparedStatement.executeUpdate();
        return n;
  ...
  public int executeUpdateWithException(SQLClause sqlClause) throws TechnicalException {
    if (m_PreparedStatement != null) {
      try {
        int n = m_PreparedStatement.executeUpdate();
        return n;
  ...
  public int executeDeleteWithException(SQLClause sqlClause) throws TechnicalException {
    if (m_PreparedStatement != null) {
      try {
        int n = m_PreparedStatement.executeUpdate();
        return n;
  ...
  public ResultSet executeSelectWithException() throws TechnicalException {
    if (m_PreparedStatement != null) {
      try {
        ResultSet r = m_PreparedStatement.executeQuery();
        return r;
