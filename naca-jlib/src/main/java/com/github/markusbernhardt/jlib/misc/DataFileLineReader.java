...
public class DataFileLineReader extends BaseDataFileBuffered {
  ...
  public LineRead readVariableLengthLine(boolean bTryReadNextLF, boolean bHeaderIsInt, LineRead lineOut) // Read a vairable length line (length is given in
                                                                                                         // record header 4 bytes)
  {
    ...
    if (recordHeader != null) {
      ...
      if (bHeaderIsInt)
        ...
        // VH Mode
        // nLength = recordHeader.getAsLittleEndingUnsignBinaryShort(); // The header is
        // 2 bytes long, with the next 2 bytes at 0
  ...
  public LineRead readBuffer(int nLength, boolean bTryReadNextLF) {
    ...
    if (bTryReadNextLF)
      nFullLength++;
    if (isPositionAtOffsetInReadAHead(nFullLength)) // The next recoprd, including optional LF is already read the in read ahead
                                                    // buffer
    {
