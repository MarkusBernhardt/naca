...
/**
 * Events are raised by applications to signal any particular circumstance, and
 * are sent to the {@link Log} listener, which will broadcast the event to all
 * active {@link LogCenter} instances. Events are caracterized by:
 * <ul>
 * <li>Its name (see {@link #getName} and {@link #getShortName}). If not
 * specified, the default value is the event class (either <i>LogEvent</i>,
 * either the name of the extending class.</li>
 * <li>Its type (see {@link #getLogEventType}). This characteristic is
 * mandatory.</li>
 * <li>Its level (see {@link #getLogLevel}). This characteristic is mandatory.
 * </li>
 ...
 * <li>A variable list of parameters (see {@link #fillMember}). It can be empty
 * if there are no parameters associated with the event.</li>
 * </ul>
 * Different constructors allow to specify the characteristics of the event.
 ...
 * <ol>
 * Additionally, the log center must be listening to the channel specified by
 * calling the {@link Log#log} method.
 * 
 * @author u930di
 */
