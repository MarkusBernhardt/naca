...
public class LogCenterDb extends LogCenter {
  ...
  void sendOutput(LogParams logParam) {
    ...
    if (m_dbConnection != null) {
      ...
      if (!m_bUseSequence)
        cs = "Insert into " + m_csMasterTable + " (" + "Log_Type, File_Name, Line, Thread, Method, Start_Time, Event_Name, Message) Values ("
            + "?,    ?,    ?,    ?,      ?,      ?,         ?,         ?)";
      else
        cs = "Insert into " + m_csMasterTable + " (" + "Log_Type, File_Name, Line, Thread, Method, Start_Time, Event_Name, Message, Id) Values ("
            + "?,    		?,    		?,    ?,      ?,      ?,         ?,         ?, 		 SEQ_LOG_ID.nextval)";
