...
import com.github.markusbernhardt.jlib.jmxMBean.BaseCloseMBean;

/**
 * A DbAcessor is attached to a Db It contains a Database identifier. All
 * databases must be uniquely identified by an single instance of derivated from
 * this class The m_csKey value must point to the section of an app.properties
 * file. The app.properties file section must be accessible in the classpath,
 * and must contains the following values in the case of PUB2000Db:
 * 
 * PUB2000Db.driver=com.ibm.db2.jcc.DB2Driver
 * PUB2000Db.connectionString=jdbc:db2://db2002.consultas.ch:50002/PUB2000T
 * PUB2000Db.user=<User> PUB2000Db.password=<Password>
 * PUB2000Db.environment=TEST
 * 
 * In that sample, m_csKey must be set to "PUB2000Db"
 */
public class DbAccessor extends BaseCloseMBean {
  private String m_csKey = null; // Gives the section name within the app.properties file. This section is used
                                 // to provide DB parameters
   ...
   * 
   * @return a database connection corresponding to the Db identified by DbId
   *         passed in the constructor. The connection is created if it is not
   *         establed yet; It's managed from the pool and cannot be accessed
   *         publicly
   **/
  DbConnectionBase getConnection() {
    // Try to get the connection from the Thread Local Storage
    DbConnectionBase dbConnectionBase = DbTLSConnectionStorage.get(this);
    if (dbConnectionBase == null) // The connection doesn't exist in the TLS: This is the 1st clause created
                                  // within this thread since laste returnConnectionToPool
    {
      // Establish a connection: It can be either got form the pool (of one is
      // available) or created.
      dbConnectionBase = DbAccessorConnectionManager.getConnection(this);
      if (dbConnectionBase != null) {
        // The connection has been obtained from pool; store it in the TLS for futher
        // reference
   ...
   * 
   * @return a database connection NOT managed in the TLS It's taken from the pool
   **/
