...
public abstract class BaseCloseMBean extends BaseDynamicMBean {
  ...
  public Object getAttribute(String csName) {
    ...
    for (int n = 0; n < m_arrMBeanAttributeInfosWrapper.size(); n++) {
      ...
      if (attributeInfo.getName()
                       .equalsIgnoreCase(csName)) // Found attribut
      {
        // Call method
  ...
  public Object invoke(String csOperationName, Object params[], String signature[]) {
    if (csOperationName != null) {
      for (int n = 0; n < m_arrMBeanOperationInfosWrapper.size(); n++) {
        ...
        if (operationInfo.getName()
                         .equalsIgnoreCase(csOperationName)) // Found attribut
        {
          // Call method
  ...
  // protected void addAttribute(String csName, String csType, String
  // csDescription, boolean bIsReadable, boolean bIsWritable, boolean bIsIs)
  // {
  // MBeanAttributeInfoWrapper attr = new MBeanAttributeInfoWrapper(csName,
  // csType, csDescription, bIsReadable, bIsWritable, bIsIs);
  ...
  // protected void addOperation(String csName, String csDescription,
  // MBeanParameterInfo[] arrSignature, String csType, int nImpact)
  // {
  // MBeanOperationInfo operation = new MBeanOperationInfo(csName, csDescription,
  // arrSignature, csType, nImpact);
  ...
  private void registerInfos(String csName, String csDescription) {
    ...
    m_MBeanInfo = new MBeanInfo(csName, csDescription, arrAttributes, null, // dConstructors,
        arrOperations, null); // dNotifications);
  }
