...
public abstract class DumpClassLoaderRegistry {
  ...
  /**
   * Returns an enumeration of all class loaders currently registered in the JVM.
   * Note that this may not include all class loaders ever' created because this
   * registry uses weak keys as class loader references.
   * 
   * @param ClassLoader
   *          array [never null].
   */
  public static ClassLoader[] getClassLoaders() {
    ...
    synchronized (CLASSLOADER_SET) {
      ...
      for (Iterator<ClassLoader> keys = CLASSLOADER_SET.keySet()
  ...
  private static final WeakHashMap<ClassLoader, ClassLoader> /* ClassLoader->null */ CLASSLOADER_SET = new WeakHashMap<ClassLoader, ClassLoader>();
