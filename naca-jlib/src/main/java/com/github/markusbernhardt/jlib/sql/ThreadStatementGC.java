...
public class ThreadStatementGC extends Thread {
  ...
  // m_nNbTotalStatementRemoved +=
  // dbConnectionPool.garbageCollectorStatementsOfAllCollections();
  // }
  // int nNbStatementAggressiveRemoved = 0;
  // if(m_tenuredPool != null && m_tenuredPool.isUsageThresholdExceeded() &&
  // m_nNbStatementForcedRemoved > 0)
  // {
  // // Aggressivelly remove statements is heap usage is to high
  // // Collect all statements from all pools
  // SortedMap<Long, StatementPosInPool> mapStatements = new TreeMap<Long,
  // StatementPosInPool>();
  // for(int n=0; n<m_arrDbConnectionPool.size(); n++)
  // {
  // DbConnectionPool dbConnectionPool = m_arrDbConnectionPool.get(n);
  ...
  // }
  // nNbStatementAggressiveRemoved =
  // aggressiveRemoveObsoleteStatements(mapStatements);
