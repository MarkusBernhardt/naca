 ...
 * 
 */
package com.github.markusbernhardt.jlib.languageUtil;
 ...
 */
/*
 * LanguageId List of all supported languages; support for a default language
 */
public class LanguageId {
  private String m_csIsoCode = null; // Public ISO code
  private String m_csCodeUpper = null; // Public textual code; Used to select a language id; it's fr, de, it, , ... It
                                       // has the corresponding value in lower case in m_csCodeLower
   ...
   * Can be used to retrieve a LanguageId by it's code LanguageId lg =
   * LanguageId.get("FR"); If the code doesn't exists, then the default language
   * id is returned.
   */
  public static LanguageId get(String cs) {
    ...
    for (int n = 0; n < ms_arrLanguages.size(); n++) {
      ...
      if (languageId.getTextCodeUpperCase()
                    .equals(cs)
          || languageId.equals(cs))
        return languageId;
    }
