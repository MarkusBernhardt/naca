 ...
 * @version $Id: DbTLSStoredConnections.java,v 1.7 2008/07/09 08:01:23 u930di
 *          Exp $
 */
public class DbTLSStoredConnections {
  private DbConnectionBase m_foreignDbConnectionBase = null; // Holds a foreign db connection, that is a connection is has been provided by a
                                                             // caller using setForeignConnection
  ...
  /*
   * int getNbConnection(DbAccessor dbId) // No need to synchonize: the
   * m_hashConById is private to the current thread { //if(m_hashConById == null)
   * return 0; // return m_hashConById.get(dbId); }
   */

  /*
   * setForeignConnection Store a foreign connection in the TLS for usage by jlib
   * management; the connection is not pooled !
   */
  ...
  /*
   * commit for an accessor Not usable for foreign connections
   */
  ...
  /*
   * rollback for an accessor Not usable for foreign connections
   */
