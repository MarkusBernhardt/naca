...
public class DbConnectionPool {
  ...
  DbConnectionPool(String csPoolName, int nNbMaxConnections, int nTimeBeforeRemoveConnection_ms, int nMaxStatementLiveTime_ms,
      int nGarbageCollectorStatement_ms) {
    m_nGarbageCollectorStatement_ms = nGarbageCollectorStatement_ms;
    DbConnectionColl dbConnectionColl = new DbConnectionColl(csPoolName, nNbMaxConnections, nTimeBeforeRemoveConnection_ms, nMaxStatementLiveTime_ms, false,
        m_nGarbageCollectorStatement_ms);
  ...
  DbConnectionPool(Tag tagSQLConfig) {
    ...
    if (tagPools != null) {
      ...
      while (tagPool != null) {
        ...
        DbConnectionColl dbConnectionColl = new DbConnectionColl(csPoolName, nMaxConnection, nTimeBeforeRemoveConnection_ms, nMaxStatementLiveTime_ms,
            bUseExplain, m_nGarbageCollectorStatement_ms);
        ...
        if (!StringUtil.isEmpty(csProgramIds)) {
          ...
          while (nIndex != -1) {
            String csProgramId = csProgramIds.substring(0, nIndex)
                                             .trim();
