...
/**
 * Abstract class declaring the basic interface for a <i>flow</i>. The events
 * (see {@link LogEvent}) raised by applications (by calling the {@link Log#log}
 * method) are classified under two different criteria:
 * <ul>
 * <li>Their level ({@link LogLevel}) indicates how important they are. More
 * important events may be logged to a database, while less important events may
 * just be logged to a local file. Event may not logged at all.</li>
 ...
 * <ul>
 * <li>The event <i>LogLevel</i> has to be superior or equal to the
 * <i>LogLevel</i> of the <i>LogCenter</i></li>
 * <li>The <i>LogCenter</i> flow has to accept the event flow.
 * {@link #isAcceptable} is the method checking this second condition.</i>
 * </ul>
 * 
 * @see LogFlowStd
 * @see LogFlowRegister
...
public abstract class LogFlow {
   ...
   * {@link LogFlowRegister} static collection. This constructor is to be called
   * from the default constructor of the extending class:
   * 
   * <pre>
   ...
   * 
   * @param cs
   *          The name to be compared with the flow's name.
   * @return <i>true</i> if the specified name corresponds to the flow's name. The
   ...
   * 
   * @param logFlow
   *          The flow to return the name of.
  ...
  /**
   * Returns a reference to the registered flow named as specified. The method
   * checks the specified name against the registerd flows (see
   * {@link LogFlowRegister}).
   * 
   * @param csFlow
   *          The name of the flow to retrieve. The name is case insensitive.
  ...
  /**
   * Should return <i>true</i> if the specified <b>logFlow</i> is accepted by the
   * current flow.
   * 
   * @param logFlow
   *          The flow to be checked.
