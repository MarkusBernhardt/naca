...
import com.github.markusbernhardt.jlib.misc.StopWatch;
import com.github.markusbernhardt.jlib.xml.Tag;

/**
 * The Log class holds a collection of {@link LogCenter}s, and broacast to them
 * the events received by the static methods {@link #log}, {@link #logCritical},
 * {@link #logImportant}... The <i>LogCenter</i>s are loaded via the
 * {@link #open} (and overloads) method. The {@link LogEvent}s are received via
 * the {@link #log} method, or any of its overloads.
 * <p/>
 * 
 * @author u930di
 */
public class Log {
  ...
  // private static Hashtable<String, LogChannelSetting> ms_hashSettings = new
  // Hashtable<String, LogChannelSetting>();
  ...
  /**
   * Initializes all {@link LogCenter}s described in the specified configuration
   * file, configured to listen to the specified channel
   * <p/>
   * 
   * @param csChannel
   *          Only <i>LogCenter</i> listening to this channel will be initialized.
   * @param csConfigFile
   *          The JLib.log xml configuration file containing the description for
   *          one or more <i>LogCenter</i> objects.
   * @param csRunId
   *          The default <i>RunId</i> identifier for the specified channel. This
   *          value will be used any time no <i>RunId</i> is specified when
   *          receiving an event through a {@link #log} call.
   * @param csProduct
   *          The default <i>Product</i> for the specified channel. This value
   *          will be used when logging an {@link LogEvent} which doesn't specify
   *          a <i>Product</i>.
   * @return The <i>LogCenter</i> descriptors. Use the simplest overload of this
   *         method. Use {@link #setRunId}, {@link #setProduct},
   *         {@link #setProcess} to configure default values for a particular
   *         channel.
   */
   ...
   *          Only <i>LogCenter</i> listening to this channel will be initialized.
   * @param isConfigFile
   *          An input stream opened on the JLib.log xml configuration file
  ...
  /**
   * Opens the specified channel on all the {@link LogCenter} described in the
   ...
   * 
   * @param csChannel
   *          The name of the channel. Leave it to <i>null</i> to affect all
   *          opened chanels.
   * @param csRunId
   *          The specified value is assigned as the default <i>RunId</i>
   *          identifier for all <i>LogCenter</i> listening to the channel. Some
   *          log centers generate a new unique value ({@link LogCenterDbFlat}
   *          when <i>null is specified. Other log centers don't use this
   *          identifier.
   */
   ...
   * 
   * @param csChannel
   *          The channel to which set the defalut <i>Product</i>. Leave it to
   *          <i>null</i> to affect all opened channels.
   * @param csProduct
   *          The default <i>Product</i> for the specified channel.
   */
   ...
   * 
   * @param csChannel
   *          The channel to retrieve the default <i>Product</i>.
   ...
   * 
   * @param csChannel
   *          The channel to which set the default <i>Product</i>. Leave it to
   *          <i>null</i> to affect all opened channels.
   * @param csProcess
   *          The default <i>Process</i> for the specified channel.
   */
  ...
  /**
   * Opens the {@link LogCenter}s described in the specified configuration file.
   * <p/>
   * 
   * @param csChannel
   *          If not <i>null</i>, only the <i>LogCenter</i> listening to the
   *          specified channel will be initialized. The other will be ignored.
   *          Usually the channel name is equal to the application name.
   *          "Application name" is used here in the large sense: it is not the
   *          name of the executable file (as would be <i>winword.exe</i> for the
   *          "Microsoft Office Word", but the application official name (as would
   *          be "WORD").
   * @param csRunId
   *          If not <i>null</i> contains the <i>RunId</i> identifier to be used
   *          by default to the specified channel. If <i>null</i>, a unique
   *          <i>RunId</i> will be generated (provided that at least one the
   *          <i>LogCenter</i> is able to generate <i>RunId</i> identifiers).
   * @param csConfigFile
   *          The JLib.log xml configuration file containing the description for
   *          one or more <i>LogCenter</i> objects.
  ...
  /**
   * Registers a new {@link LogCenter} in the active log centers collection. Once
   * registered, the log centers receive the log events through the
   * {@link #sendLog} method.
   * 
   * @param logCenter
   *          The log center to register.
   */
  ...
  public synchronized static LogCenterPluginConsole getLogCenterPluginConsole() {
    if (m_arrLogCenter != null) {
      for (int n = 0; n < m_arrLogCenter.size(); n++) {
        LogCenter logCenter = m_arrLogCenter.get(n);
        if (logCenter.getType()
                     .equalsIgnoreCase("LogCenterPluginConsole"))
   ...
   * Each <i>LogCenter</i> will decide if it accepts the event based upon three
   * criteria:
   * <ul>
   * <li>The event is sent on the channel the <i>LogCenter</i> is listening
   * to.</li>
   * <li>The event has a {@link LogFlow} accepted by the <i>LogCenter</i> flow
   * (see protected property {@link #m_logFlow}).</li>
   * <li>The event has a {@link LogLevel} equal or higher than the minimal
   * required by the <i>LogCenter</i> (see property {@link getLevel}.</li>
   * </ul>
   * Additionally, the <i>LogCenter</i> must be enabled.
   * 
   * @param logParams
   *          A structure containing all paremeters characterizing one log event.
   */
   ...
   * 
   * @param tagSettings
   *          The [Settings] tag of a [LogCenter] definition.
   */
   ...
   * {@link LogFlow} and {@link LogLevel} of the event. The specified event will
   * be broadcasted to all registered {@link LogCenter}s. <i>LogCenter</i>s are
   * registered based on the JLib.log xml configuration file, by the {@link #open}
   * method.
   * 
   * @param csChannel
   *          The channel to send the event to.
   * @param logEvent
   *          The event to send to the specified channel.
   * @param csMessage
   *          An additional free text message. If no message is required, this
   *          paremeter can be null.
   */
  ...
  /**
   * Logs an event to the specified channel, with the specified free text message
   * and <i>RunId</i> and <i>RuntimeId</i>, using the {@link LogFlow} and
   * {@link LogLevel} of the event. The specified event will be broadcasted to all
   * registered {@link LogCenter}s. <i>LogCenter</i>s are registered based on the
   * JLib.log xml configuration file, by the {@link #open} method.
   * 
   * @param csChannel
   *          The channel to send the event to.
   * @param logEvent
   ...
   *          paremeter can be null.
   * @param csRunId
   *          Specifies a particular <i>RunId</i> identifier for the event. If
   *          <i>null</i>, the default <i>RunId</i> of the specified channel is
   *          used.
   * @param csRuntimeId
   *          Specified a particular <i>RuntimeId</i> identifier for the event. If
   *          <i>null</i>, the default <i>RuntimeId</i> of the specified channel
   *          is used.
   */
  ...
  /**
   * Logs an event to the specified channel, with the specified free text message
   * and {@link LogLevel}, using the specified channel's <i>RunId</i> and
   * <i>RuntimeId</i> identifiers, and the event's {@link LogLevel}. The specified
   * event will be broadcasted to all registered {@link LogCenter}s.
   * <i>LogCenter</i>s are registered based on the JLib.log xml configuration
   * file, by the {@link #open} method.
   * 
   * @param csChannel
   *          The channel to send the event to.
   * @param logFlow
   *          If specified, the event's flow will be ignored, and this will be
   *          used instead.
   * @param logEvent
   *          The event to send to the specified channel.
   * @param csMessage
  ...
  /**
   * Logs an event on the {@link LogFlowStd#Monitoring} flow to the specified
   * channel, with the specified message.
   * 
   * @param csChannel
   *          The channel to send the event to.
   * @param logEvent
  ...
  /**
   * Logs an event on the {@link LogFlowStd#Monitoring} flow to the specified
   * channel, with the specified message and the specified {@link LogLevel}.
   * 
   * @param csChannel
   *          The channel to send the event to.
   * @param logEvent
   *          The event to send to the specified channel.
   * @param logLevel
   *          Will be used (if specified) instead of the event's level.
   * @param csMessage
   *          An additional free text message. If no message is required, this
   *          paremeter can be null.
   */
   ...
   * @param csMessage
   *          An additional free text message. If no message is required, this
   *          paremeter can be null. With this method it is not possible to
   *          specify the event name. Use instead a custom {@link LogEvent}, and
   *          send it using the standard {@link Log#log} method.
   */
  ...
  /**
   * Simplest method to send an event with the {@link LogLevel#Normal} level. The
   * event will be sent with:
   * <ul>
   * <li>{@link LogLevel} set to <i>Critical</i>.</li>
   * <li>{@link LogFlow} set to <i>Any</i>.</li>
   * <li>{@link LogEventType} set to <i>Remark</i>.</li>
  ...
  /**
   * Simplest method to send an event with the {@link LogLevel#Verbose} level. The
   * event will be sent with:
   * <ul>
   * <li>{@link LogLevel} set to <i>Critical</i>.</li>
   * <li>{@link LogFlow} set to <i>Any</i>.</li>
  ...
  /**
   * Simplest method to send an event with the {@link LogLevel#Debug} level. The
   * event will be sent with:
   * <ul>
   * <li>{@link LogLevel} set to <i>Critical</i>.</li>
   * <li>{@link LogFlow} set to <i>Any</i>.</li>
