...
import com.github.markusbernhardt.jlib.sql.DbConnectionManager;
import com.github.markusbernhardt.jlib.sql.DbPreparedStatement;
import com.github.markusbernhardt.jlib.xml.Tag;

/**
 * Log center saving events into a flat database. This log center is used to
 * keep record of applications execution cycle:
 * <ul>
 ...
 * one.</li>
 * <li>Eventually, the error is so critical that the application has to abort
 * prematurely.</li>
 * <li>Application reports the amount of processed information elements.</li>
 * <li>Application finishes.</li>
 * </ul>
 * Events logged through this class are thereafter visible with the COP/Log
 * interface (http://c930cop.consultas.ch/LOG). For the COP/LOG interface to be
 * able to group events by execution cycles, and to keep track of applications
 * launching child applications, some identifiers are needed.
 * <ul>
 * <li><i>Process</i> is the internal application name. The term "Application
 * Name" has to be used in its most general sense.</li>
 * <li><i>RuntimeId</i> is a unique identifier retrieved when the application
 * starts, and kept during all the application execution cycle. It is used to
 * stamp all events raised by the application.</li>
 * <li><i>RunId</i> is a unique identifier retrieved when the application
 * starts, much like <i>RuntimeId</i>. The difference is that, if the
 * application launches a child application, it will pass the <i>RunId</i>
 * identifier. It is used to stamp all events raised by the application, plus
 * all events raised by the child applications.</li>
 * </ul>
 * The COP/Log interface produces two types of reports:
 * <ul>
 * <li>Execution reports, that show when an application starts, stops, how many
 * information elements it has processed, how many exceptions has been raised,
 * how much time it has been running, etc.</li>
 * <li>Commercial reports, that show how many elements of a particular client,
 * product, brand, source, etc. has been processed in general, or by a
 * particular application.</li>
 * </ul>
 * For that second kind of report, an additional identifier is needed to specify
 * the client, product, brand, source, etc. This additional identifier is the
 * <i>Product</i>.
 * <p/>
 * 
 ...
 * </pre>
 * 
 * This configuration predefines the default <i>Process</i> name, and the
 * default <i>Product</i> name. That means that any {@link LogEvent} not
 * specifying the {@link LogEvent#getProcess} or the
 * {@link LogEvent#getProduct}, will be assigned to the default ones if the
 * event is accepted by the log center.
 */
...
public class LogCenterDbFlat extends LogCenter {
  ...
  void sendOutput(LogParams logParam) {
    if (m_dbConnection != null) {
      ...
      String cs = "Insert into " + m_csTable
          + "(Machine, Process, Run_Mode, Ins_Date, Event_Message, Log_Type, File_Name, Line, Thread, Method, Start_Time, Event_Id, Run_Id, Product, Runtime"
          + csParamNames + ") values ("
          + " ?,       ?,       ?,        ?,        ?,             ?,        ?,         ?,    ?,      ?,      ?,          ?,        ?,      ?,       ?"
          + csParamQuestions + ")";
      int nCol = 0;
  ...
  private boolean addLogEventDefinition(int nEventId, LogParams logParam) {
    ...
    String cs = "Insert into " + m_csLogEventDefinitionTable + "(Event_Name, Event_Id, Short_Event_Name" + csParamNames + ") values ("
        + "?,           ?,         ?" + csParamQuestions + ")";
  ...
  /**
   * Returns the current <i>RunId</i> identifier. If it is not set (or has been
   * set to <i>null</i>), creates a new unique identifier.
   */
  ...
  /**
   * Returns the current <i>RuntimeId</i> identifier. If it is not set (or has
   * been set to <i>null</i>), creates a new unique identifier.
   */
    ...
    if (rs != null) {
      ...
      try {
        ...
        if (bNext) {
          ...
          nRunId++;
          // stSelect.close();
          // String csUpdate = "update " + m_csTableRunId + " set RunId=" + nRunId + "
          // where channel='"+csOrganisation+"'";
