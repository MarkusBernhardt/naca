...
public class DefaultNamespaceContextProvider implements NamespaceContext {
  ...
  /**
   * The root element of the node being processed. The method assumes that all
   * namespaces are declared in the root element, which is true in most cases.
   */
  ...
  // ***************************************************************************
  /**
   * This constructor uses the <b>node</b> parameter to reach the root element of
   * the document, assuming that there will be all namespace declarations.
   * 
   * @param node
   *          The context node for performing XPath operations.
   */
  public DefaultNamespaceContextProvider(Node node) {
    if (node.getNodeType() == Node.DOCUMENT_NODE)
      _rootElement = node.getFirstChild();
    else {
      _rootElement = node.getOwnerDocument()
                         .getFirstChild();
  ...
  /**
   * Constructor. Looks for the default namespace defined for the specified node:
   * <ul>
   * <li>If the specified node is a <i>DOCUMENT_NODE</i>, the method examines the
   * first child (which corresponds to the document element):
   * <ul>
   * <li>If it hasn't any prefix (its prefix is an empty string), it means that
   * the document element belongs to the default namespace. Thus the default
   * namespace is the namespace of the document element.</li>
   * <li>Else the method looks for the <i>xmlns</i> attribute, which contains the
   * default namespace for the child elements.</li>
   * </ul>
   * </li>
   ...
   * 
   * @param node
   *          The default namespace URI will be taken from the specified node.
   *
   *          public DefaultNamespaceContextProvider(Node node) { String prefix;
   *          String xmlns; if (node.getNodeType()==Node.DOCUMENT_NODE) {
   *          node=node.getFirstChild(); prefix=node.getPrefix(); if
   *          (prefix==null) _defaultNamespaceURI=node.getNamespaceURI(); else
   *          _defaultNamespaceURI=XmlHelper.GetNodeAttribute(node,"xmlns"); }
   *          else do { prefix=node.getPrefix(); if (prefix==null) {
   *          _defaultNamespaceURI=node.getNamespaceURI(); break; } else {
   *          xmlns=XmlHelper.GetNodeAttribute(node,"xmlns"); if
   *          (xmlns.length()>0) { _defaultNamespaceURI=xmlns; break; } } }
  ...
  /**
   * Returns the list of prefixes corresponding to the specified namespace. This
   * method is not implementes, as it is not used by the Xerces implementation of
   * the {@link XPath.evaluate} method.
   */
