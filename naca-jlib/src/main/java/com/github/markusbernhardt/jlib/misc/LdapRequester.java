...
/*
 * Sample call: class ResourceManager { ... // Members variables to initialize
 * by some meanings private String m_csLDAPServer = "" ; // Main LDAP server
 * address private String m_csLDAPServer2 = "" ; // 2nd LDAP server address
 * private String m_csLDAPServer3 = "" ; // 3rd LDAP server address private
 * String m_csLDAPDomain = "" ; // Domain private String m_csLDAPRootOU = "" ;
 * // Root OU; e.g.: OU=FUTUR_PUBLIGROUPE,DC=Publigroupe,DC=net private String
 * m_csLDAPGenericUser = "" ; // LDap connection user private String
 * m_csLDAPGenericPassword = "";// LDap connection password ... public
 * CLDAPRequester getLDAPRequester() { return new CLDAPRequester(m_csLDAPServer,
 * m_csLDAPServer2, m_csLDAPServer3, m_csLDAPDomain, m_csLDAPRootOU,
 * m_csLDAPGenericUser, m_csLDAPGenericPassword) ; } ... }
 *
 * Application code: ... LDapRequester ldapReq =
 * m_ResourceManager.getLDAPRequester() ; String csUserDN =
 * ldapReq.getUserLogin(m_csUserLdapId, csPassword, bLoginAuto) ; boolean
 * bLogged = !StringUtil.IsEmpty(csUserDN); ... // To get LDap attributs
 * m_csApplicationCredentials = ldapReq.getAttribute(csUserDN,
 * "extensionAttribute12") ; if (m_csApplicationCredentials == null) {
 * m_csApplicationCredentials = "" ; }
 * 
 * // To get user complete name String csSn = ldapReq.getAttribute(csUserDN,
 * "sn") ; if (csSn == null) { m_csUserLdapName = ""; } else { m_csUserLdapName
 * = csSn; String csGivenName = ldapReq.getAttribute(csUserDN, "givenName") ; if
 * (csGivenName != null) { m_csUserLdapName += " " + csGivenName; } }
 */
...
public class LdapRequester {
  ...
  /**
   * @param csUser:
   *          specific user we want to login
   * @param csPassword:
   *          specific password
   * @param bUseGenericUser:
   *          true if connecting using generic user, not the specific user (csUser
   *          / csPassword); in that case csUser/csPassword is ignored
  ...
  /**
   * @param csUserDN:
   *          User DN whose attribute is serached
   * @param csAttributName:
   *          Attribut name
