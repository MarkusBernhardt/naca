...
import com.github.markusbernhardt.jlib.jmxMBean.CompositeDataDesc;
import com.github.markusbernhardt.jlib.jmxMBean.CompositeTypeDesc;

/**
 * Class receiving {@link LogEvent}s and storing them into a media. The media
 * depends on the actual implementation of the class. Known implementations are:
 * <ul>
 * <li>{@link com.github.markusbernhardt.jlib.log.LogCenterConsole}, sends the
 * events to the console.</li>
 * <li>{@link com.github.markusbernhardt.jlib.log.LogCenterDb}, sends the events
 * to a database.</li>
 * <li>{@link com.github.markusbernhardt.jlib.log.LogCenterDbFlat}, sends the
 * events to a COP/LOG compatible database.</li>
 * <li>{@link com.github.markusbernhardt.jlib.log.LogCenterFile}, sends the
 * events to a file.</li>
 * </ul>
 * 
 * @author PJD
...
public abstract class LogCenter extends LogCenterCloseMBeam // LogCenterOpenMBean
{
   ...
   * <i>LogCenterLoader</i> has collected from a [LogCenter] section of the
   * JLib.log xml configuration file. This configuration file is originally
   * provided to the {@link Log#open} method, which is the method that initializes
   * the {@link Log} static instance containing the collection of
   * <i>LogCenter</i>s.
   * 
   * @param logCenterLoader
   *          A xml reader which has loaded a [LogCenter] section of the JLib.log
   ...
   * {@link LogParams} decorator}, checks if the event meet the conditions to be
   * accepted by the <i>LogCenter</i>, in which case it stores it. The three
   * conditions are:
   * <ul>
   ...
   * 
   * @param logParam
   *          The description of the event to be logged.
   */
  ...
  /**
   * Add the description of one {@link LogEvent} to the events buffer. If the
   * events buffer size outpasses its limit, the buffer is flushed via
   * {@link outputAllLogParamsItems}, and all events are stored.
   * 
   * @param logParam
   *          The description of the event to be stored in the events buffer.
   */
  ...
  /**
   * Empties the events buffer. The emptying sequence is the following:
   * <ul>
   * <li></li>
   * <li></li>
   * <li></li>
  ...
  /**
   * Method called before storing one or more events via {@link sendOutput}. This
   * method can be implemented at extending the class to perform any customized
   * action before storing the event(s). If no action is required, use the
   * following code to disable the method:
   * 
   * <pre>
   * void preSendOutput() {
   * }
  ...
  /**
   * Method called by {@link output} for immediately store an event. If the buffer
   * is being flushed, this method is called once for each event in the buffer.
   * <p/>
   * This method has to be implemented by extending the <i>LogCenter</i> class.
   * 
   * @param logParam
   *          The parameters of the {@link LogEvent} to be processed.
   */
  abstract void sendOutput(LogParams logParam);

  /**
   * Method called after storing one or more events via {@link sendOutput}. This
   * method can be implemented at extending the class to perform any customized
   * action after storing the event(s). If no action is required, use the
   * following code to disable the method:
   * 
   * <pre>
   * void postSendOutput() {
  ...
  /**
   * Returns <i>true</i> if the <i>LogCenter</i> is enabled. A disabled
   * <i>LogCenter</i> doesn't accept any {@link LogEvent}. An enabled
   * <i>LogCenter</i> accepts events provided three conditions are met:
   * <ul>
   * <li>The event is sent on the channel the <i>LogCenter</i> is listening
   * to.</li>
   * <li>The event has a {@link LogFlow} accepted by the <i>LogCenter</i> flow
  ...
  /**
   * Enables the <i>LogCenter</i>. A disabled <i>LogCenter</i> doesn't accept any
   * {@link LogEvent}. An enabled <i>LogCenter</i> accepts events provided three
   * conditions are met:
   * <ul>
   * <li>The event is sent on the channel the <i>LogCenter</i> is listening
   * to.</li>
   ...
   * 
   * @param b
   *          If <i>true</i>, it enables the <i>LogCenter</i>. Otherwise it
   *          disables it.
   */
  ...
  /**
   * Sets the minimal level required for the {@link LogEvent}s to be accepted by
   * the <i>LogCenter</i>.
   * 
   * @param csLevel
   *          The minimal required level. Provided level should be one of the
   *          accepted by {@link LogLevel}.
   */
  ...
  /**
   * Sets the minimal level required for the {@link LogEvent}s to be accepted by
   * the <i>LogCenter</i> to {@link LogLevel#Critical}.
   */
  ...
  /**
   * Sets the minimal level required for the {@link LogEvent}s to be accepted by
   * the <i>LogCenter</i> to {@link LogLevel#Important}.
   */
  ...
  /**
   * Sets the minimal level required for the {@link LogEvent}s to be accepted by
   * the <i>LogCenter</i> to {@link LogLevel#Normal}.
   */
  ...
  /**
   * Sets the minimal level required for the {@link LogEvent}s to be accepted by
   * the <i>LogCenter</i> to {@link LogLevel#Verbose}.
   */
  ...
  /**
   * Sets the minimal level required for the {@link LogEvent}s to be accepted by
   * the <i>LogCenter</i> to {@link LogLevel#Debug}.
   */
  ...
  /**
   * Sets the minimal level required for the {@link LogEvent}s to be accepted by
   * the <i>LogCenter</i> to {@link LogLevel#FineDebug}.
   */
