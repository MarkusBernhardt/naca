...
/**
 * Class to decorate a {@link LogEvent} instance with some additional
 * properties. Additional properties are:
 * <ul>
 * <li>A text message.</li>
 * <li>The channer where the message is sent to.</li>
 * <li>Information about the file, the class, the method and the thread where
 * the event has been created.</li>
 * </ul>
...
public class LogParams {
   ...
   * 
   * @param csChannel
   *          The channel where the event is to be sent.
   * @param logEvent
   *          The event itself.
   * @param csMessage
   *          A text message with additional description about the event.
   */
  ...
  /**
   * Class constructor specifying the <i>RunId</i> and <i>RuntimeId</i>
   * identifiers. See the {@link Log} class overview for more details about these
   * identifiers.
   * 
   * @param csChannel
   *          The channel where the event is to be sent.
  ...
  boolean isAcceptable(LogLevel minLogLevel, LogFlow logFlow) {
    if (logFlow.isAcceptable(m_logEvent.getLogFlow())) {
      if (m_logEvent.getLogLevel()
                    .isGreaterOrEqual(minLogLevel))
        return true;
    }
    return false;
  }
