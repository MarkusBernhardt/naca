...
public class Pic9Comp0BufferSupport {
  ...
  // }
  //
  // void set(VarBufferPos buffer, int nPosition, int nNbDigitInteger, int
  // nTotalSize)
  // {
  // m_buffer = buffer;
  ...
  static public void setFromRightToLeft(VarBufferPos buffer, int nPosition, int nNbDigitInteger, int nTotalSize, int nOffset, boolean bSigned,
      boolean bSignLeading, int nValue) {
  ...
  static public void setFromRightToLeft(VarBufferPos buffer, int nPosition, int nNbDigitInteger, int nTotalSize, int nOffset, boolean bSigned,
      boolean bSignLeading, long lValue) {
    int nMinIndex = nPosition + nOffset;
    if (bSigned && bSignLeading)
  ...
  /*
   * old version static public void setFromRightToLeft(VarBufferPos buffer, int
   * nPosition, int nNbDigitInteger, int nTotalSize, int nOffset, boolean bSigned,
   * boolean bSignLeading, int nValue) { // Fill the buffer with '0' on each byte
   * buffer.fillBlankComp0AtOffset(nTotalSize, nOffset);
   * 
   * int nMinIndex = 0; if(bSigned && bSignLeading) nMinIndex = 1;
   * 
   * char cSign = '+'; if(nValue < 0) { nValue = -nValue; cSign = '-'; }
   * 
   * int nNibblePos = nNbDigitInteger-1; if(bSigned && !bSignLeading) {
   * buffer.setCharAt(nPosition+nNibblePos+nOffset, cSign); nNibblePos--; } do {
   * char cDigit = (char)((nValue % 10) + '0');
   * buffer.setCharAt(nPosition+nNibblePos+nOffset, cDigit); nNibblePos--; nValue
   * /= 10; } while (nValue != 0 && nNibblePos >= nMinIndex);
   * 
   * if(bSigned && bSignLeading) { nNibblePos = 0;
   * buffer.setCharAt(nPosition+nNibblePos+nOffset, cSign); nNibblePos--; } }
   */
  ...
  // static private void addCharRightToLeft(VarBufferPos buffer, int nPosition,
  // int nNibblePos, int nOffset, char cDigitValue)
  // {
