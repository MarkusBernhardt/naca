 ...
 */
package com.github.markusbernhardt.naca.rt.idea.action;
...
import com.github.markusbernhardt.naca.rt.idea.emulweb.CEmulMapFieldLoader;
import com.github.markusbernhardt.naca.rt.idea.emulweb.CScenarioPlayer;
...
import com.github.markusbernhardt.naca.rt.idea.view.View;
...
public class ActionCompat extends Action {
  ...
  public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
    if (m_prgseq == null) {
      ...
      response.getOutputStream()
              .println("Internal Error : ProgramSequencer not valid");
      return null;
    }

    HttpSession javaSession = request.getSession(true);
    /*
     * OnlineSession appSession = null ; appSession =
     * (OnlineSession)javaSession.getAttribute("AppSession"); if (appSession ==
     * null) { appSession = new OnlineSession(false) ;
     * javaSession.setAttribute("AppSession", appSession);
     * appSession.setInputWrapper(new CEmulMapFieldLoader()); } else { // create
     * wrapper for form fields CHTTPMapFieldLoader reqLoader = new
     * CHTTPMapFieldLoader(request); appSession.setInputWrapper(reqLoader); }
     * 
     * if(appSession.blockUntilLocked()) { // 1st session has finished return
     * appSession.m_actionForward; }
     */
    ...
    appSession.m_actionForward = doExecute(appSession, javaSession, mapping, form, request, response);
  ...
  private ActionForward doExecute(OnlineSession appSession, HttpSession javaSession, ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (csPrintScreen != null && csPrintScreen.equals("requested")) {
      ...
      appSession.getXMLData()
                .getDocumentElement()
                .setAttribute("printScreen", "show");
      appSession.setActionAlias("naca.do");
      return mapping.findForward("ViewCompat");
    }
    ...
    if (!appSession.isLogged() && !appSession.isPlayingScenario()) {
      if (appSession.getLUName() == null || appSession.getLUName()
                                                      .equals("")) {
    ...
    // test if we are playing a scenario, and if interaction with program has to be
    // done
    if (appSession.isCallProgram()) {
      ...
      if (m_inputAnalyser.BuildXMLData(appSession)) {
        try {
          ...
          // if (!cs.startsWith("nacaLib") && !cs.startsWith("idea") &&
          // !cs.startsWith("org")
  ...
  // public void RunClientRequest(OnlineSession appSession) throws
  // AbortSessionException
  // {
  // // build XML Data
