...
import com.github.markusbernhardt.naca.rt.fpacPrgEnv.FPacVarManager;
...
public abstract class VarBase extends CJMapObject {
  ...
  // private void dupVarDefShiftingAbsoluteStartPosition(int nShift,
  // ArrayList<VarDefBase> arrVarShifted)
  // {
  ...
  // VarBase varChild =
  // m_bufferPos.getProgramManager().getVarFullName(varDefRedefines);
  ...
  // void assignBuffer(BaseProgramManager programManagerDest, VarBuffer
  // bufferSource)
  // {
  // Log.logCritical("var buffer assigned"+toString());
  // m_bufferPos = new VarBufferPos(programManagerDest, bufferSource,
  // m_varDef.m_nDefaultAbsolutePosition);
  //
  ...
  public void setAtAdress(VarAndEdit varSource) {
    ...
    BaseProgramManager pm = TempCacheLocator.getTLSTempCache()
                                            .getProgramManager();
  ...
  public void setCustomBuffer(char[] cBuffer) {
    // Old Code
    // if(m_varDef.getLevel() != 1) // Only level 01 can have a custom buffer; their
    // children are also mapped the the buffer
    // return ;
    //
    // BaseProgramManager programManager = m_bufferPos.getProgramManager();
    //
    // // Create a VarBuffer and set it as the buffer of this
    // VarBuffer varBuffer = new VarBuffer(programManager, cBuffer); // But it has
    // it's own private var buffer
    // int nShift = m_bufferPos.m_nAbsolutePosition;
    //
    // assignBuffer(programManager, varBuffer); // The current var and all it's
    // children must use the current var buffer
    // ArrayList<VarDefBase> arrVarShifted = new ArrayList<VarDefBase>();
    // dupVarDefShiftingAbsoluteStartPosition(0 - nShift, arrVarShifted);

    // New code
    if (m_varDef.getLevel() != 1) // Only level 01 can have a custom buffer; their children are also mapped the
                                  // the buffer
      return;
  ...
  // tempCache.getSharedProgramInstanceData();
  // m_varDef.moveCorrespondingItemAndChildren(sharedProgramInstanceData,
  // tempCache.getProgramManager(), varDefDestGroup, nSourceOffset, nDestOffset);
  // //m_varDef.initializeItemAndChildren(m_bufferPos.getProgramManager(),
  // initializeManagerManager, nOffset);
  // }
  ...
  public void moveCorresponding(MoveCorrespondingEntryManager moveCorrespondingEntryManager, VarBase varDestGroup) {
    ...
    if (moveCorrespondingEntryManager != null && moveCorrespondingEntryManager.isFilled()) {
      ...
      m_varDef.moveCorrespondingItemAndChildren(moveCorrespondingEntryManager, sharedProgramInstanceData, tempCache.getProgramManager(), varDefDestGroup,
  ...
  // {
  // TempCache tempCache = TempCacheLocator.getTLSTempCache();
  // InitializeManager initializeManagerManager =
  // tempCache.getInitializeManagerNone();
  //
  // int nOffset = getInitializeReplacingOffset(tempCache);
  // m_varDef.initializeItemAndChildren(m_bufferPos, initializeManagerManager,
  // nOffset);
  // }
  ...
  // {
  // String cs =
  // m_varDef.getFullName(m_bufferPos.getProgramManager().getSharedProgramInstanceData());
  // return cs;
  // }
  // return "";
  // }
  ...
  // public void copyBytesFromSourceIntoBodyAndHeader(InternalCharBuffer
  // charBuffer)
  // {
  // m_varDef.copyBytesFromSource(m_bufferPos, m_bufferPos.m_nAbsolutePosition,
  // charBuffer);
  // }
  ...
  public void setSemanticContextValue(String csValue) {
    // if(m_bufferPos != null)
    // m_bufferPos.setSemanticContextValue(csValue,
    // m_bufferPos.m_nAbsolutePosition);
  }

  public String getSemanticContextValue() {
    // if(m_bufferPos != null)
    // {
    // String csSemanticValue =
    // m_bufferPos.getSemanticContextValue(m_bufferPos.m_nAbsolutePosition);
  ...
  public String getSemanticContextValue(int nAbsolutePosition) {
    ...
    // {
    // String csSemanticValue =
    // m_bufferPos.getSemanticContextValue(nAbsolutePosition);
    // return csSemanticValue;
    // }
    return "";
  }
  ...
  public void setFromLineRead2DestWithFilling(LineRead lineRead, VarBase varDest2) {
    ...
    if (nSourceLength > nDest1Length) // More data than buffer: We must limit the data to copy in the buffer1
      nSourceLength = nDest1Length; // Source length is limited by number of bytes of the destination1 (that is the
                                    // variable specified by into() method

    int nFillLength2 = 0;
    int nDest2Length = varDest2.getBodyLength();

    if (nDest1Length < nDest2Length) // The length of destination variable 2 (file descriptor) is longer than the
                                     // length of the variable specified by into()
      nFillLength2 = nDest1Length - nDest2Length; // We must fill the remaining bytes of varDest2

    if (nDest1Length > nDest2Length) // The length of destination variable 2 (file descriptor) is shoreter than the
                                     // length of the variable specified by into()
  ...
  /* protected */ public VarDefBuffer m_varDef = null;
