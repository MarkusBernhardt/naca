...
import com.github.markusbernhardt.naca.rt.batchPrgEnv.BatchProgramManagerFactory;
...
public abstract class FPacProgram extends BaseProgram {
  ...
  public void procedureDivision() // Virtual that can be derived
  {
    ...
    if (v != null && v.getInt() != 0) {
      ...
      CCommarea commarea = getProgramManager().getEnv()
                                              .getCommarea();
  ...
  // {
  // return new VarFPacNoTypeUndef(fd.getFPacVarManager(), fd.getVarBuffer(),
  // nAbsolutePosition);
  // }
  //
  ...
  private boolean isPackedHexa(String cs) // Autodermine if the cs value is a packed one or a string described in
                                          // hexadecimal codes
  {
