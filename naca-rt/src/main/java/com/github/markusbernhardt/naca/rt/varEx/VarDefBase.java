...
import com.github.markusbernhardt.naca.rt.exceptions.OccursOverflowException;
...
/**
 * @author U930DI TODO To change the template for this generated type comment go
...
public abstract class VarDefBase extends CJMapObject // implements Serializable
{
  public VarDefBase(VarDefBase varDefParent, VarLevel varLevel) {
    boolean bWSVar = !varLevel.getProgramManager()
  ...
  public int calcSizeVarInEdit() {
    ...
    if (m_varDefFormRedefineOrigin != null) // Var in a map redefine
    {
      ...
      if (!isEditInMapRedefine()) // do not change the size of the edit in map redefine, only the size of the var
                                  // groups in an edit of a map redefine
        m_nTotalSize = n;
    }
    return m_nTotalSize;
  }
  ...
  public void calcOccursOwners() {
    if (m_occursItemSettings != null && m_occursItemSettings.m_arrVarDefOccursOwner != null) {
      ...
      for (int n = 0; n < nNbDimensions; n++) {
        ...
        m_occursItemSettings.m_aOccursOwnerLocation[n] = new OccursOwnerLocation(nDistanceFromOccursOwner, varDefOccursOwner.m_nDefaultAbsolutePosition,
            nSignleEntrySize);
  ...
  private void calcAbsolutePosition(SharedProgramInstanceData sharedProgramInstanceData) {
    ...
    if (m_varDefRedefinOrigin != null) // We are a redefine
    {
      if (isVarInMapRedefine() && m_varDefRedefinOrigin.isEditInMapRedefine()) {
        // We are a var that redefines an edit; The var must point to the text part of
        // the edit, not in the attribute header
  ...
  void adjustSetting(VarDefBuffer varDefBufferCopySingleItem, int nAbsStart, int nDebugIndexes, int nNbDim, VarDefBase varDefOccursParent) {
    ...
    varDefBufferCopySingleItem.m_arrChildren = m_arrChildren; // PJD; Was = null, but assigned to children array because of ebcdic comparison
                                                              // of occursed items. We need to have access to the children.
  ...
  VarDefBuffer getCachedGetAt(TempCache cache, int x) {
    ...
    if (cache != null) {
      ...
      if (coupleVarGetAt != null) {
        // Adjust varDefGetAt to m_varDef.getAt(x); It is already created in the correct
        // type
  ...
  private void checkIndex(VarDefBase varDef, int nIndexValue, String csIndexName) {
    ...
    // if(varDef.m_OccursDef != null && nIndexValue >=
    // varDef.m_OccursDef.getNbOccurs())
    {
  ...
  int getAbsStart(int nXBase0) {
    int nAbsStart = getAbsolutePositionOccursOwnerAtDim(0) + (nXBase0 * getSizeOccursOwnerOf1Entry(0)) + getDistanceFromOccursOwner(0);
    return nAbsStart;
  }
  ...
  int getAbsStart(int nXBase0, int nYBase0) {
    int nAbsStart = getAbsolutePositionOccursOwnerAtDim(1) + (nYBase0 * getSizeOccursOwnerOf1Entry(1)) + getDistanceFromOccursOwner(1)
        + (nXBase0 * getSizeOccursOwnerOf1Entry(0)) + getDistanceFromOccursOwner(0);
    return nAbsStart;
  }

  int getAbsStart(int nXBase0, int nYBase0, int nZBase0) {
    int n = getAbsolutePositionOccursOwnerAtDim(2) + (nZBase0 * getSizeOccursOwnerOf1Entry(2)) + getDistanceFromOccursOwner(2)
        + (nYBase0 * getSizeOccursOwnerOf1Entry(1)) + getDistanceFromOccursOwner(1) + (nXBase0 * getSizeOccursOwnerOf1Entry(0)) + getDistanceFromOccursOwner(0);
    return n;
  }
  ...
  public VarDefBase getUnprefixNamedChild(SharedProgramInstanceData sharedProgramInstanceData, String csName, IntegerRef rnChildIndex) {
    ...
    for (int nIndex = 0; nIndex < nNbChildren; nIndex++) {
      VarDefBase varDefChild = getChild(nIndex);
      String csChildName = varDefChild.getUnprefixedName(sharedProgramInstanceData)
                                      .toUpperCase();
    ...
    for (int n = 0; n < nNbChildren && !foundFlag.isFound(); n++) {
      ...
      if (!foundFlag.isFound()) {
        ...
        if (varDefChild.isVarInMapRedefine() && varDefChild.m_varDefRedefinOrigin != null) // we are a var redefine, and we know what we redefines
        {
          ...
          {
            int nNbEditAlredayCounted = varDefChild.m_varDefRedefinOrigin.getNbEdit(); // Number of items alreday counted in the var redefine origin: it must
  ...
  // public void serializeDetails(ObjectOutputStream out, Hashtable<VarDefBase,
  // Integer> hashVarDefById, int nId) throws IOException
  // {
  ...
  //
  // serializeVarDefId(out, hashVarDefById, m_varDefParent); // Serialized our
  // parent's id
  ...
  // private void serializeArrayVarDef(ObjectOutputStream out,
  // Hashtable<VarDefBase, Integer> hashVarDefById, ArrayList<VarDefBase> arr)
  // throws IOException
  // {
  // int nNbChildren = 0;
  // if(arr != null)
  ...
  // public void deserializeDetails(ObjectInputStream in, ArrayList<VarDefBuffer>
  // arrVarDef, int nId) throws IOException, ClassNotFoundException
  // {
  ...
  // ArrayList<VarDefBase> findArrVarDefFromId(ObjectInputStream in,
  // ArrayList<VarDefBuffer> arrVarDef) throws IOException
  // {
  ...
  // private void serializeVarDefId(ObjectOutputStream out, Hashtable<VarDefBase,
  // Integer> hashVarDefById, VarDefBase varDef) throws IOException
  // {
  // if(varDef != null)
  // {
  ...
  // VarDefBase findVarDefFromId(ObjectInputStream in, ArrayList<VarDefBuffer>
  // arrVarDef) throws IOException
  // {
  // if(arrVarDef != null)
  // {
  // int nId = in.readInt();
  ...
  public String toString() {
    BaseProgramManager programManager = TempCacheLocator.getTLSTempCache()
                                                        .getProgramManager();
  ...
  private int m_n_PreviousSameLevel_Id = 0; // high short:m_varDefPreviousSameLevel id; low short: Id of the variable's an
                                            // index in SharedProgramInstanceData m_arrVarName array
  // Grouping:
  // private VarDefBase m_varDefPreviousSameLevel = null; // Previous VarDef at
  // the same level
  // private int m_nId;
