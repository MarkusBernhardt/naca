...
public class FPacFileDescriptor extends BaseFileDescriptor {
  ...
  public FPacFileDescriptor(FPacProgram program, String csLogicalName) {
    super(program.getProgramManager()
                 .getEnv(),
        csLogicalName);
    init(program);
  }
  ...
  public RecordDescriptorAtEnd read() {
    ...
    if (m_fileManagerEntry.isVariableLength()) // Variable size record
    {
      ...
      if (header != null) {
        ...
        int nHeaderLength = m_varBuffer.setFromLineRead(header, 0); // write the record after the record length at the beginning; it includes the
                                                                    // length itself
  ...
  private void doWrite() {
    if (!m_fileManagerEntry.isVariableLength()) // Constant record size
    {
      ...
      {
        int nRecordLength = m_varBuffer.getIntAt(0); // Read record length encoded in the 4 leading bytes; it doesn't includes the
                                                     // record header itself, nor the trailing LF
