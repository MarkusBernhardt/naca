...
public class VarDefNumIntComp0 extends VarDefNum {
  ...
  void writeIntComp0(VarBufferPos buffer, int nValue) {
    ...
    // Pic9Comp0BufferSupport pic9Comp0BufferSupport = new
    // Pic9Comp0BufferSupport(buffer, buffer.m_nAbsolutePosition, m_nTotalSize,
    // m_nTotalSize);
    // pic9Comp0BufferSupport.setFromRightToLeft(0, nValue);
  }

  void writeIntComp0(VarBufferPos buffer, int nOffset, int nValue) {
    // RWNumIntComp0.internalWriteAbsoluteIntComp0(buffer, nOffset, nValue,
    // buffer.m_nAbsolutePosition, m_nTotalSize);
    if (!m_bBlankWhenZero)
  ...
  void writeIntComp0AsLong(VarBufferPos buffer, long lValue) {
    // RWNumIntComp0.internalWriteAbsoluteIntComp0AsLong(buffer, lValue,
    // buffer.m_nAbsolutePosition, m_nTotalSize);
    if (!m_bBlankWhenZero)
  ...
  int compare(ComparisonMode mode, VarBufferPos buffer2, VarDefX varDef1, VarBufferPos buffer1) {
    // see
    // http://publibz.boulder.ibm.com/cgi-bin/bookmgr_OS390/BOOKS/IGYLR205/6.1.6.5.3?SHELF=&DT=20000927030801&CASE=
    // see
