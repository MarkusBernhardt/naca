...
public class VarDefX extends VarDefVariable {
  ...
  void write(VarBufferPos buffer, VarDefNumDecSignComp0 varSource, VarBufferPos bufferSource) {
    // String cs = varSource.getAsAlphaNumString(bufferSource);
    // writeRightPadding(buffer, cs, ' '); // @see
    // http://www.helsinki.fi/atk/unix/dec_manuals/cobv27ua/cobrm_029.htm#index_x_737
    // Parag 9.c: A noninteger numeric literal or data item cannot be moved to an
    // alphanumeric or alphanumeric edited data item.
  }
  ...
  // private int writeRightPadding(VarBufferPos buffer, int nOffset, String cs,
  // char cPad)
  // {
  // return internalWriteRightPadding(buffer, buffer.m_nAbsolutePosition+nOffset,
  // m_nTotalSize, cs, cPad);
  // }
  ...
  // public void initializeAtOffset(VarBufferPos buffer, int nOffset,
  // InitializeCache initializeCache)
  // {
  // internalWriteRightPaddingBlankInitSpace(buffer,
  // buffer.m_nAbsolutePosition+nOffset, m_nTotalSize, initializeCache);
  // //writeRightPadding(buffer, nOffset, " ", ' ');
  // }
