...
public class DataDivision extends Division {
  ...
  public VarBuffer manageWorkingLinkageVars(BaseProgram program, boolean bFirstInstance, ArrayList<CCallParam> arrCallerCallParam,
    ...
    if (m_FileSection != null) {
      ...
      program.getProgramManager()
             .assignBufferFile(varBufferFile);
    }
    VarBuffer varBufferLS = computeLinkageVarBuffer();

    program.getProgramManager()
           .assignBufferWS(varBufferWS);

    program.getProgramManager()
           .assignBufferLS(varBufferLS);

    if (bFirstInstance)
      m_WorkingStorageSection.fillWorkingInitialValues(program.getProgramManager()
  ...
  public VarBuffer computeLinkageVarBuffer() {
    return m_LinkageSection.computeStorage(true); // Compute Linkage section vars that are not already set with an arg provided By
                                                  // Ref
  }
