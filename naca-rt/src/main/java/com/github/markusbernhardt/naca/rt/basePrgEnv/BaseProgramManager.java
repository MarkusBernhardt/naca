...
import com.github.markusbernhardt.jlib.misc.ArrayDyn;
import com.github.markusbernhardt.jlib.misc.ArrayFix;
import com.github.markusbernhardt.jlib.misc.ArrayFixDyn;
import com.github.markusbernhardt.jlib.misc.Time_ms;
import com.github.markusbernhardt.naca.rt.base.CJMapObject;
import com.github.markusbernhardt.naca.rt.calledPrgSupport.BaseCalledPrgPublicArgPositioned;
import com.github.markusbernhardt.naca.rt.exceptions.CESMReturnException;
import com.github.markusbernhardt.naca.rt.exceptions.CGotoException;
import com.github.markusbernhardt.naca.rt.exceptions.CGotoOtherSectionException;
import com.github.markusbernhardt.naca.rt.exceptions.CGotoOtherSectionParagraphException;
import com.github.markusbernhardt.naca.rt.exceptions.NacaRTException;
...
import com.github.markusbernhardt.naca.rt.program.Copy;
import com.github.markusbernhardt.naca.rt.program.CopyManager;
...
import com.github.markusbernhardt.naca.rt.programStructure.DataDivision;
import com.github.markusbernhardt.naca.rt.programStructure.DataSectionFile;
import com.github.markusbernhardt.naca.rt.programStructure.Division;
...
import com.github.markusbernhardt.naca.rt.sqlSupport.SQLExecuteStart;
...
import com.github.markusbernhardt.naca.rt.varEx.CCallParam;
...
import com.github.markusbernhardt.naca.rt.varEx.EditInMap;
...
import com.github.markusbernhardt.naca.rt.varEx.VarDefBase;
import com.github.markusbernhardt.naca.rt.varEx.VarDefBuffer;
import com.github.markusbernhardt.naca.rt.varEx.VarInternalInt;
...
public abstract class BaseProgramManager extends CJMapObject {
  ...
  public BaseProgram prepareCall(BaseProgramLoader baseProgramLoader, BaseProgram currentProgram, ArrayList arrCallerCallParam, BaseEnvironment env,
    ...
    if (env != null && env.getCommarea() != null) {
      ...
      CCallParam CallParam = env.getCommarea()
                                .buildCallParam();
      arrCallerCallParam.add(CallParam);
    }

    // int nParametersTotalLength =
    // calcCallParametersTotalLength(arrCallerCallParam);
            ...
            if (csTypeName.equals("nacaLib.varEx.Var")) {
              ...
              if (var != null) {
                if (var.getVarDef() != null) {
                  if (bSetPrefix) {
                    m_sharedProgramInstanceData.setVarFullName(var.getVarDef()
                                                                  .getId(),
                        csPrefixeName + csName);
                  } else {
                    m_sharedProgramInstanceData.setVarFullName(var.getVarDef()
                                                                  .getId(),
                        csName);
              ...
              Form form = (Form) obj;
              if (bSetPrefix) {
                m_sharedProgramInstanceData.setVarFullName(form.getVarDef()
                                                               .getId(),
                    csPrefixeName + csName);
              } else {
                m_sharedProgramInstanceData.setVarFullName(form.getVarDef()
              ...
              if (edit != null) {
                if (bSetPrefix) {
                  m_sharedProgramInstanceData.setVarFullName(edit.getVarDef()
                                                                 .getId(),
                      csPrefixeName + csName);
                } else {
                  m_sharedProgramInstanceData.setVarFullName(edit.getVarDef()
              ...
              if (mapRedefine != null) {
                if (bSetPrefix) {
                  m_sharedProgramInstanceData.setVarFullName(mapRedefine.getVarDef()
                                                                        .getId(),
                      csPrefixeName + csName);
                } else {
                  m_sharedProgramInstanceData.setVarFullName(mapRedefine.getVarDef()
  ...
  public void performThrough(Paragraph paragraphBegin, Paragraph paragraphEnd) {
    // Enum all paragraphs that are between functorBegin and functorEnd, whatever
    // their sections
  ...
  private void logSevereErrorNext(String csTitle, String csText) {
    ...
    sbText.append("\r\n");
    if (TempCacheLocator.getTLSTempCache()
                        .getProgramManager() != this)
  ...
  // private Hashtable<String, VarBase> m_hashVarsFullName = null; // hash table
  // of the varBase indexed by their full name
  ...
  // {
  // String csFileLine = StackStraceSupport.getFileLineAtFirstAppCall(); // Caller
  // File Line
  ...
  public CSQLStatus sqlRollback() {
    ...
    if (e != null) {
      // String csFileLine = StackStraceSupport.getFileLineAtStackDepth(3); // Caller
      // File Line
      if (m_sqlStatus == null)
        m_sqlStatus = new CSQLStatus();
  ...
  public void determineCommareaLength(BaseEnvironment env) {
    if (env == null || env.getCommarea() == null) {
      ...
      m_varEIBCALEN.set(env.getCommarea()
                           .getLength());
    }
  }
  ...
  private SortParagHandler m_sortParagHandler = null; // object wrapping a SortCommand for supporting release call with input
                                                      // paragraphs.
