...
import com.github.markusbernhardt.jlib.sql.SQLTypeOperation;
import com.github.markusbernhardt.naca.rt.accounting.AccountingRecordTrans;
...
public class SQL {
   ...
   * @return this Defines the destination variable (varDestCol) that will receive
   *         a recordser column value after a select, with cursor or not The
   *         varIndicator variable will be set to -1 if the recordset column value
   *         is SQL NULL The varIndicator variable will be set to 0 if the
   *         recordset column value is not SQL NULL These into() methods must
   *         match exactly the column name described in the SQL select clause
   */
   ...
   * @param int
   *          nValue: Value given
   * @return this Used to provided values to a insert or update SQL clause. The
   *         nName identifies the column set exemple (in this case, see 1st
   *         .value() call) : sql("insert into Entries (Id, Author, Text) VALUES
   *         (#1, #2, #3)") .value(1, 2) .value(2, "toto") .value(3, VText)
   */
   ...
   *         nName identifies the column set exemple (in this case, see 1st
   *         .value() call) : sql("insert into Entries (Id, Author, Text) VALUES
   *         (#a, #b, #c)") .value("a", 2) .value("b", "toto") .value("c", VText)
   */
   ...
   * @return this Used to provided values to a insert or update SQL clause. The
   *         nName identifies the column set exemple (in this case, see 1st
   *         .value() call) : sql("insert into Entries (price, Author, Text)
   *         VALUES (#1, #2, #3)") .value(1, 3.14) .value(2, "toto") .value(3,
   *         VText)
   */
   ...
   *         nName identifies the column set exemple (in this case, see 1st
   *         .value() call) : sql("insert into Entries (price, Author, Text)
   *         VALUES (#a, #b, #c)") .value("a", 3.14) .value("b", "toto")
   *         .value("c", VText)
   */
   ...
   *          csValue: Value given
   * @return this Used to provided values to a insert or update SQL clause. The
   *         nName identifies the column set exemple (in this case, see 2nd
   *         .value() call) : sql("insert into Entries (price, Author, Text)
   *         VALUES (#1, #2, #3)") .value(1, 3.14) .value(2, "toto") .value(3,
   *         VText)
   */
   ...
   * @return this Used to provided values to a insert or update SQL clause. The
   *         nName identifies the column set exemple (in this case, see 3rd
   *         .value() call) : sql("insert into Entries (price, Author, Text)
   *         VALUES (#1, #2, #3)") .value(1, 3.14) .value(2, "toto") .value(3,
   *         VText)
   */
   ...
   *         SELECT (with cursor or not), UPDATE or DELETE exemple (: sql("update
   *         Entries SET Text=#1, Author=#2 WHERE Id=#3 or Id=#4") .value(1,
   *         VText) .value(2, VAuthor) .param(3, VCurrentId) .param(4, 2);
   */
   ...
   * @return this USed to set a parameter's value in a SQL WHERE clause of a
   *         SELECT (with cursor or not), UPDATE or DELETE exemple (: sql("update
   *         Entries SET Text=#1, Author=#2 WHERE Id=#a or Id=#b") .value(1,
   *         VText) .value(2, VAuthor) .param("a", VCurrentId) .param("b", 2);
   */
   ...
   * @return this USed to set a parameter's value in a SQL WHERE clause of a
   *         SELECT (with cursor or not), UPDATE or DELETE exemple (see 2nd .param
   *         method call): sql("update Entries SET Text=#1, Author=#2 WHERE Id=#a
   *         or Id=#b") .value(1, VText) .value(2, VAuthor) .param("a",
   *         VCurrentId) .param("b", 2);
   */
   ...
   *         SELECT (with cursor or not), UPDATE or DELETE exemple (see 2nd .param
   *         method call): sql("update Entries SET Text=#1, Author=#2 WHERE Id=#a
   *         or Price>#b") .value(1, VText) .value(2, VAuthor) .param("a",
   *         VCurrentId) .param("b", 5.5);
   */
   ...
   *         SELECT (with cursor or not), UPDATE or DELETE exemple (see 2nd .param
   *         method call): sql("update Entries SET Text=#1, Author=#2 WHERE Id=#a
   *         or Town=#b") .value(1, VText) .value(2, VAuthor) .param("a",
   *         VCurrentId) .param("b", "Geneva");
   */
  ...
  private int getNbTables() {
    ...
    if (nPosFrom != -1) {
      ...
      if (nPosEnd != -1)
        csTables = m_csQueryUpper.substring(nPosFrom, nPosEnd)
                                 .trim();
      else
