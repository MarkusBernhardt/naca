...
public class VarDefNumDecSignTrailingComp0 extends VarDefNum {
  ...
  CStr getAsAlphaNumString(VarBufferPos buffer) {
    ...
    CStr cs3 = TempCacheLocator.getTLSTempCache()
                               .getReusableCStr();
  ...
  // private long internalReadSignSeparatedTrailingIntComp0AsLong(VarBufferPos
  // buffer)
  // {
  ...
  private Dec internalReadSignSeparatedTrailingDecComp0(VarBufferPos buffer) {
    ...
    // char cSign =
    // buffer.getCharAt(buffer.m_nAbsolutePosition+m_nNbDigitInteger+m_nNbDigitDecimal);
    if (cSign == '-')
  ...
  private int writeSignSeparatedTrailingDecComp0(VarBufferPos buffer, int nOffset, Dec decValue) {
    int nPosition = RWNumIntComp0.internalWriteAbsoluteIntComp0AsLong(buffer, nOffset, decValue.getUnsignedLong(), buffer.m_nAbsolutePosition,
        m_nNbDigitInteger);
