...
import com.github.markusbernhardt.jlib.xml.TagCursor;
...
public class CustomApplicationLauncherAction extends Action {
  ...
  private String GetValFromRef(String ref, Tag tagParam, OnlineSession appSession) {
    ...
    if (prgseq != null) {
      ...
      // }
        ...
        String val = "";
        String[] fields = tagParam.getVal("field")
                                  .split("#");
  ...
  private String GetValFromField(String field, OnlineSession appSession) {
    ...
    if (posSubstr != -1) {
      ...
      fieldWork = field.substring(posSubstr, posFirst)
                       .trim();
      if (posSecond == -1) {
        beginIndex = new Integer(field.substring(posFirst + 1, posEnd)
                                      .trim()).intValue();
      } else {
        beginIndex = new Integer(field.substring(posFirst + 1, posSecond)
                                      .trim()).intValue();
        endIndex = new Integer(field.substring(posSecond + 1, posEnd)
                                    .trim()).intValue();
      }
    } else {
      fieldWork = field;
