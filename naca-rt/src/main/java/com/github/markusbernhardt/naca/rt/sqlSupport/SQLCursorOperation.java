...
public class SQLCursorOperation {
  // public SQLCursorOperation(CSQLConnection sqlConnection, VarBuffer Working,
  // SQLCursor sqlCursor, String csClause, CSQLStatus sqlStatus)
  public SQLCursorOperation(BaseProgramManager programManager, SQLCursor sqlCursor, String csClause) {
    DbConnectionBase sqlConnection = programManager.getEnv()
                                                   .getSQLConnection();
    ...
    } else // Row Id must have been generated in the cursor
    {
      // PJD ROWID Support:
      /*
       * ROWID rowId = sqlCursor.getCurrentRowId(); if(rowId != null) { csClause +=
       * " WHERE ROWID=?"; m_sqlUpdateDelete = new SQL(Working, sqlConnection,
       * csClause, false); m_sqlUpdateDelete.setCurrentRowId(rowId); }
       */
    }
