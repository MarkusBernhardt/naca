...
public class VarDefNumDecComp0 extends VarDefNum {
  ...
  Dec getAsDecodedDec(VarBufferPos buffer) {
    ...
    // BufChunk bufChunkDec =
    // buffer.getBufChunkAt(buffer.m_nAbsolutePosition+m_nNbDigitInteger,
    // m_nNbDigitDecimal);
  ...
  CStr getAsAlphaNumString(VarBufferPos buffer) {
    ...
    CStr cstr = TempCacheLocator.getTLSTempCache()
                                .getReusableCStr();
  ...
  private int writeDecComp0(VarBufferPos buffer, Dec decValue) {
    // return internalWriteDecComp0(buffer, decValue, m_nNbDigitInteger,
    // m_nNbDigitDecimal);

    // RWNumIntComp0.setFromRightToLeft(buffer, 0, decValue.getUnsignedLong(),
    // m_nTotalSize, m_nNbDigitInteger);
