...
import com.github.markusbernhardt.jlib.misc.CurrentDateInfo;
...
import com.github.markusbernhardt.jlib.sql.SQLLoadStatus;
import com.github.markusbernhardt.jlib.threads.ThreadPoolRequest;
import com.github.markusbernhardt.naca.rt.sqlSupport.RecordColTypeManagerBase;
import com.github.markusbernhardt.naca.rt.sqlSupport.RecordColTypeManagerChar;
import com.github.markusbernhardt.naca.rt.sqlSupport.RecordColTypeManagerDate;
import com.github.markusbernhardt.naca.rt.sqlSupport.RecordColTypeManagerDecimal;
import com.github.markusbernhardt.naca.rt.sqlSupport.RecordColTypeManagerDecimalInt;
import com.github.markusbernhardt.naca.rt.sqlSupport.RecordColTypeManagerDecimalLong;
import com.github.markusbernhardt.naca.rt.sqlSupport.RecordColTypeManagerOther;
import com.github.markusbernhardt.naca.rt.sqlSupport.RecordColTypeManagerTimestamp;
import com.github.markusbernhardt.naca.rt.sqlSupport.RecordColTypeManagerVarchar;
...
public class TableToTransfer extends ThreadPoolRequest {
        ...
        try {
          ...
          while (resultSet.next()) {
            ...
            for (int nColumn = 0; nColumn < nNbColumns; nColumn++) {
              ...
              if (!bCol) {
                String cs = "Problem during transfer of a column. Source table=" + csPrefixedTableName + "; Destination table=" + csDestinationTableName
                    + "; Column name=" + arrColNames.get(nColumn) + "; Clause=" + csClause + "; Record number=" + nNbRecordRead;
                Log.logCritical(cs);
                sbSQLError = appendIfPossible(sbSQLError, cs);
              }
              b |= bCol;
            ...
            if (b) {
              ...
              if (dbInsertStatement.getBatchSize() == nBatchSize) {
                SQLLoadStatus status = doWriteBatchRecords(dbInsertStatement, nNbRecordRead, nBatchSize, sbSQLError, csPrefixedTableName,
          ...
          if (dbInsertStatement != null) {
            ...
            if (status.isSuccess()) {
              ...
              String cs = "Execute batch insert error for records. Status=" + status.toString() + "Source table=" + csPrefixedTableName + "; Destination table="
                  + csDestinationTableName + "; Record number range" + nMin + ", " + nNbRecordRead + "]";
              Log.logCritical(cs);
              sbSQLError = appendIfPossible(sbSQLError, cs);
              dbTransferDesc.setTransferGlobalFailure();
            }
          ...
          String cs = "Exception catched during transfer. Source table=" + csPrefixedTableName + "; Destination table=" + csDestinationTableName + "; Clause="
              + csClause + "; Exception catched=" + e.getMessage();
          sbSQLError = appendIfPossible(sbSQLError, cs);
          Log.logCritical(cs);
          dbTransferDesc.setTransferGlobalFailure();
        }
  ...
  private SQLLoadStatus doWriteBatchRecords(DbPreparedStatement dbInsertStatement, int nNbRecordRead, int nBatchSize, StringBuilder sbSQLError,
      String csPrefixedTableName, String csDestinationTableName) {
    int nMin = nNbRecordRead - nBatchSize;
  ...
  private synchronized void reportTransfer(DbConnectionBase dbConnectionSource, DbConnectionBase dbConnectionDestination, int nNbRecordRead,
      StringBuilder sbSQLError, String csPrefixedTableName, StopWatch sw, String csDestinationTableName) {
    // clause is "update m_csDefinitionTable set LASTWRITE=?, NBREAD=?, NBWRITE=?,
    // SQLERROR=? Where TNAME='xxx'"
    ...
    Log.logCritical("Finished transfer from source table " + csPrefixedTableName + " to destination table " + csDestinationTableName + "; Records read="
        + nNbRecordRead + "; Records written=" + nNbRecordWritten + "; Transfet Time=" + sw.getElapsedTime() + " ms; Text error=" + csError);
  ...
  private DbPreparedStatement getInsertStatement(DbConnectionBase dbConnectionDestination, ResultSet resultSet, int nNbColumns,
      ResultSetMetaData resultSetMetaData, ArrayList<String> arrColNames) {
    try {
