...
public class DataSection extends CJMapObject {
  ...
  private void createRootVar(BaseProgram prg, String csSuffix) {
    ...
    if (m_RootVar.m_varDef != null) {
      pm.getSharedProgramInstanceData()
        .setVarFullName(m_RootVar.getVarDef()
                                 .getId(),
            "%InternalRoot_" + csSuffix + "%");
    }
  }
  ...
  public VarBuffer computeStorage(boolean bFirstInstance) {
    ...
    SharedProgramInstanceData sharedProgramInstanceData = m_prg.getProgramManager()
                                                               .getSharedProgramInstanceData();
    int nBufferSize = 0;
    if (m_RootVar != null) {
      if (bFirstInstance) // The var defs of the catalog have not been already computed: 1st absolute run;
                          // the 2nd, 3rd... run of a program alreday loaded have access to the catalog of
                          // varDef: No need to recompute var def again.
      {
  ...
  public void fillWorkingInitialValues(SharedProgramInstanceData sharedProgramInstanceData) {
    ...
    if (cache != null && m_RootVar != null)
      m_RootVar.getVarDef()
               .fillInitialValueAndClearUnusedMembers(cache, sharedProgramInstanceData, m_Buffer);
  }
  ...
  public void dumpRootVar(String csSectionName) {
    if (IsSTCheck) {
      if (m_RootVar != null) {
        ...
        m_RootVar.getVarDef()
                 .dumpToSTCheck(m_prg.getProgramManager());
      }
    }
  }
