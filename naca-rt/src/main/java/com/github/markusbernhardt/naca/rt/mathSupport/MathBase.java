...
public class MathBase extends CJMapObject {
  /**
   * Constructor Base class for all math operations
   */
  public MathBase() {
  }

  public String getSTCheckValue() {
    String sValue = m_d.abs()
                       .unscaledValue()
                       .toString();
    return sValue;
  }
   ...
   * @param Var
   *          varDest
   * @return this Set the destination var to the result of the last math
   *         operation, adjusting precision, but without roundings
   */
  ...
  public MathBase to(VarAndEdit varDest) {
    ...
    varDest.set(dec);
    if (varDest.getVarDef()
               .getLength() < dec.toString()
                                 .length()) {
      m_bError = true;
    }
    return this;
   ...
   *          varDest
   * @return this Set the destination var to the result of the last math
   *         operation, adjusting precision, with roundings if needed by
  ...
  /**
   * @return true if an error has been set during one of the math operations done
   *         on the current objet Errors are set for divide by 0
   */
