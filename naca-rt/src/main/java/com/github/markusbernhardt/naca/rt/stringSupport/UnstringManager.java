 ...
 */
package com.github.markusbernhardt.naca.rt.stringSupport;
...
class UnstringManager {
  ...
  void doInto(Var varDelimiterDest, Var varDelimiterIn, Var varCountDest) {
    if (!m_bFailed) {
      ...
      if (m_arrDelimiters.isEmpty()) {
        ...
        int i = Math.min(varDelimiterDest.getLength(), m_csCurrentSource.length() - 1);
      ...
      if (delimiterUsed != null) // Found 1st delimitered string
      {
        ...
        incTallyingCount();
        nPointer0Based = delimiterUsed.removeDelimiterString(m_csCurrentSource, nPositionEndSepartorUsed); // Remove delimiter string, optionnally managing all
                                                                                                           // occurences
        ...
        // m_csCurrentSource =
        // delimiterUsed.getRemaingStringAfterSeparator(m_csCurrentSource); // Keep only
        // right part, after all separators
      }
  ...
  boolean failed() {
    if (m_bFailed)
      return m_bFailed;
    return checkIfRemainingUnfilledChunks(); // If we have some chunks left that have not been conummed by into calls(), then
                                             // we have an error
  }
}
