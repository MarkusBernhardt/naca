...
public abstract class BaseFileDescriptor extends CJMapObject {
  ...
  private Var m_varLengthDependingOn = null; // Variable declared by a FileDescriptorDepending: it gives the variable whose
                                             // value give the length of the dynamic part of the record
  ...
  private void computeSizeConstantRecordLength() {
    if (m_varVariableLengthMarker != null && m_varLevel01 != null) {
      ...
      m_nSizeOccursDependingOn = m_varVariableLengthMarker.getAt(1)
                                                          .getTotalSize();
    } else {
