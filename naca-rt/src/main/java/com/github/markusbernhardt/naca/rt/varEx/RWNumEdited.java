  ...
  static String internalFormatAndWrite(Dec dec, String csFormat, boolean bBlankWhenZero) {
    ...
    for (int nFormatIndex = nDecimalSeparatorFormatPos; nFormatIndex >= 0; nFormatIndex--) // From right to left for integer part
    {
      ...
      else if (cFormat == '0' || cFormat == '/' || cFormat == ',' || cFormat == '\'') // Warning, ',' stands for 1000 separator, not decimal dot !!!
      {
        ...
        bSuppressLeading0 = true;
        sDest.setCharAt(nFormatIndex, cSource); // 1st pass: recopy the source char; it will be suppressed in next pass if
                                                // needed
        nPosSource--;
      }
    }
    ...
    } else if (nPos$ != -1) // special case where there is no place left for the money sign, but we must set
                            // it insted of the forst digit
    {
      char cMoney = csFormat.charAt(nPos$);
    ...
    if (!bSignFilled) {
      try {
        ...
        if (cFormat == '+') {
          ...
          // sDest = sDest.deleteCharAt(nLgFormat-1); // Delete first char to have the
          // place to set the sign at the last position
