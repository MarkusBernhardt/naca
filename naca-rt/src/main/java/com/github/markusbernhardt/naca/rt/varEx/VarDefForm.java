      ...
      for (int nChild = 0; nChild < nNbChildren && nPos != -1; nChild++) // Skip first child, as it is the fake header
      {
        ...
        if (varDefChild != null) {
          if (varDefChild == varDefEdit) // The child is the current edit
          {
            ...
            // String csText =
            // varDefChild.getRawStringExcludingHeader(var2Edit.m_bufferPos); // getString
          ...
          } else // The child is a var that may be interleaved with edits, or after all edits
          {
            ...
            {
              // String csText =
              // varDefChild.getRawStringExcludingHeader(varChild.m_bufferPos);
              String csText = varChild.m_bufferPos.getBodyCStr(varDefChild)
                                                  .getAsString();
