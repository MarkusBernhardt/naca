...
public class BtreeFile {
  ...
  public boolean internalSortInsertWithRecordIndexAtEnd(byte tbyData[], int nSourceOffset, int nTotalLength, int nNbRecordRead, boolean bVariableLength) {
    if (m_nNbthreadsSort == 0) // No thread for sorting
    {
      ...
      // LittleEndingUnsignBinaryBufferStorage.writeInt(tbyKey, nNbRecordRead,
      // m_keyDescription.m_nKeyLength-4); // Intel format
  ...
  public boolean externalSortInsertWithRecordIndexAtEnd(Environment env, LineRead lineRead, int nNbRecordRead, boolean bFileInEbcdic,
    ...
    if (m_nNbthreadsSort == 0) // No thread for sorting
    {
      ...
      // MultiThreadedSortAddItem item = new MultiThreadedSortAddItem(this, tbyData,
      // nOffset, nTotalLength, nNbRecordRead, bFileInVariableLength);
      m_threadsPoolWriter.enqueue(item);
      return true;
    }
  }
  ...
  public boolean tryLaunchAsyncSortReader() {
    if (m_threadsPoolWriter != null) // We are using a pool of threads for adding items for sorting; wait until all
                                     // items have been completly added
    {
