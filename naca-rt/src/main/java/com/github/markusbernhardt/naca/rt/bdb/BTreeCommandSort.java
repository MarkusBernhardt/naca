...
public class BTreeCommandSort {
  ...
  // {
  // int nNbRecordRead = 0;
  // DataFileLineReader dataFileIn = new DataFileLineReader(m_csFileIn,
  // nBufferChunkReadAHead, 0);
  // boolean bInOpened = dataFileIn.open();
  // if(bInOpened)
  // {
  // boolean b = true;
  ...
  // {
  //// if(m_bFileInEbcdic)
  //// AsciiEbcdicConverter.swapByteEbcdicToAscii(lineRead.getBuffer(),
  // lineRead.getOffset(), lineRead.getTotalLength());
  //
  // //String cs = lineRead.getChunkAsString();
  //
  // b = btreeFile.externalSortInsertWithRecordIndexAtEnd(m_btreeEnv.getEnv(),
  // lineRead, nNbRecordRead, m_bFileInEbcdic);
  // lineRead = dataFileIn.readNextUnixLine();
  // nNbRecordRead++;
  // }
  // dataFileIn.close();
  // }
  // Log.logCritical("" + nNbRecordRead + " records imported into btree file from
  // " + m_csFileIn);
  //
  // return nNbRecordRead;
  // }
