...
public class EditInMap extends Edit {
  EditInMap(DeclareTypeEditInMap declareTypeEdit) {
    ...
    declareTypeEdit.getProgramManager()
                   .registerEditInMap(this);

    declareTypeEdit.registerEditInForm(this);

    m_csDeclaredEditName = declareTypeEdit.m_csName;
    // m_varDef.setDefaultFullName(declareTypeEdit.m_csName); // For debug puropose
    // only; will be replaced by edit's java variable name
  ...
  public Element exportXML(Document doc, String csLangId) {
    if (doc != null) {
      ...
      if (m_attrManager.m_csDevelopableMark != null && !m_attrManager.m_csDevelopableMark.equals("")
          && csFieldValue.indexOf(m_attrManager.m_csDevelopableMark) != -1) {
  ...
  public void saveEditAttributesInVarDef() {
    ...
    // setSemanticContextValue(m_attrManager.m_csSemanticContext); // Restore
    // original semantic context already saved in attributes
  }
  ...
  public void restoreEditAttributesInVarDef() {
    ...
    // setSemanticContextValue(varDefEdit.m_attrManager.m_csSemanticContext); //
    // Restore original semantic context saved in varDef attributes
  }
