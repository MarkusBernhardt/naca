...
import com.github.markusbernhardt.naca.rt.idea.manager.CMapFieldLoader;
...
public class CScenarioPlayer extends CJMapObject {
  ...
  public CScenarioPlayer(String filepath, OnlineSession session) {
    ...
    if (m_docScenario == null) {
      ...
      String title = m_docScenario.getDocumentElement()
                                  .getNodeName();
  ...
  public void StepScenario() {
    if (m_nPlayerState == 0) { // initial state : call program for first page
      m_nPlayerState = ScenarioPlayerState.CALL_PROGRAM;
    } else if (m_nPlayerState == ScenarioPlayerState.CALL_PROGRAM) { // program has been called, show the page with new fields
      m_nPlayerState = ScenarioPlayerState.SHOW_PAGE;
      Document data = getCurrentPage();
      m_session.setXMLData(data);
    } else if (m_nPlayerState == ScenarioPlayerState.SHOW_PAGE) { // the new page has been shown, call program with real values
      m_nPlayerState = ScenarioPlayerState.CALL_PROGRAM;
      Document data = getCurrentPage();
      CMapFieldLoader fieldLoader = m_session.getInputWrapper();
      KeyPressed kp = fieldLoader.getKeyPressed();
      if (kp == KeyPressed.ENTER) { // replace key pressed by scenario key pressed
        String k = data.getDocumentElement()
                       .getAttribute("keypressed");
        KeyPressed kp2 = KeyPressed.getKey(k);
        if (kp2 != null) {
          fieldLoader.setKeyPressed(kp2);
        }
      } else { // user hit some key : stop scenario
        m_docScenario = null;
        m_lstPages = null;
        m_nCurrentPage = 0;
        m_nPlayerState = 0;
  ...
  public String getDisplay() {
    if (m_docScenario != null && m_lstPages != null) {
      if (m_nCurrentPage < m_lstPages.getLength()) {
        if (m_nPlayerState == ScenarioPlayerState.CALL_PROGRAM) { // program has been called, show the page with new fields
  ...
  private void doLogPlaying(Document xmlOutput) {
    ...
    m_docScenarioPlayingLog.getDocumentElement()
                           .appendChild(ePage);
    ...
    if (m_modeRecord == ScenarioRecordDataMode.MODE_3270) {
      String lang = xmlOutput.getDocumentElement()
                             .getAttribute("lang");
      ...
      for (int i = 0; i < lst.getLength(); i++) {
        ...
        f.length = eField.getAttribute("Len");
        // String modified = eField.getAttribute("KeyHit");
        f.value = eField.getAttribute("Val")
                        .trim();
        f.modified = eField.getAttribute("Update");
    ...
    if (m_modeRecord == ScenarioRecordDataMode.MODE_3270) {
      ...
      for (int i = 0; i < lst.getLength(); i++) {
        ...
        if (ff != null) {
          ...
          // System.out.println("Label not found in original page : col="+f.poscol+" ;
          // line="+f.posline+" ; value="+f.value) ;
        }
      }
      lst = xmlOutput.getElementsByTagName("title");
      for (int i = 0; i < lst.getLength(); i++) {
        ...
        if (ff != null) {
          ...
          // System.out.println("title not found in original page : col="+f.poscol+" ;
          // line="+f.posline+" ; value="+f.value) ;
        }
      }
      ...
      try {
        ...
        while (f != null) {
          // System.out.println("Field not found in new page : col="+f.poscol+" ;
          // line="+f.posline+" ; value="+f.value) ;
