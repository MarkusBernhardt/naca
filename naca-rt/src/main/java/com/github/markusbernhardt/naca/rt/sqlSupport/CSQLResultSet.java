...
import com.github.markusbernhardt.jlib.sql.LogSQLException;
...
import com.github.markusbernhardt.naca.rt.misc.SemanticContextDef;
...
public class CSQLResultSet extends CJMapObject {
    ...
    if (m_r != null) {
      try {
        ...
        if (e.getErrorCode() == -99999) {
          ...
          BaseProgramLoader.logMail(m_csProgramName + " - JDBC warning",
              "Warning while executing CSQLResultSet::next() on result set for program=" + m_csProgramName + ", clause=" + m_csQuery, e);
          if (m_sqlStatus != null)
            m_sqlStatus.setSQLCode(SQLCode.SQL_NOT_FOUND);
        } else {
    ...
    if (baseRecordColTypeManager != null) {
      ...
      try {
        ...
        if (csColTypeName.equals("CHAR")) {
          ...
          //
          // baseRecordColTypeManager = new RecordColTypeManagerDecimal(nColSourceIndex);
          // recordSetCacheColTypeType.set(nColSourceIndex0Based,
          // baseRecordColTypeManager);
  ...
  private void doFillIntoValues(SQL sql, boolean bCursor, boolean bRowIdGenerated, SQLRecordSetVarFiller sqlRecordSetVarFiller) {
    ...
    // Consume leading and ending unitary columns; a select with * must follow the
    // syntax: Select [col]*, [*]*, [col]* from ...
    // There cannot be unique cols between stars: that is select toto, *, titi, *,
    // tutu is illegal.
    ...
    if (bSkippedStar) {
      ...
      for (int nColRecordSet = nNbcolUnitaryLeft; nColRecordSet < nNbColInRecordSet - nNbcolUnitaryRight; nColRecordSet++) // enum all varing length col form
                                                                                                                           // the record set
      {
        rnChildIndex.set(-1);
        ...
        for (int nColDest = nNbcolUnitaryLeft; nColDest < nNbColsDest - nNbcolUnitaryRight; nColDest++) // Enum all groups
        {
          ...
          if (varChild != null) {
            ...
            if (!bChildAlreadyFilled) {
              ...
              // String csValue = getColValueAsString(nColRecordSet,
              ...
              // {
              // String csSemanticContext =
              // m_semanticContextDef.getSemanticContextValueDefinition(csTableColName);
