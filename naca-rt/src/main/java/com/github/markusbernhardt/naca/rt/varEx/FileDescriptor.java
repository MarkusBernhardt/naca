...
import com.github.markusbernhardt.jlib.misc.BaseDataFileBuffered;
import com.github.markusbernhardt.jlib.misc.EnvironmentVar;
import com.github.markusbernhardt.jlib.misc.LineRead;
import com.github.markusbernhardt.jlib.misc.LittleEndingSignBinaryBufferStorage;
import com.github.markusbernhardt.jlib.misc.LogicalFileDescriptor;
import com.github.markusbernhardt.jlib.misc.RecordLengthDefinition;
...
import com.github.markusbernhardt.naca.rt.batchOOApi.WriteBufferExt;
...
public class FileDescriptor extends BaseFileDescriptor {
  ...
  private void writeFrom(VarBase varFrom, boolean bRewriteMode) {
    ...
    if (varLevel01 != varFrom)
      varLevel01.setFromByteArray(tbyFilebuffer, 0, nMinSize); // Used when record buffer is longer than working buffer; we must keep the right
                                                               // part of the record at the initialized values
  ...
  public RecordDescriptorAtEnd readInto(Var varDest) {
    ...
    if (hasVarVariableLengthMarker()) {
      ...
      if (nRecordLength > 0) {
        ...
        // lineRead =
        // ((DataFileLineReader)m_fileManagerEntry.m_dataFile).readDirect(nRecordLength);
      }
  ...
  // {
  // byte tbyFilebuffer[] =
  // m_fileManagerEntry.m_dataFile.getByteBuffer(nRecordSize);
  // varDest.exportToByteArray(tbyFilebuffer, nRecordSize);
  // m_varLevel01.setFromByteArray(tbyFilebuffer, 0, nRecordSize);
  // }
  // }
  ...
  public LineRead readALine(BaseDataFileBuffered dataFileIn, LineRead lastLineRead) {
    ...
    if (isVariableLength()) {
      ...
      lastLineRead = dataFileIn.readVariableLengthLine(bReadLF, bHeader4Bytes, lastLineRead); // Read a vairable length line (length is given in record header 4
                                                                                              // bytes)
    } else {
  ...
  public void tryAutoDetermineRecordLengthIfRequired(BaseDataFile dataFileIn) {
    // the return value is a flag that indicates if we have a valid file position on
    // output
    if (isVariableLength())
      return; // We are a variable length file: no need to try to autodetermine record length;
              // file position is valid
    if (getRecordLengthDefinition() != null)
      return; // we have the record definition: no need to try to autodetermine record length;
              // file position is valid
