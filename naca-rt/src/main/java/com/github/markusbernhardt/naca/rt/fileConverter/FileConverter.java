 ...
 */
package com.github.markusbernhardt.naca.rt.fileConverter;
...
import com.github.markusbernhardt.jlib.misc.LittleEndingUnsignBinaryBufferStorage;
...
public class FileConverter {
  ...
  // private static final byte[] AFP_EBCDIC_SFI = { (byte)0xD3, (byte)0xEE,
  // (byte)0x9B };
  // private static final byte[] AFP_EBCDIC_BOC = { (byte)0xD3, (byte)0xA8,
  // (byte)0x92 };
  // private static final byte[] AFP_EBCDIC_OCD = { (byte)0xD3, (byte)0xEE,
  // (byte)0x92 };
  // private static final byte[] AFP_EBCDIC_EOC = { (byte)0xD3, (byte)0xA9,
  // (byte)0x92 };
  // private static final byte[] AFP_EBCDIC_IOB = { (byte)0xD3, (byte)0xAF,
  // (byte)0xC3 };
  ...
  public boolean execute(String csParameter) {
    if (csParameter != null && !csParameter.equals("")) {
      ...
      if (csParameterUpper.indexOf("RECORDLENGTH={") != -1) {
        ...
        if (csParameterUpper.indexOf("PADDINGHEX={") != -1) {
          ...
          int nVal = Integer.decode(csDigit)
                            .intValue();
    ...
    else if (m_bSuppressVariableLength)
    ...
    if (m_bList) {
      ...
      if (bConvOpened) {
        ...
        while ((lineRead = m_file.readALine(dataFileList, null)) != null) {
          String csFile = lineRead.getChunkAsString()
                                  .trim();
          convert(csFile);
        }
        dataFileList.close();
        ...
        if (m_bAddVariableLength) {
          ...
          if (logicalFileDescriptor.isVariableLength()) {
            ...
            if (logicalFileDescriptor.getRecordLengthDefinition() != null) {
              int iLength = logicalFileDescriptor.getRecordLengthDefinition()
                                                 .getRecordLength();
  ...
  private void convertNext(DataFileLineReader dataFileIn, LineRead lineRead) {
    ...
    if (m_bConvertInEbcdicAFP || m_bConvertInEbcdicAFPInfoPrint) {
      if (nLengthLine > 6 && arrByteValue[0] == AFP_ASCII_5A) {
        ...
        // }
      } else if (nLengthLine > 4 && arrByteValue[0] == '#' && arrByteValue[1] == '3' && arrByteValue[2] == '0' && arrByteValue[3] == '0'
          && arrByteValue[4] == '#') {
