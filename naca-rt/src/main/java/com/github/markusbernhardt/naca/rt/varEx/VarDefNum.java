...
import com.github.markusbernhardt.naca.rt.sqlSupport.CSQLItemType;
import com.github.markusbernhardt.naca.rt.tempCache.CStr;
import com.github.markusbernhardt.naca.rt.tempCache.CStrNumber;
...
/*
 * http://www.helsinki.fi/atk/unix/dec_manuals/cobv27ua/cobrm_029.htm#
 * index_x_737 MOVES: Editing, De-Editing, and Data Conversion During Elementary
 * Moves
 * 
 * 
 * Editing, de-editing, or other required internal data conversions occur during
 * elementary moves. They are controlled by the description of dest-item. When
 * dest-item is alphanumeric or alphanumeric edited, alignment and space-filling
 * occur according to the Standard Alignment Rules. If lit or src-item is signed
 * numeric, the operational sign is not moved. If the operational sign occupies
 * a separate character position: The sign character is not moved. The size of
 * lit or src-item is considered to be one less than its actual size (in terms
 * of Standard Data Format characters).
 * 
 * If the sending operand is numeric and contains the PICTURE symbol (P), all
 * digit positions specified with this symbol are considered to have the value
 * zero and are counted in the size of the sending operand. When dest-item is
 * numeric or numeric edited, decimal point alignment and zero-filling occur
 * according to the Standard Alignment Rules. When dest-item is a signed numeric
 * item, the sign from lit or src-item is placed in it. If the sending item is
 * unsigned, a positive sign is placed in dest-item. When dest-item is an
 * unsigned numeric item, the absolute value of lit or src-item is moved. When
 * lit or src-item is alphanumeric, the move occurs as if the sending item were
 * described as an unsigned numeric integer. When src-item is numeric edited,
 * the compiler de-edits it before moving it to dest-item. Src-item can be
 * signed. When dest-item is alphabetic, justification and space-filling occur
 * according to the Standard Alignment Rules. Nonelementary Moves
 * 
 * 
 * A nonelementary move occurs as if it were an alphanumeric-to-alphanumeric
 * elementary move. However, there is no internal data conversion. The move is
 * not affected by individual elementary or group items in either src-item or
 * dest-item, except as noted in the General Rules for the OCCURS clause.
 * Summary
 * 
 * 
 * Table 6-13 summarizes the valid types of MOVE statements. References after
 * slash marks show the applicable General Rule. For example, moving a numeric
 * edited item to an alphabetic item is invalid because of General Rule 9b.
 * 
 * Table 6-13 Valid MOVE Statements Category of Receiving Data Item (dest-item)
 * Category of Sending Data Item (lit or src-item) Alphabetic Alphanumeric
 * Edited Alphanumeric Numeric Integer Numeric Noninteger Numeric Edited
 * Alphabetic Yes/13 Yes/11 No/9a Alphanumeric Yes/13 Yes/11 Yes/12 Alphanumeric
 * Edited Yes/13 Yes/11 No/9a Numeric Integer No/9b Yes/11 Yes/12 Numeric
 * Noninteger No/9b No/9c Yes/12 Numeric Edited No/9b Yes/11 Yes/12
 */
...
public abstract class VarDefNum extends VarDefVariable {
  ...
  // Comp0
  // protected int internalWriteDecComp0(VarBufferPos buffer, Dec decValue, int
  // nNbDigitInteger, int nNbDigitDecimal)
  // {
  // int nPosition = RWNumIntComp0.internalWriteAbsoluteIntComp0AsLong(buffer,
  // decValue.getUnsignedLong(), buffer.m_nAbsolutePosition, nNbDigitInteger);
  // String csValueDec = decValue.getDecPart();
  // nPosition = internalWriteRightPadding(buffer, nPosition, nNbDigitDecimal,
  // csValueDec, '0');
  // return nPosition;
  // }
  ...
  // protected int readIntComp0(VarBuffer buffer, int nAbsolutePosition, int
  // nTotalSize)
  // {
  ...
  // protected int readSignIntComp0(VarBuffer buffer, int nAbsolutePosition, int
  // nTotalSize)
  // {
  ...
  protected String internalReadSignedIntComp0AsString(VarBufferPos buffer, int nAbsolutePosition, int nTotalSize) {
    String cs = "";
    if (nTotalSize > 1) {
      cs = buffer.getStringAt(nAbsolutePosition, nTotalSize - 1)
                 .getAsString();
    }
    // int nValue = NumberParser.getAsInt(cs);
    // nValue *= 10;
  ...
  CStr internalReadSignedIntComp0AsUnsignedString(VarBufferPos buffer, int nAbsolutePosition, int nTotalSize) {
    ...
    CStrNumber csNum = TempCacheLocator.getTLSTempCache()
                                       .getCStrNumber();
    csNum.set(cs, 1);
  ...
  // Comp3
  // public static int internalWriteEncodeComp3(VarBufferPos buffer, int nOffset,
  // String s, boolean bPositive, boolean bSigned)
  // {
  ...
  // protected int internalReadIntSignComp3(VarBufferPos buffer, int
  // nNbDigitInteger)
  // {
  // int nValue = Pic9Comp3BufferSupport.getAsInt(buffer, nNbDigitInteger,
  // m_nTotalSize);
  // return nValue;
  ////
  ...
  // protected long internalReadIntSignComp3AsLong(VarBufferPos buffer, int
  // nNbDigits)
  // {
  // long lValue = Pic9Comp3BufferSupport.getAsLong(buffer, nNbDigits,
  // m_nTotalSize);
  // return lValue;
  //// String s = buffer.getStringAt(buffer.m_nAbsolutePosition, m_nTotalSize);
  ...
  // protected long internalReadIntComp3AsLong(VarBufferPos buffer, int
  // nNbDigitInteger)
  // {
  // long lValue = Pic9Comp3BufferSupport.getAsLong(buffer, nNbDigitInteger,
  // m_nTotalSize);
  // return lValue;
  //
  //// String s = buffer.getStringAt(buffer.m_nAbsolutePosition, m_nTotalSize);
  ...
  // protected int writeIntComp3(VarBufferPos buffer, int nValue, int
  // nNbDigitInteger)
  // {
  // if(nValue < 0)
  // nValue = -nValue;
  ...
  // protected void writeIntComp3(VarBufferPos buffer, int nValue, int
  // nNbDigitInteger)
  // {
  // Pic9Comp3BufferSupport.setFromRightToLeft(buffer, nNbDigitInteger,
  // m_nTotalSize, 0, false, nValue);
  // }

  // protected int writeIntComp3(VarBufferPos buffer, int nOffset, int nValue, int
  // nNbDigitInteger)
  // {
  // if(nValue < 0)
  // nValue = -nValue;
  ...
  // protected void writeIntComp3(VarBufferPos buffer, int nOffset, int nValue,
  // int nNbDigitInteger)
  // {
  // Pic9Comp3BufferSupport.setFromRightToLeft(buffer, nNbDigitInteger,
  // m_nTotalSize, nOffset, false, nValue);
  // }

  // protected int writeIntComp3AsLong(VarBufferPos buffer, long lValue, int
  // nNbDigitInteger)
  // {
  // if(lValue < 0)
  // lValue = -lValue;
  ...
  // protected void writeIntComp3AsLong(VarBufferPos buffer, long lValue, int
  // nNbDigitInteger)
  // {
  // Pic9Comp3BufferSupport.setFromRightToLeft(buffer, nNbDigitInteger,
  // m_nTotalSize, 0, false, lValue);
  // }

  // protected int writeIntComp3AsLong(VarBufferPos buffer, int nOffset, long
  // lValue, int nNbDigitInteger)
  // {
  // if(lValue < 0)
  // lValue = -lValue;
  //
  ...
  // protected void writeIntComp3AsLong_TOTO(VarBufferPos buffer, int nOffset,
  // long lValue, int nNbDigitInteger)
  // {
  // Pic9Comp3BufferSupport.setFromRightToLeft(buffer, nNbDigitInteger,
  // m_nTotalSize, nOffset, false, lValue);
  // }

  // protected int writeIntSignComp3(VarBufferPos buffer, int nValue, int
  // nNbDigitInteger)
  // {
  ...
  // protected void writeIntSignComp3(VarBufferPos buffer, int nValue, int
  // nNbDigitInteger)
  // {
  // Pic9Comp3BufferSupport.setFromRightToLeftSigned(buffer, nNbDigitInteger,
  // m_nTotalSize, nValue);
  // }

  // protected void writeIntSignComp3(VarBufferPos buffer, String csValue, int
  // nNbDigitInteger)
  // {
  // Pic9Comp3BufferSupport.setFromRightToLeft(buffer, nNbDigitInteger,
  // m_nTotalSize, 0, true, csValue);
  // }
  //
  // protected int writeIntSignComp3(VarBufferPos buffer, int nOffset, int nValue,
  // int nNbDigitInteger)
  // {
  // boolean bPositive = true;
  // if(nValue < 0)
  ...
  // protected void writeIntSignComp3(VarBufferPos buffer, int nOffset, int
  // nValue, int nNbDigitInteger)
  // {
  // Pic9Comp3BufferSupport.setFromRightToLeft(buffer, nNbDigitInteger,
  // m_nTotalSize, nOffset, true, nValue);
  // }

  // protected int writeIntSignComp3AsLong(VarBufferPos buffer, long lValue, int
  // nNbDigitInteger)
  // {
  // return writeIntSignComp3AsLong(buffer, 0, lValue, nNbDigitInteger);
  // }
  //
  // protected int writeIntSignComp3AsLong(VarBufferPos buffer, int nOffset, long
  // lValue, int nNbDigitInteger)
  // {
  // boolean bPositive = true;
  // if(lValue < 0)
  ...
  int internalReadIntSignComp4WithMaxDigits(VarDefBase varDef, VarBufferPos buffer, int nNbDigitsToKeep) {
    ...
    // lValue = Pic9Comp3BufferSupport.keepRightMostDigits(varDef, lValue,
    // nNbDigitsToKeep);
    return (int) lValue;
  }
