...
import com.github.markusbernhardt.jlib.log.AssertException;
...
import com.github.markusbernhardt.jlib.misc.DataFileReadWrite;
import com.github.markusbernhardt.jlib.misc.DateUtil;
import com.github.markusbernhardt.jlib.misc.EnvironmentVar;
import com.github.markusbernhardt.jlib.misc.FileEndOfLine;
import com.github.markusbernhardt.jlib.misc.FileSystem;
...
import com.github.markusbernhardt.naca.rt.exceptions.DumpProgramException;
import com.github.markusbernhardt.naca.rt.fileConverter.FileSearchGeneration;
import com.github.markusbernhardt.naca.rt.fpacPrgEnv.VarFPacLengthUndef;
...
public class NacaToolBox extends CJMapObject {
  ...
  public void addDate(VarAndEdit outputDate) {
    String inputYear = outputDate.getString()
                                 .substring(0, 2)
                                 .trim();
  ...
  public void addDate(VarAndEdit outputDate, VarAndEdit inputDate) {
    String inputDay = inputDate.getString()
                               .substring(4, 6);
    String inputMonth = inputDate.getString()
                                 .substring(2, 4);
    String inputYear = inputDate.getString()
                                .substring(0, 2);
  ...
  public void addDate(VarAndEdit outputDate, VarAndEdit inputDate, VarAndEdit arg) {
    ...
    if (arg.getString()
           .substring(5, 6)
           .equals(" ")) {
      argDay = arg.getString()
                  .substring(2, 5);
      argMonth = "00";
    } else {
      argDay = arg.getString()
                  .substring(4, 6);
      argMonth = arg.getString()
                    .substring(2, 4);
    }
    String argYear = arg.getString()
                        .substring(0, 2);
    String argOp = arg.getString()
                      .substring(6, 7);

    if (!argOp.trim()
              .equals("")
        && !argOp.equals("+") && !argOp.equals("-")) {
  ...
  public void doDateJob(VarAndEdit vOperation, VarAndEdit vParam1, VarAndEdit vParam2, VarAndEdit vParam3) {
    String operation = vOperation.getString()
                                 .trim();
    String csYear = vParam1.getString()
                           .substring(0, 4);
    String csMonth = vParam1.getString()
                            .substring(4, 6);
    String csDay = vParam1.getString()
                          .substring(6, 8)
                          .trim();
    ...
    int argWeek = 0;
    if (vParam3 != null) {
      argYear = NumberParser.getAsInt(vParam3.getString()
                                             .substring(0, 4));
      if (vParam3.getString()
                 .substring(6, 8)
                 .trim()
                 .length() == 0) {
        argWeek = NumberParser.getAsInt(vParam3.getString()
                                               .substring(4, 6));
      } else if (vParam3.getString()
                        .substring(6, 8)
                        .trim()
                        .length() == 1) {
        argDay = NumberParser.getAsInt(vParam3.getString()
                                              .substring(4, 7));
      } else {
        argMonth = NumberParser.getAsInt(vParam3.getString()
                                                .substring(4, 6));
        argDay = NumberParser.getAsInt(vParam3.getString()
                                              .substring(6, 8));
      }
    ...
    if (operation.equals("P")) {
      ...
      csYear = vParam2.getString()
                      .substring(0, 4);
      csMonth = vParam2.getString()
                       .substring(4, 6);
      csDay = vParam2.getString()
                     .substring(6, 8)
                     .trim();
      int year = NumberParser.getAsInt(csYear);
      if (csDay.length() > 1) {
      ...
      // int diffDaysTotal = elapsedTime(calendar, calendarEnd,
      ...
      if (diffYears > 0) {
        ...
        // int DiffDayOfYear = elapsedTime(calendar, calendarEnd,
        // GregorianCalendar.DATE);
  ...
  private String completeDate(GregorianCalendar gc, boolean bLongInfo) {
    ...
    if (csDate.substring(4)
        ...
        || csDate.substring(4)
                 .equals("1226")
        || csDayEaster.equals(csDayOfYear) || diffEasterDays == 1 || // lundi de P�ques
  ...
  public void checkShortNumber(Var returnCode, VarAndEdit input, int length, Var output, Var posBeforeDecimal, Var posAfterDecimal) {
    checkNumber(returnCode, input.getString()
                                 .substring(0, length)
                                 .trim(),
        output, posBeforeDecimal, posAfterDecimal, true);
  }
  ...
  public void checkLongNumber(Var returnCode, VarAndEdit input, int length, Var output, Var posBeforeDecimal, Var posAfterDecimal) {
    checkNumber(returnCode, input.getString()
                                 .substring(0, length)
                                 .trim(),
        output, posBeforeDecimal, posAfterDecimal, false);
  }
  ...
  /*
   * Var dfhcommarea = declare.level(1).var() ; // (41) 01 DFHCOMMAREA. Var net =
   * declare.level(5).picX(8).var() ; // (42) 05 NET PIC X(08). Var term =
   * declare.level(5).picX(4).var() ; // (43) 05 TERM PIC X(04). Var netlu62 =
   * declare.level(5).picX(8).var() ; // (44) 05 NETLU62 PIC X(08). Var termlu62 =
   * declare.level(5).picX(4).var() ; // (45) 05 TERMLU62 PIC X(04).
   */
  public void getTerminal(BaseSession baseSession, Var param) {
    ...
    param.set(
        StringUtil.rightPad(net, 8, ' ') + StringUtil.rightPad(term, 4, ' ') + StringUtil.rightPad(netLu62, 8, ' ') + StringUtil.rightPad(termLu62, 4, ' '));
  }
  ...
  /*
   * Var p2_Loce = declare.level(1).var() ; // (46) 01 P2-LOCE. Var p2_Adrpayn =
   * declare.level(5).picX(3).var() ; // (47) 05 P2-ADRPAYN PIC X(3). Var
   * p2_Adrnpe = declare.level(5).picX(8).var() ; // (48) 05 P2-ADRNPE PIC X(8).
   * Var p2_Adrloce = declare.level(5).picX(20).var() ; // (49) 05 P2-ADRLOCE PIC
   * X(20). Var p2_Adrpays = declare.level(5).picX(3).var() ; // (50) 05
   * P2-ADRPAYS PIC X(3). Var p2_Adrpayl = declare.level(5).picX(20).var() ; //
   * (51) 05 P2-ADRPAYL PIC X(20). Var filler$0 =
   * declare.level(5).picX(6).valueSpaces().var() ; // (52) 05 FILLER PIC X(6)
   * VALUE SPACE. Var filler$1 = declare.level(1).redefines(p2_Loce).var() ; //
   * (53) 01 FILLER REDEFINES P2-LOCE. Var p2_Ligloce1 = declare.level(5).var() ;
   * // (54) 05 P2-LIGLOCE1. Var p2_Cdretn = declare.level(10).picX(1).var() ; //
   * (55) 10 P2-CDRETN PIC X. Var filler$2 = declare.level(10).picX(29).var() ; //
   * (56) 10 FILLER PIC X(29). Var p2_Ligloce2 = declare.level(5).picX(30).var() ;
   * // (57) 05 P2-LIGLOCE2 PIC X(30).
   */
  public void formatForeignAddress(Var param) {
    ...
    if (formatting.equals("1")) {
      line1 = adrnpe.getString()
                    .trim()
          + " " + adrloce.getString()
                         .trim();
    } else if (formatting.equals("2")) {
      line1 = adrpays.getString()
                     .trim()
          + adrnpe.getString()
                  .trim()
          + " " + adrloce.getString()
                         .trim();
      line2 = adrpayl.getString()
                     .trim();
  ...
  /*
   * 
   * Var vc01pa24 = declare.level(1).var() ; // (238) 01 VC01PA24. Var dicjob =
   * declare.level(5).picX(8).var() ; // (239) 05 DICJOB PIC X(8). Var filler$48 =
   * declare.level(5).redefines(dicjob).filler() ; // (240) 05 FILLER REDEFINES
   * DICJOB. Var cdret = declare.level(10).picX(1).var() ; // (241) 10 CDRET PIC
   * X. Var filler$49 = declare.level(10).picX(7).filler() ; // (242) 10 FILLER
   * PIC X(7). Var diccomp = declare.level(5).picX(8).var() ; // (243) 05 DICCOMP
   * PIC X(8). Var utient = declare.level(5).var() ; // (244) 05 UTIENT. Var
   * utiste = declare.level(10).picX(3).var() ; // (245) 10 UTISTE PIC X(3). Var
   * uticpr = declare.level(10).picX(3).var() ; // (246) 10 UTICPR PIC X(3). Var
   * dicproc = declare.level(5).picX(8).var() ; // (247) 05 DICPROC PIC X(8). Var
   * trtref = declare.level(5).picX(3).var() ; // (248) 05 TRTREF PIC X(3). Var
   * filler$50 = declare.level(5).occurs(2).filler() ; // (249) 05 FILLER OCCURS
   * 2. Var orsparam = declare.level(10).picX(79).var() ; // (250) 10 ORSPARAM PIC
   * X(79). Var suite = declare.level(10).picX(1).var() ; // (251) 10 SUITE PIC X.
   */
  public void startBatch(Var param) {
    Var returnCode = param.getVarChildAt(1);
    String job = param.getString()
                      .substring(0, 8)
                      .trim();
    String account = param.getString()
                          .substring(8, 16)
                          .trim();
    String entity = param.getString()
                         .substring(16, 22)
                         .trim();
    String procedure = param.getString()
                            .substring(22, 30)
                            .trim();
    String reference = param.getString()
                            .substring(30, 33)
                            .trim();
    String card = param.getString()
                       .substring(33);

    int cardCount = (card.trim()
                         .length()
        / 80) + 1;
    ...
    returnCode.set("0");
    try {
      String prefix = m_ProgramManager.getEnv()
                                      .getConfigOption("StartBatchPrefix");
      String jobId = OnlineEnvironment.getNextJobBatchID();

      File temp = File.createTempFile(m_ProgramManager.getEnv()
                                                      .getTerminalID()
          + new DateUtil("HHmmssSSS").toString(), null);
      if (m_ProgramManager.getEnv()
                          .getConfigOption("StartBatchHostFtpUrl")
                          .equals("")) {
        startBatchLinuxPrepareFtp(temp, cards);
        String local = m_ProgramManager.getEnv()
                                       .getConfigOption("StartBatchLinuxLocal");
        boolean isLocal = Boolean.parseBoolean(local);
        String ftpUrl = m_ProgramManager.getEnv()
                                        .getConfigOption("StartBatchLinuxFtpUrl");
        String date = new DateUtil("yyyyMMdd").toString();
        String time = new DateUtil("HHmmssSSS").toString()
                                               .substring(0, 7)
            + jobId.substring(2);
        String filename = "temp." + procedure + "." + date + "." + time;
        int rc = 0;
        if (rc == 0) {
          String sshPath = m_ProgramManager.getEnv()
                                           .getConfigOption("StartBatchLinuxSshPath");
          String sshUser = m_ProgramManager.getEnv()
                                           .getConfigOption("StartBatchLinuxSshUser");
          String sshCommand = m_ProgramManager.getEnv()
                                              .getConfigOption("StartBatchLinuxSshCommand");
          String jobclass = startBatchLinuxGetJobclass(cards);
          String jobname = job.substring(0, 4) + prefix + jobId.substring(1);
          rc = startBatchLinuxSsh(isLocal, sshPath, sshUser, ftpUrl, sshCommand, procedure, account, entity, reference, filename, date, time, jobclass,
              jobname);
        }
        returnCode.set(rc);
      }
  ...
  public int startBatchLinuxSsh(boolean isLocal, String sshPath, String sshUser, String url, String sshCommand, String procedure, String account, String entity,
      String reference, String fileName, String date, String time, String jobclass, String jobname) {
    int rc = 0;

    try {
      String[] commands;
      if (isLocal) {
        commands = new String[] { sshCommand, procedure, account, entity, reference, fileName, date, time, jobclass, jobname };
      } else {
        commands = new String[] { sshPath + "ssh", sshUser + "@" + url, sshCommand, procedure, account, entity, reference, fileName, date, time, jobclass,
            jobname };
      }
      Runtime.getRuntime()
             .exec(commands);
  ...
  private String startBatchLinuxGetJobclass(String[] cards) {
    ...
    if (nPos == -1) {
      String test = m_ProgramManager.getEnv()
                                    .getConfigOption("StartBatchHostTest");
  ...
  private void startBatchHostPrepareFtp(File temp, String job, String account, String entity, String procedure, String reference, String[] cards, String prefix,
    ...
    out.newLine();
    out.write("// " + m_ProgramManager.getEnv()
                                      .getSQLConnection()
                                      .getEnvironmentPrefix()
        + jobClass + msgClass + jobCard);
    ...
    if (cardOutput > 0) {
      for (int i = 0; i < cardOutput; i++) {
        ...
        out.write(cards[cardItem].substring(6)
                                 .trim());
        cardItem++;
      }
    }
  ...
  public void formatSQLCode(Var varErrorMessage, Var varErrorTextLen) {
    int nSizeBufferError = varErrorMessage.getVarChildAt(1)
                                          .getInt();
    int nSizeLine = varErrorTextLen.getInt();

    CSQLStatus sqlStatus = m_ProgramManager.getSQLStatus();
    if (sqlStatus != null) {
      varErrorMessage.getVarChildAt(2)
                     .set(sqlStatus.toString());
    }
  }
  ...
  public void getJobInfo(Var jobInfo) {
    ...
    jobInfo.set(StringUtil.rightPad(jobId, 8, ' ') + StringUtil.rightPad(stepId, 8, ' ') + StringUtil.rightPad(procId, 8, ' '));
  }
  ...
  public String getJobInfoForKey(String key) {
    ...
    if (value.equals("") && m_ProgramManager.getEnv()
                                            .getBaseSession() != null)
      value = m_ProgramManager.getEnv()
                              .getBaseSession()
                              .getLogicalJobInfo(key);
    return value;
  }

  public int getCardKey(Var card, Var key, Var value, Var index) {
    String csCard = card.getString()
                        .trim();
    if (csCard.length() > 1 && csCard.charAt(csCard.length() - 1) == '*')
      csCard = csCard.substring(0, csCard.length() - 1)
                     .trim();
    ...
    if (parms.length > pos + 1) {
      ...
      if (posEqual == -1) {
        ...
        keyFound = parms[pos].substring(0, posEqual)
                             .trim();
        valueFound = parms[pos].substring(posEqual + 1)
                               .trim();
        if (valueFound.length() > 1 && valueFound.charAt(0) == '\'' && valueFound.charAt(valueFound.length() - 1) == '\'') {
          valueFound = valueFound.substring(1, valueFound.length() - 1);
        }
        if (keyFound.length() > key.getVarChildAt(2)
                                   .getLength())
          return 4;
        if (valueFound.length() > value.getVarChildAt(2)
                                       .getLength())
          return 12;
        key.getVarChildAt(2)
           .set(keyFound);
        value.getVarChildAt(2)
             .set(valueFound);
      }
      if (parms.length > pos + 1)
        index.set(pos);
  ...
  public void dynamicAllocation(Var[] params) {
    ...
    if (operation == 1 || operation == 11) {
      for (int i = 1; i < params.length; i++) {
        String key = params[i].getString()
                              .substring(0, 8)
                              .trim();
        String value;
        if (key.equals("PRTY") || key.equals("COPIES")) {
          value = params[i].getVarChildAt(2)
        ...
        m_ProgramManager.getEnv()
                        .getBaseSession()
                        .addDynamicAllocationInfo(key, value);
      }
    } else {
    ...
    if (operation == 1) {
      ...
      String ddname = m_ProgramManager.getEnv()
                                      .getBaseSession()
                                      .getDynamicAllocationInfo("DDNAME");
      String dsn = m_ProgramManager.getEnv()
                                   .getBaseSession()
                                   .getDynamicAllocationInfo("DSN");
      ...
      if (dsn == null) {
        ...
        dsn = dynamicAllocationPath + "temp/linux." + getJobInfoForKey("JOBID") + "." + getJobInfoForKey("STEPID") + ".I" + csDate + ".H" + csHour
            + m_ProgramManager.getEnv()
                              .getBaseSession()
                              .getNextDynamicAllocationID();
        dsn = dsn.toLowerCase();
      } else {
   ...
   * TextPrintHelper textPrintHelper = formatPSFTextGetInstance(); ReturnCode rc =
   * textPrintHelper.defineFonts(FontSets.fontset1); returnCode.set(rc.getCode());
   * }
   */
  public void formatPSFLoadColumn(Var nbColumn, Var rule, Var interline, Var returnCode) {
    ...
    // Var s31_Colonne = declare.level(10).occurs(4).var() ; // (1444) 10
    // S31-COLONNE OCCURS 4.
    // Var s31_Pos = declare.level(15).picS9(9).comp().var() ; // (1445) 15 S31-POS
    // PIC S9(9) COMP.
    // Var s31_Large = declare.level(15).picS9(9).comp().var() ; // (1446) 15
    // S31-LARGE PIC S9(9) COMP.
    // Var s31_Flags = declare.level(15).picS9(9).comp().var() ; // (1447) 15
    // S31-FLAGS PIC S9(9) COMP.
    // Var s31_Aligne = declare.level(15).picS9(9).comp().var() ; // (1448) 15
    // S31-ALIGNE PIC S9(9) COMP.
    // Var s31_Lmode = declare.level(15).picS9(9).comp().var() ; // (1449) 15
    // S31-LMODE PIC S9(9) COMP.
    // Var s31_Yline = declare.level(15).picS9(9).comp().var() ; // (1450) 15
    // S31-YLINE PIC S9(9) COMP.
    // Var s31_Cyline = declare.level(15).picS9(9).comp().var() ; // (1451) 15
    // S31-CYLINE PIC S9(9) COMP.
    // Var s31_Trc = declare.level(15).picX(1).var() ; // (1452) 15 S31-TRC PIC X.
    // Var filler$273 = declare.level(15).picX(3).filler() ; // (1453) 15 FILLER PIC
    // XXX.
    /*
     * Var varColonne = rule.getVarChildAt(1); Var varPos =
     * varColonne.getVarChildAt(1); Var varLarge = varColonne.getVarChildAt(2); Var
     * varFlags = varColonne.getVarChildAt(3); Var varAligne =
     * varColonne.getVarChildAt(4); Var varLmode = varColonne.getVarChildAt(5); Var
     * varYline = varColonne.getVarChildAt(6); Var varCyline =
     * varColonne.getVarChildAt(7); Var varTrc = varColonne.getVarChildAt(8);
     * 
     * ColumnDef[] cols = new ColumnDef[nbColumn.getInt()]; for (int i=0, j=1; i <
     * nbColumn.getInt(); i++, j++) { int nPos = varPos.getAt(j).getInt(); int
     * nLarge = varLarge.getAt(j).getInt(); int nFlags = varFlags.getAt(j).getInt();
     * int nAligne = varAligne.getAt(j).getInt(); int nLmode =
     * varLmode.getAt(j).getInt(); int nYline = varYline.getAt(j).getInt(); int
     * nCyline = varCyline.getAt(j).getInt(); String csTrc =
     * varTrc.getAt(j).getString(); cols[i] = new ColumnDef(nPos, nLarge, flags,
     * quad, nLmode, nYline, nCyline, cfid); } FormatDef format = new
     * FormatDef(cols, interline.getInt());
     */
    /*
     * FormatDef format = null; int nRule =
     * rule.getVarChildAt(1).getVarChildAt(9).getAt(1).getInt(); switch (nRule) {
     * case 1: format = FormatsSet1.format1; break; case 2: format =
     * FormatsSet1.format2; break; case 3: format = FormatsSet1.format3; break; case
     * 4: format = FormatsSet1.format4; break; case 5: format = FormatsSet1.format5;
     * break; case 6: format = FormatsSet1.format6; break; case 7: format =
     * FormatsSet1.format7; break; case 8: format = FormatsSet1.format8; break; case
     * 9: format = FormatsSet1.format9; break; case 10: format =
     * FormatsSet1.format10; break; case 11: format = FormatsSet1.format11; break;
     * case 12: format = FormatsSet1.format12; break; case 13: format =
     * FormatsSet1.format13; break; case 14: format = FormatsSet1.format14; break;
     * case 15: format = FormatsSet1.format15; break; case 16: format =
     * FormatsSet1.format16; break; default:
     * Assert("formatPSFLoadColumn doesn't implement the rule " + rule); break; }
  ...
  public void formatPSFText(Var input, Var blockMode, Var output, Var blockHeight, Var returnCode) {
    ...
    // Var s32_Inbch = declare.level(10).picS9(9).comp().var() ; // (2498) 10
    // S32-INBCH PIC S9(9) COMP.
    // Var s32_Imaxch = declare.level(10).picS9(9).comp().value(4001).var() ; //
    // (2499) 10 S32-IMAXCH PIC S9(9) COMP VALUE 4001.
    // Var s32_Intxt = declare.level(10).var() ; // (2500) 10 S32-INTXT.
    // Var s32_Ichar = declare.level(15).occurs(4001).picX(1).var() ; // (2501) 15
    // S32-ICHAR OCCURS 4001 PIC X.

    /*
     * ReturnCode rc; PSFOutputBuffer psf = new PSFOutputBuffer(4001); if
     * (blockMode.getInt() == 1) rc =
     * textPrintHelper.printText(input.getVarChildAt(3).getString().substring(0,
     * input.getVarChildAt(1).getInt()), TextBlockMode.BLKM_MORE, psf); else rc =
     * textPrintHelper.printText(input.getVarChildAt(3).getString().substring(0,
     * input.getVarChildAt(1).getInt()), TextBlockMode.BLKM_ENDOFBLOCK, psf);
     * 
     * // Var s32_Output = declare.level(5).var() ; // (2493) 05 S32-OUTPUT. // Var
     * s32_Onbch = declare.level(10).picS9(9).comp().var() ; // (2494) 10 S32-ONBCH
     * PIC S9(9) COMP. // Var s32_Omaxch
     * =declare.level(10).picS9(9).comp().value(4001).var();// (2495) 10 S32-OMAXCH
     * PIC S9(9) COMP VALUE 4001. // Var s32_Outxt = declare.level(10).var() ; //
     * (2496) 10 S32-OUTXT. // Var s32_Ochar =
     * declare.level(15).occurs(4001).picX(1).var() ; // (2497) 15 S32-OCHAR OCCURS
     * 4001 PIC X. output.getVarChildAt(1).set(psf.getLength() - 1);
     * output.getVarChildAt(3).importFromByteArray(psf.getBytes(), psf.getLength());
     * blockHeight.set(psf.getHeight()); returnCode.set(rc.getCode());
     */
  }
  ...
  public void formatPSFLine(Var input, Var blockMode, Var output, Var blockHeight, Var returnCode) {
    ...
    // Var s33_Pos = declare.level(10).picS9(9).comp().var() ; // (2509) 10 S33-POS
    // PIC S9(9) COMP.
    // Var s33_Large = declare.level(10).picS9(9).comp().var() ; // (2510) 10
    // S33-LARGE PIC S9(9) COMP.
    // Var s33_Yline = declare.level(10).picS9(9).comp().var() ; // (2511) 10
    // S33-YLINE PIC S9(9) COMP.
    // Var s33_Cyline = declare.level(10).picS9(9).comp().var() ; // (2512) 10
    // S33-CYLINE PIC S9(9) COMP.
    /*
     * LineDef lineDef = new LineDef(input.getVarChildAt(1).getInt(),
     * input.getVarChildAt(2).getInt(), input.getVarChildAt(3).getInt(),
     * input.getVarChildAt(4).getInt());
     * 
     * ReturnCode rc; PSFOutputBuffer psf = new PSFOutputBuffer(4001); if
     * (blockMode.getInt() == 1) rc = textPrintHelper.drawLine(lineDef,
     * TextBlockMode.BLKM_MORE, psf); else rc = textPrintHelper.drawLine(lineDef,
     * TextBlockMode.BLKM_ENDOFBLOCK, psf);
     * 
     * // Var s32_Output = declare.level(5).var() ; // (2493) 05 S32-OUTPUT. // Var
     * s32_Onbch = declare.level(10).picS9(9).comp().var() ; // (2494) 10 S32-ONBCH
     * PIC S9(9) COMP. // Var s32_Omaxch
  ...
  /*
   * private TextPrintHelper formatPSFTextGetInstance() { TextPrintHelper
   * textPrintHelper =
   * (TextPrintHelper)m_ProgramManager.getEnv().getBaseSession().getSpecialObject(
   * "TextPrintHelper"); if (textPrintHelper == null) { textPrintHelper = new
   * TextPrintHelper();
   * m_ProgramManager.getEnv().getBaseSession().addSpecialObject(
   * "TextPrintHelper", textPrintHelper); } return textPrintHelper; }
   */
  public void formatPSFOrderText(Var input, Var output) {
    /*
     * ImagePrintHelper imagePrintHelper =
     * (ImagePrintHelper)m_ProgramManager.getEnv().getBaseSession().getSpecialObject
     * ("ImagePrintHelper"); if (imagePrintHelper == null) { imagePrintHelper = new
     * ImagePrintHelper();
     * m_ProgramManager.getEnv().getBaseSession().addSpecialObject(
     * "ImagePrintHelper", imagePrintHelper); }
  ...
  public void formatXMLOrder(Var input, Var output) {
    /*
     * XmlPrintHelper xmlPrintHelper =
     * (XmlPrintHelper)m_ProgramManager.getEnv().getBaseSession().getSpecialObject(
     * "XmlPrintHelper"); if (xmlPrintHelper == null) { xmlPrintHelper = new
     * XmlPrintHelper();
     * m_ProgramManager.getEnv().getBaseSession().addSpecialObject("XmlPrintHelper",
     * xmlPrintHelper); } xmlPrintHelper.setProgramManager(m_ProgramManager);
     * xmlPrintHelper.print(input, output);
     */
  }

  public void restartFile(Var outputFile) {
    String csLogicalFileName = outputFile.getVarChildAt(1)
                                         .getString()
                                         .trim();
    int nNbRecordsToKeep = outputFile.getVarChildAt(2)
                                     .getInt();
    restartFile(csLogicalFileName, nNbRecordsToKeep);
  }

  public void restartFile(String csLogicalFileName, int nNbRecordsToKeep) {
    FileDescriptor file = new FileDescriptor(csLogicalFileName, m_ProgramManager.getEnv()
                                                                                .getBaseSession());
