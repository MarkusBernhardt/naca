...
import com.github.markusbernhardt.jlib.misc.IntegerRef;
import com.github.markusbernhardt.naca.rt.bdb.BtreeSegmentKeyTypeFactory;
...
public class VarDefG extends VarDefVariable {
  ...
  void write(VarBufferPos buffer, VarDefG varSource, VarBufferPos bufferSource) {
    ...
    // If the sending item is a group item, and the receiving item is an elementary
    // item, the compiler ignores the receiving item description except for the size
    // description, in bytes, and any JUSTIFIED clause. It conducts no conversion or
    // editing on the sending item's data
    internalPhysicalWriteToGroup(buffer, varSource, bufferSource);
  }

  void write(VarBufferPos buffer, VarDefX varSource, VarBufferPos bufferSource) {
    // http://h71000.www7.hp.com/DOC/73final/6297/6297_profile_010.html#alpha_elem_move_sec
    // If either the sending or receiving item is a group item, the compiler
    // considers the move to be a group move. It treats both the sending and
    // receiving items as if they were alphanumeric items.
    internalPhysicalWriteToGroup(buffer, varSource, bufferSource);
  }
  ...
  int compare(ComparisonMode mode, VarBufferPos buffer2, VarDefNumDecComp0 varDefNum1, VarBufferPos buffer1) {
    ...
    // operand and a nonnumeric operand. If you try to compare these two items, you
    // receive a diagnostic message at compile time.
    // more relax here:
  ...
  int compare(ComparisonMode mode, VarBufferPos buffer2, VarDefNumIntComp4 varDefNum1, VarBufferPos buffer1) {
    // see
    // http://h71000.www7.hp.com/DOC/73final/6297/6297_profile_010.html#group_items_sec
    ...
    // and, therefore, includes in its length any operational sign, whether carried
    // as a separate character or as an overpunched character. Overpunched
    // characters are never ASCII numeric digits. They are characters ranging from A
    // to R, left brace ({), or right brace (}). Thus, the sign and the digits,
    // stored as ASCII bytes, take part in the comparison, and zeros are not
    // supplied for P characters in the PICTURE character-string.
  ...
  int compare(ComparisonMode mode, VarBufferPos buffer2, VarDefNumIntSignComp0Long varDefNum1, VarBufferPos buffer1) {
    ...
    // treats the numeric operand as if it had been moved into an alphanumeric data
    // item the same size as the numeric operand and then compared. This causes any
    // operational sign, whether carried as a separate character or as an
    // overpunched character, to be stripped from the numeric item so that it
    // appears to be an unsigned quantity.
    // In addition, if the PICTURE character-string of the numeric item contains
    // trailing P characters, indicating that there are assumed integer positions
    // that are not actually present, they are filled with zero digits. Thus, an
    // item with a PICTURE character-string of S9999PPP is moved to a temporary
    // location where it is described as 9999999. If its value is 432J (--4321), the
    // value in the temporary location will be 4321000. The numeric digits take part
    // in the comparison.
    // String cs1 = varDefNum1.getRawStringExcludingHeader(buffer1);
    CStr cs1 = buffer1.getBodyCStr(varDefNum1);
  ...
  // private void getComparisonBufferItemAndChildren(TempCache tempCache,
  // ComparisonMode mode, CStr csToFill, IntegerRef riAbsolutePosition,
  ...
  //
  // if(nNbDimRemaining < 0) // Indexed item initializied (x.getAt(n)); the offset
  // is alreday managed
  // {
  // fillComparisionBuffer(tempCache, mode, csToFill, riAbsolutePosition, varDef,
  // buffer);
  // }
  ...
  // if(varDefItem != null)
  // fillComparisionBuffer(tempCache, mode, csToFill, riAbsolutePosition,
  // varDefItem, buffer);
  ...
  // if(varDefChild != null)
  // if(!varDefChild.isARedefine() && !getFiller()) // PJD: Added because
  // redefines and filler must not be taken in account for
  // varDefChild.getComparisonBufferItemAndChildren(tempCache, mode, csToFill,
  // riAbsolutePosition, varDefChild, buffer);
  // }
  // }
  // }

  // private void fillComparisionBuffer(TempCache tempCache, ComparisonMode mode,
  // CStr csToFill, IntegerRef riAbsolutePosition, VarDefBuffer varDef,
  // VarBufferPos buffer)
  // {
  // CStr cs = buffer.getBodyCStrAtAbsolutePosition(mode, riAbsolutePosition,
  // varDef);
  ...
  private void fillChildrenBuffer(TempCache tempCache, ComparisonMode mode, CStr csToFill, IntegerRef riAbsolutePosition, VarDefBuffer varDef,
      VarBufferPos buffer) {
  ...
  int compare(ComparisonMode mode, VarBufferPos buffer2, VarDefG varDef1, VarBufferPos buffer1) {
    ...
    if (mode == ComparisonMode.Ebcdic) {
      ...
      // CStr cs = buffer1.getBodyCStrAtAbsolutePosition(mode, riAbsolutePosition,
      // varDefChild);
      ...
      // CStr cs = buffer2.getBodyCStrAtAbsolutePosition(mode, riAbsolutePosition,
      // varDefChild);
  ...
  private void internalPhysicalWriteToGroup(VarBufferPos bufferDest, VarDefBuffer varSource, VarBufferPos bufferSource) {
    ...
    int nPositionDest = bufferDest.copyBytesFromSource(getBodyAbsolutePosition(bufferDest), bufferSource, varSource.getBodyAbsolutePosition(bufferSource),
        nNbCharToCopy);
