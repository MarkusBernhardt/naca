...
public class TransThreadMBean extends BaseCloseMBean {
  ...
  public String getB_EnvironmentCreationTime() {
    if (m_env != null)
      return m_env.getCreationDateInfo()
  ...
  public int getLastTransactionExecTime_s() {
    if (m_env != null) {
      if (!m_env.isRunning())
        return (int) m_env.getStartRunTime()
                          .getTimeOffset_ms(m_env.getEndRunTime())
            / 1000;
      return (int) m_env.getStartRunTime()
                        .getTimeOffsetFromNow_ms()
          / 1000;
    }
    return 0;
  }
