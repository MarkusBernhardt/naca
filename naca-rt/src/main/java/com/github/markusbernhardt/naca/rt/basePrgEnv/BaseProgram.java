...
import com.github.markusbernhardt.jlib.misc.ConsoleInput;
import com.github.markusbernhardt.jlib.misc.JVMReturnCodeManager;
import com.github.markusbernhardt.jlib.misc.NumberParser;
import com.github.markusbernhardt.naca.rt.base.CJMapObject;
import com.github.markusbernhardt.naca.rt.base.JmxGeneralStat;
import com.github.markusbernhardt.naca.rt.exceptions.CExitException;
import com.github.markusbernhardt.naca.rt.exceptions.CGotoException;
import com.github.markusbernhardt.naca.rt.exceptions.CStopRunException;
import com.github.markusbernhardt.naca.rt.mapSupport.Map;
import com.github.markusbernhardt.naca.rt.mathSupport.MathAdd;
import com.github.markusbernhardt.naca.rt.mathSupport.MathBase;
import com.github.markusbernhardt.naca.rt.mathSupport.MathDivide;
import com.github.markusbernhardt.naca.rt.mathSupport.MathMultiply;
import com.github.markusbernhardt.naca.rt.mathSupport.MathPow;
import com.github.markusbernhardt.naca.rt.mathSupport.MathSubtract;
import com.github.markusbernhardt.naca.rt.misc.KeyPressed;
import com.github.markusbernhardt.naca.rt.misc.LogDisplay;
import com.github.markusbernhardt.naca.rt.misc.NacaToolBox;
import com.github.markusbernhardt.naca.rt.misc.Pointer;
import com.github.markusbernhardt.naca.rt.misc.StringAsciiEbcdicUtil;
import com.github.markusbernhardt.naca.rt.program.CCallProgram;
import com.github.markusbernhardt.naca.rt.program.CJMapRunnable;
import com.github.markusbernhardt.naca.rt.program.CopyReplacing;
...
import com.github.markusbernhardt.naca.rt.programPool.SharedProgramInstanceDataCatalog;
import com.github.markusbernhardt.naca.rt.sqlSupport.CSQLStatus;
import com.github.markusbernhardt.naca.rt.sqlSupport.SQL;
import com.github.markusbernhardt.naca.rt.sqlSupport.SQLCall;
import com.github.markusbernhardt.naca.rt.sqlSupport.SQLCursor;
import com.github.markusbernhardt.naca.rt.sqlSupport.SQLCursorFetch;
import com.github.markusbernhardt.naca.rt.sqlSupport.SQLCursorOperation;
import com.github.markusbernhardt.naca.rt.stringSupport.Concat;
import com.github.markusbernhardt.naca.rt.stringSupport.InspectConvert;
import com.github.markusbernhardt.naca.rt.stringSupport.InspectReplacing;
import com.github.markusbernhardt.naca.rt.stringSupport.InspectTallying;
import com.github.markusbernhardt.naca.rt.stringSupport.Unstring;
...
import com.github.markusbernhardt.naca.rt.varEx.CobolConstantHighValue;
import com.github.markusbernhardt.naca.rt.varEx.CobolConstantLowValue;
import com.github.markusbernhardt.naca.rt.varEx.CobolConstantSpace;
import com.github.markusbernhardt.naca.rt.varEx.CobolConstantZero;
import com.github.markusbernhardt.naca.rt.varEx.ComparisonMode;
import com.github.markusbernhardt.naca.rt.varEx.Cond;
import com.github.markusbernhardt.naca.rt.varEx.Console;
import com.github.markusbernhardt.naca.rt.varEx.DataSection;
import com.github.markusbernhardt.naca.rt.varEx.Edit;
import com.github.markusbernhardt.naca.rt.varEx.FileDescriptor;
import com.github.markusbernhardt.naca.rt.varEx.Form;
import com.github.markusbernhardt.naca.rt.varEx.InitializeCache;
import com.github.markusbernhardt.naca.rt.varEx.InternalCharBuffer;
import com.github.markusbernhardt.naca.rt.varEx.MapRedefine;
import com.github.markusbernhardt.naca.rt.varEx.MoveCorrespondingEntryManager;
import com.github.markusbernhardt.naca.rt.varEx.Output;
import com.github.markusbernhardt.naca.rt.varEx.RecordDescriptorAtEnd;
import com.github.markusbernhardt.naca.rt.varEx.SortCommand;
import com.github.markusbernhardt.naca.rt.varEx.SortDescriptor;
import com.github.markusbernhardt.naca.rt.varEx.SortParagHandler;
import com.github.markusbernhardt.naca.rt.varEx.Var;
import com.github.markusbernhardt.naca.rt.varEx.VarAndEdit;
import com.github.markusbernhardt.naca.rt.varEx.VarBase;
import com.github.markusbernhardt.naca.rt.varEx.VarEnumerator;
import com.github.markusbernhardt.naca.rt.varEx.VarTypeId;
...
public abstract class BaseProgram extends CJMapObject {
  ...
  /**
   * Method: Constructor Main entry point of a program
   * 
   * @param:
   * @return:
   */
   ...
   * 
   * @param:
   * @return: Internal Prorgram manager Objet; A program has 2 sides: - a public
   *          one, that is accessible to applications programs - an internal one
   *          that i sused only internally by the library. This methods gives
   *          access to this internal manager <b>This method is not intended to be
   *          used by an application</b>
   */
  /**
   * @return
   */
  ...
  // protected VarSectionDeclaration declare = null; // Declare Object used for
  // data section and variable declaration inside an application

  // http://www.nwrdc.fsu.edu/contents_support_cics_progguide_general.html#dfhcommarea
  // CICS LINK: COMMAREA is passed by ref: The linked prog can return values by
  // the Commearea
  ...
  // Program code
  /**
   * Method: procedureDivision Virtual method that ca be derived to describe
   * formally an entry point for am application program, that is not at in the
   * first paragraph Commment <b>It is not mandatory to derived in an app, but it
   * makes life easier</b>
   * 
   * @param:
   * @return:
  ...
  /**
   * Method: Coin call Declare a sub-program to call; Used internally
   * 
   * @param: IN
   *           Class classPrgToCall: Gives the java class of the sub program to
   *           call. It must itself be derived form Program.
   * @return: CCallProgram: A internal object, that is used to define a program to
   *          be called later
   * @see call(String ) <b>Not intended to be used by applications directly, but
   *      they should use call(String) instead</b>
   */
  ...
  protected CCallProgram call(VarAndEdit var) {
    return call(var.getString()
                   .trim());
  }

  /**
   * Method: call Declare a sub-program to call
   * 
   * @param: IN
   *           String csPrgClassName: Program name to call
   * @return: CCallProgram: A internal object, that is used to define a program to
   *          be called later
   */
  ...
  /**
   * Method: isEqual return true if the 2 variables' value are equal. * @param: IN
   * Var var1: 1st variable to compare
   * 
   * @param: IN
   *           Var var2: 2nd Variable to compare
   * @return: true if var1's value == var2's value, false otherwise. The variables
   *          can by of different type. A conversion can be done in order to have
   *          comparable values. The comparison is done in Unicode.
   */
  ...
  /**
   * Method: isEqual return true if the 2 variables' value are equal. * @param: IN
   * int a: 1st variable to compare
   * 
   * @param: IN
   *           int b: 2nd Variable to compare
  ...
  /**
   * Method: isEqual return true if the 2 variables' value are equal. * @param: IN
   * String a: 1st variable to compare
   * 
   * @param: IN
   *           int b: 2nd Variable to compare
  ...
  /**
   * Method: isEqual Returns true if the 2 integers in param are equals
   * 
   * @param: IN
   *           MathBase a: Contains the result of a previous math operations; it
   *           will be evaluated as an integer (can be rounded)
   * @param: int
   *           b: int to compare
   ...
   *           will be evaluated as an integer (can be rounded)
   * @param: int
   *           b: Var to compare; it is comverted internally in an int
   * @return: true if they are equals, false otherwise
   */
  ...
  /**
   * Method: is Condition value negative evaluation
   * 
   * @param: IN
   *           Cond cond
   ...
   * @param: IN
   *           Cond cond
   * @return: true if the Cond evaluates as false @ see Class Cond
   */
  ...
  /**
   * Method: is Variable value boolean comparison
   * 
   * @param: IN
   *           Variable v
   * @return: true if the variable is true. @ see Class Cond
   */
  ...
  /**
   * Method: isLowValue return true if the Var contains only low value bytes.
   * 
   * @param: IN
   *           Var var
  ...
  /**
   * Method: isLowValue return true if the String contains only low value bytes.
   * 
   * @param: IN
   *           String cs
  ...
  /**
   * Method: isNotLowValue Opposite result than isLowValue
   * 
   * @param:
   * @return:
   * @see isLowValue
  ...
  /**
   * Method: isHighValue return true if the String contains only high value bytes.
   * 
   * @param: IN
   *           String cs
  ...
  /**
   * Method: isNotHighValue Opposite result than isHighValue
   * 
   * @param:
   * @return:
   * @see isHighValue
   ...
   * @param: IN
   *           VarAndEdit var
   * @return: true if the var contains only space chars; false otherwise If the
   *          var is an Edit, then only, it's textual value is taken in account
   */
   ...
   * @param: IN
   *           VarAndEdit var
   * @return: false if the var contains only space chars; true otherwise If the
   *          var is an Edit, then only, it's textual value is taken in account
   */
  ...
  /**
   * Method: isZero return true if the Var contains only 0 chars
   * 
   * @param: IN
   *           var var
   * @return:
  ...
  /**
   * Method: isZero return false if the Var contains only 0 chars
   * 
   * @param: IN
   *           var var
   * @return:
  ...
  /**
   * Method: isZero return true if the int equals 0
   * 
   * @param: IN
   *           var var
   * @return:
  ...
  /**
   * Method: isAll return true if the var's value contains only the same char
   * cPattern
   * 
   * @param: IN
   *           var var
  ...
  /**
   * Method: isNotAll return true if the var's value contains only occurences of
   * the same string csPattern
   * 
   * @param: IN
   *           Var var
   * @param: IN
  ...
  /**
   * Method: isAll return true if the var's value contains only occurences of the
   * same string csPattern
   * 
   * @param: IN
   *           Var var
   * @param: IN
  ...
  /**
   * Method: isAll return true if the string's value contains only the same char
   * cPattern
   * 
   * @param: IN
   *           String csValue
  ...
  /**
   * Method: isAll return true if the var's value contains only occurences of the
   * same string contained into varPattern
   * 
   * @param: IN
   *           Var var
   * @param: IN
  ...
  /**
   * Method: isDifferent return true if the 2 var's value are different
   * 
   * @param: IN
   *           Var var1
   * @param: IN
  ...
  /**
   * Method: isLess return true if var1's value < var2's value
   * 
   * @param: IN
   *           Var var1
   * @param: IN
   ...
   * @param: IN
   *           VarAndEdit var2
   * @return: true if var1's value < var2's value, false otherwise. The comparison
   *          is done using ebcdic ordering Warning: This method is available only
   *          for ebcdic compatibility. It should not be used except when dealing
   *          with not ascii convertered data If Var1 or var2 are Edit or dervied
   *          from Edit, then only the textual part of the edit is taken in
   *          account
   */
   ...
   * @param: IN
   *           VarAndEdit var2
   * @return: true if var1's value <= var2's value, false otherwise. If Var1 or
   *          var2 are Edit or dervied from Edit, then only the textual part of
   *          the edit is taken in account
   */
   ...
   * @param: IN
   *           VarAndEdit var2
   * @return: true if var1's value <= var2's value, false otherwise. The
   *          comparison is done using ebcdic ordering Warning: This method is
   *          available only for ebcdic compatibility. It should not be used
   *          except when dealing with not ascii convertered data If Var1 or var2
   *          are Edit or dervied from Edit, then only the textual part of the
   *          edit is taken in account
   */
   ...
   * @param: IN
   *           VarAndEdit var2
   * @return: true if var1's value > var2's value, false otherwise If Var1 or var2
   *          are Edit or dervied from Edit, then only the textual part of the
   *          edit is taken in account
   */
  ...
  /**
   * Method: isGreater return true if var1's value > var2's value
   * 
   * @param: IN
   *           MathBase var1
   * @param: IN
  ...
  /**
   * Method: isGreater return true if var1's value > nb; var1 is evaluated as an
   * int (can be rounded).
   * 
   * @param: IN
   *           MathBase var1
   ...
   * 
   * @param: IN
   *           Var var1: 1st operand of the operation; must contain a numeric
   *           value, or string that evalues as a numeric value; it can be signed
   *           or not; it can contain either an integer or decimal value
   * @param: IN
   *           Var var2: same remark than var1
   * @return: MathAdd: wrapper object containg the sum of the 2 parameters; the
   *          maximum precision is kept
   */
   ...
   * 
   * @param: IN
   *           int m: integer ot add to n
   * @param: IN
   *           int n: integer ot add to m
   * @return: MathAdd: wrapper object containg the sum of the 2 parameters; the
   *          maximum precision is kept
   */
   ...
   * 
   * @param: IN
   *           Var var1: operand of the operation; must contain a numeric value,
   *           or string that evalues as a numeric value; it can be signed or not;
   *           it can contain either an integer or decimal value
   * @param: IN
   *           double d: floting point value added to var, giving an operand of
   *           the operation
   * @return: MathAdd: wrapper object containg the sum of the 2 parameters; the
   *          maximum precision is kept
   */
   ...
   *           or not; it can contain either an integer or decimal value
   * @param: IN
   *           String s: String that must be able to be converted to a numeric
   *           value
   * @return: MathAdd: wrapper object containg the sum of the 2 parameters; the
   *          maximum precision is kept
   */
   ...
   *           or not; it can contain either an integer or decimal value
   * @param: IN
   *           MathBase mathBase: Math container giving an operand of the
   *           operation
   * @return: MathAdd: wrapper object containg the sum of the 2 parameters; the
   *          maximum precision is kept
   */
   ...
   *           operation
   * @param: IN
   *           int n: operand of the operation
   * @return: MathSubtract: wrapper object containg the sum of the 2 parameters;
   *          the maximum precision is kept
   */
  ...
  /**
   * Method: subtract addition
   * 
   * @param: IN
   *           Var var1: operand of the operation; Must evaluate to a numeric
   *           value
   * @param: IN
   *           Var var2: operand of the operation; Must evaluate to a numeric
   *           value
   * @return: MathSubtract: wrapper object containg the var1 - var2, while keeping
   ...
   * @param: IN
   *           int n: operand of the operation
   * @return: MathSubtract: wrapper object containg the var1 - n, while keeping
   *          the maximum precision
   */
   ...
   *           Var var1: operand of the operation; Must evaluate to a numeric
   *           value
   * @return: MathSubtract: wrapper object containg the var1, while keeping the
   *          maximum precision
   */
   ...
   *           Var var2: operand of the operation; Must evaluate to a numeric
   *           value
   * @return: MathSubtract: wrapper object containg the n - var2, while keeping
   *          the maximum precision
   */
   ...
   *           value
   * @param: IN
   *           double d: operand of the operation;
   * @return: MathSubtract: wrapper object containg the var1 - d, while keeping
   *          the maximum precision
   */
   ...
   *           Var var2: operand of the operation; Must evaluate to a numeric
   *           value
   * @return: MathSubtract: wrapper object containg the d - var2, while keeping
   *          the maximum precision
   */
   ...
   *           value
   * @param: IN
   *           String s: operand of the operation; Must evaluate to a numeric
   *           value
   * @return: MathSubtract: wrapper object containg the var1 - var2, while keeping
   *          the maximum precision
   */
   ...
   * 
   * @param: IN
   *           String: operand of the operation; Must evaluate to a numeric value
   * @param: IN
   *           Var var2: operand of the operation; Must evaluate to a numeric
   *           value
   * @return: MathSubtract: wrapper object containg the s - var2, while keeping
   *          the maximum precision
   */
   ...
   *           value
   * @param: IN
   *           MathBase mathBase: operand of the operation
   * @return: MathSubtract: wrapper object containg the var1 - mathBase, while
   *          keeping the maximum precision
   */
   ...
   * 
   * @param: IN
   *           MathBase mathBase: operand of the operation; Must evaluate to a
   *           numeric value
   * @param: IN
   *           Var var2: operand of the operation; Must evaluate to a numeric
   *           value
   * @return: MathSubtract: wrapper object containg the mathBase - var2, while
   *          keeping the maximum precision
   */
   ...
   * @param: IN
   *           int n: operand of the operation;
   * @return: MathSubtract: wrapper object containg the mathBase - n, while
   *          keeping the maximum precision
   */
   ...
   * @param: IN
   *           double d: operand of the operation;
   * @return: MathSubtract: wrapper object containg the mathBase - d, while
   *          keeping the maximum precision
   */
   ...
   *           MathBase mathBase: operand of the operation;
   * @param: IN
   *           String s: operand of the operation; must evaluates to a numeric
   *           value
   * @return: MathSubtract: wrapper object containg the mathBase - s, while
   *          keeping the maximum precision
   */
   ...
   * 
   * @param: IN
   *           MathBase mathBase1: operand of the operation;
   * @param: IN
   *           MathBAse mathBase2: operand of the operation;
   * @return: MathSubtract: wrapper object containg the mathBase1 - mathBase2,
   *          while keeping the maximum precision
   */
   ...
   * 
   * @param IN
   *          var1 Dividende variable; may be integer or decimal
   * @param IN
   *          var2 Divisor variable; may be integer or decimal
   * @return Return the MathDivide objet that embed the return of var1 / var2.
   *         It's value may also be used as modulo and rest of an integer division
   */
   ...
   * 
   * @param IN
   *          Var var1 Dividende variable; may be integer or decimal
   * @param IN
   *          double d Divisor variable
   * @return Return the MathDivide objet that embed the return of var1 / var2.
   *         It's value may also be used as modulo and rest of an integer division
   */
   ...
   *          Var var1 Dividende variable; may be integer or decimal
   * @param IN
   *          String s Divisor variable, must contain a numeric value, that may be
   *          decimal or not
   * @return Return the MathDivide objet that embed the return of var1 / var2.
   *         It's value may also be used as modulo and rest of an integer division
   */
   ...
   *          Var var1 Dividende variable; may be integer or decimal
   * @param IN
   *          MathBase mathBase Divisor variable resulting from a previosu
   *          operation
   * @return Return the MathDivide objet that embed the return of var1 / var2.
   *         It's value may also be used as modulo and rest of an integer division
   */
   ...
   * 
   * @param IN
   *          MathBase mathBase: Dividende variable; may be integer or decimal
   * @param IN
   *          Var var2 Divisor variable resulting from a previosu operation
   * @return Return the MathDivide objet that embed the return of var1 / var2.
   *         It's value may also be used as modulo and rest of an integer division
   */
   ...
   *          MathBase mathBase: Dividende variable; may be integer or decimal
   * @param IN
   *          int n Divisor variable
   * @return Return the MathDivide objet that embed the return of mathBase / n.
   *         It's value may also be used as modulo and rest of an integer division
   */
   ...
   *          MathBase mathBase: Dividende variable; may be integer or decimal
   * @param IN
   *          String s Divisor variable
   * @return Return the MathDivide objet that embed the return of mathBase / s.
   *         It's value may also be used as modulo and rest of an integer division
   */
   ...
   * 
   * @param IN
   *          MathBase mathBase1: Dividende variable; may be integer or decimal
   * @param IN
   *          MathBase mathBase2 Divisor variable
   * @return Return the MathDivide objet that embed the return of mathBase1 /
   *         MathBase2. It's value may also be used as modulo and rest of an
   *         integer division
   */
   ...
   * 
   * @param IN
   *          Var var1: operand1 variable; may be integer or decimal
   * @param IN
   *          Var var2: operand2 variable
   ...
   * 
   * @param IN
   *          int a: operand1 variable; may be integer or decimal
   * @param IN
   *          int b: operand2 variable
   ...
   * 
   * @param IN
   *          int n: operand1 variable; may be integer or decimal
   * @param IN
   *          Var var2: operand2 variable
   ...
   * 
   * @param IN
   *          double d: operand1 variable; may be integer or decimal
   * @param IN
   *          Var var2: operand2 variable
   ...
   *          Var var1: operand1 variable; may be integer or decimal
   * @param IN
   *          String s: operand2 variable that must parse as a number
   ...
   * 
   * @param IN
   *          String s: operand1 variable; may be integer or decimal
   * @param IN
   *          Var var1: operand2 variable
   ...
   * 
   * @param IN
   *          MathBase mathBase1: operand1 variable; may be integer or decimal
   * @param IN
   *          MathBase mathBase2: operand2 variable
   * @return Return the MathMultiply objet that embed the return of mathBase1 *
   *         mathBase2.
   */
   ...
   * @param IN
   *          MathBase mathBase2: operand2 variable
   * @return Return the MathMultiply objet that embed the return of var1 *
   *         mathBase1.
   */
   ...
   * 
   * @param IN
   *          MathBase mathBase: operand1 variable; may be integer or decimal
   * @param IN
   *          Var var: operand2 variable
   * @return Return the MathMultiply objet that embed the return of mathBase *
   *         var.
   */
   ...
   *          int n: operand1 variable; may be integer or decimal
   * @param IN
   *          MathBase mathBase: operand2 variable
  ...
  /**
   * Method: setTrue Force a condition to always evaluate as true
   * 
   * @param: IN
   *           Cond cond
  ...
  /**
   * Method: move move a value into a var
   * 
   * @param: IN
   *           int n
   * @param: OUT
  ...
  /**
   * Method: move move a constant (LowValue, HighValue, Zero, Space) into a var;
   * all data chars of the destination var are filled with the source constant
   * 
   * @param: CobolConstant
   *           constant
   * @param: OUT
  ...
  /**
   * Method: move move a flotting point numeric value into a var
   * 
   * @param: IN
   *           double d
   * @param: OUT
  ...
  /**
   * Method: move move a string value into a var
   * 
   * @param: IN
   *           String cs
   * @param: OUT
  ...
  /**
   * Method: move fill the destination var with the source string, used as a
   * pattern
   * 
   * @param: IN
   *           String cs
  ...
  /**
   * Method: move move a var's value into another var; convertions may occur,
   * depending on the pic declaration of the 2 vars.
   * 
   * @param: IN
   *           Var varSource
   * @param: OUT
  ...
  // {
  // logCesm("moveAllEdit: varSource="+varSource.getLoggableValue()+" to
  // Edit="+varDest.getLoggableValue());
  // varDest.set(varSource);
  // return this;
  // }

  /**
   * Method: move move all fields of a form (GUI applications only) into a
   * destionation var. Every field has a 7 chars header containg various
   * attributes, follwed by the filed's data value. It can be thought of as a
   * serialization of a form into a var; <b>used internally</b>
   * 
   * @param: IN
   *           Form formSource
   * @param: OUT
  ...
  /**
   * Method: move move a var content into all fields of a form (GUI applications
   * only). The var source must have been previously filled by a previous
   * move(Form formSource, Var varDest). Thus this method can be thought of as a
   * deserialization of a var into a form; <b>used internally</b>
   * 
   * @param: IN
   *           Var varSource
   * @param: OUT
  ...
  // {
  // logCesm("move var to Form: varSource="+varSource.getLoggableValue()+" to
  // Form="+fromDest.getLoggableValue());
  // fromDest.decodeFromVar(varSource);
  // return this;
  // }

  /**
   * move move a form to another one
   * 
   * @param IN
   *          formSource: Source form
   * @param OUT
   *          fromDest: Destination form
   * @return: current program object; enables to chain another Program's method.
   */
  // protected Program move(Form formSource, Form fromDest)
  // {
  // logCesm("move Form to Form: formSource="+formSource.getLoggableValue()+" to
  // Form="+fromDest.getLoggableValue());
  ...
  /**
   * Method: move fill the destination var with the sourceVar contents, used as a
   * pattern
   * 
   * @param: IN
   *           Var varSource
  ...
  /**
   * Method: moveCorresponding move all identically named vars below varSource to
   * varDestGroup
   * 
   * @param: IN
   *           Var varSource: It may have children
   * @param: OUT
   *           Var varDestGroup: It may have children
   * @return: current program object; enables to chain another Program's method.
   */
  ...
  protected BaseProgram moveCorresponding(VarBase varSource, VarBase varDestGroup) {
    ...
    // This method is to help a recursive descent in case the move corresponding
    // manages multi level depth. It seems not to be the case, so it's implemented
    // as signle level depth.
    if (varSource != null) {
  ...
  // protected BaseProgram moveCorresponding(VarBase varSource, VarBase
  // varDestGroup)
  // {
  // if(IsSTCheck)
  // Log.logFineDebug("moveCorresponding_V_V:" + varSource.getSTCheckValue() + ":"
  // + varDestGroup.getSTCheckValue());
  //
  // CacheMoveCorresponding cacheMoveCorresponding =
  // getCacheMoveCorresponding(varSource, varDestGroup);
  //
  // // This method is to help a recursive descent in case the move corresponding
  // manages multi level depth. It seems not to be the case, so it's implemented
  // as signle level depth.
  ...
  // }
  //
  // private CacheMoveCorresponding getCacheMoveCorresponding(VarBase varSource,
  // VarBase varDestGroup)
  // {
  ...
  /**
   * Method: perform calls a paragraph, using the library. Used to wrap and hide
   * internally used exception handlers
   * 
   * @param: IN
   *           Paragraph paragraph: Functor objet which is called
   * @return:
   */
  ...
  /**
   * Method: perform calls the first paragraph of a section, using the library.
   * 
   * @param: IN
   *           Section section: Functor objet that group multiple paragraphs. All
   *           paragraphs of the section identified will be called in sequence if
   *           no flow breaking (goto StopRun ...) occurs.
   * @return:
   */
  ...
  /**
   * Method: performTrough calls sequentially all paragraphs that lies between
   * paragraphBegin and paragraphEnd.
   * 
   * @param: Paragraph
   *           paragraphBegin: Functor objet that identified the first paragraph
   *           to call
   * @param: Paragraph
   *           paragraphEnd: Functor objet that identified the last paragraph to
   *           call
   * @return:
   */
  ...
  /**
   * Method: goTo Transfer the flow control the provided paragraph. No stack
   * pushing occurs.
   * 
   * @param: Paragraph
   *           paragraph: Functor objet that identified the paragraph where to
  ...
  /**
   * Method: goTo Transfer the flow control the first paragrpaph of the provided
   * section. No stack pushing occurs.
   * 
   * @param: Section
   *           section: Functor objet that contains mulpliple paragraphs. The
   *           first one is used as the destination of the flow control.
   * @return:
   */
   ...
   *           Return from the current sub program; do nothing if we are not
   *           inside a sub program. A sub program is a program that is called
   *           explicitly by another application program
   */
  ...
  /**
   * exit Do nothing: The EXIT statement enables you to assign a procedure-name to
   * a given point in a program. As an IBM extension, the EXIT statement does not
   * need to appear in a sentence by itself. Any statements following the EXIT
   * statement are executed; the EXIT statement is treated as the CONTINUE
   * statement.
   * 
  ...
  protected void exit() {
    ...
    // The EXIT statement enables you to assign a procedure-name to a given point in
    // a program.
   ...
   * 
   * @param IN
   *          Var var: Variable that is incremented of 1; the var must contain a
   *          numeric value
   */
   ...
   * 
   * @param IN/OUT
   *          Var var: Variable that is decremented of 1; the var must contain a
   *          numeric value
   */
  ...
  /**
   * @param IN
   *          int nStep: Increment step
   * @param IN/OUT
   *          Var var: Variable that is incremented of nStep; the var must contain
   *          a numeric value
   */
  ...
  /**
   * @param IN
   *          Var varStep: Increment step
   * @param IN/OUT
   *          Var var: Variable that is incremented of varStep's value; the var
  ...
  /**
   * @param IN
   *          int nStep: Decrement step
   * @param IN/OUT
   *          Var var: Variable that is decremented of nStep; the var must contain
   *          a numeric value
   */
  ...
  /**
   * @param IN
   *          MathBase value: Contains a numeric vale, resulting from a previos
   *          calculous
   * @param OUT
   *          Var var: Variable whose value is set on output the value of
   *          mathBase; No rounding is done upon set var's value
   ...
   * @param OUT
   *          Var var: Variable whose value is set on output the value of
   *          mathBase; The var's value is rounded upon setting, as opposite of
   *          "MathBase compute(MathBase mathBase, Var var)"
   * @return mathBase object, enabling to chain calculous
   */
   ...
   *          int nValue
   * @param OUT
   *          Var varDest; Set the value of the varDest to nValue; same as
   *          "move(nValue, varDest);"
   * @return
   */
  ...
  protected SQLCall sqlCall(VarAndEdit statement) {
    ...
    String csStatement = statement.getString()
                                  .trim();
  ...
  protected SQL sql(String csStatement) {
    ...
    // String csFileLine = StackStraceSupport.getFileLineAtStackDepth(2); // Caller
    // File Line
    // int nHashFileLine = 0;
    // if(csFileLine != null)
    ...
    // Was active: DbConnectionBase SQLConnection =
  ...
  /**
   * @param IN
   *          csString Statement; Must be sql valid.
   * @return SQLCursor internal object used to chain sql cusror declarations.
   */

  /*
   * protected SQLCursor sqlCursor(String csStatement) {
   * if(m_ProgramManager.m_CESMEnv != null) return
   * sqlCursor(m_ProgramManager.m_CESMEnv.getSQLConnection(), csStatement); return
   * sqlCursor(null, csStatement); }
   */
  /**
   * @param SQLConnection
   ...
   *         Remark: This method should not be used as it requires a
   *         SQLConnection. To use the default SQL connection established in a
   *         standard way, use SQL sql(String csStatement)
   */
  // protected SQLCursor sqlCursor(CSQLConnection SQLConnection, String
  // csStatement)
  // {
  // SQLCursor sqlCursor = new
  // SQLCursor(m_ProgramManager.m_DataDivision.getWorkingStorageSectionVarBuffer(),
  // SQLConnection, csStatement);
  // return sqlCursor;
  // }

  /**
   * @param IN/OUT
   *          SQLCursor sqlCursor Opens the cursor in parameter
   */
  ...
  // m_ProgramManager.m_CESMEnv, csQuery, m_ProgramManager.getSQLStatus());
  // sqlCursor.open();
  // return sqlCursor;
  // }
  ...
  /**
   * @param IN/OUT
   *          SQLCursor sqlCursor Closes the cursor in parameter
   */
  ...
  protected SQLCursorOperation cursorUpdateCurrent(SQLCursor sqlCursor, String csUpdateClause) {
    ...
    SQLCursorOperation sqlCursorOperation = new SQLCursorOperation(m_BaseProgramManager, sqlCursor, csUpdateClause);
  ...
  protected SQLCursorOperation cursorDeleteCurrent(SQLCursor sqlCursor, String csDeleteClause) {
    ...
    SQLCursorOperation sqlCursorOperation = new SQLCursorOperation(m_BaseProgramManager, sqlCursor, csDeleteClause);
    return sqlCursorOperation;
  }
   ...
   * @param IN/OUT
   *          SQLCursor sqlCursor
   * @return Internal SQLCursor object; The into parameters are filled with the
   *         columns values read during the fetch.
   */
  ...
  /**
   * @param VAR
   *          OUT var: variable that is filled with CobolConstant.Space characters
   */
  ...
  /**
   * @param VAR
   *          OUT var: variable that is filled with CobolConstant.LowValue
   *          characters
   */
  ...
  /**
   * @param VAR
   *          OUT var: variable that is filled with CobolConstant.HighValue
   *          characters
   */
  ...
  /**
   * @param VAR
   *          OUT var: variable that is filled with CobolConstant.Zero characters
   */
  ...
  /**
   * @param OUT
   *          Cond cond: Force the condition to always evalate as true
   */
  ...
  /**
   * @param IN
   *          Var var: Variable whose value is evaluated
  ...
  /**
   * @param IN
   *          Var var: Variable whose value is tested as been a valid number
  ...
  /**
   * @param OUT
   *          Var vSource Intialize the var group in parameter, whatever it's
   *          children types
   * @see http://www.helsinki.fi/atk/unix/dec_manuals/cobv27ua/cobrm_028.htm#index_x_727
   *      for sample
   */
  ...
  // getProgramManager().getOrCreateInitializeCacheAtFirstAppCall();
  // return initializeCache;
  // }
  //
  // protected void initializeOpt(VarAndEdit vSource, InitializeCache
  // initializeCache)
  // {
  // if(IsSTCheck)
  // Log.logFineDebug("initialize_V:" + vSource.getSTCheckValue());
  //
  // //InitializeCache initializeCache =
  // getProgramManager().getOrCreateInitializeCacheAtStackDepth(3);
  // vSource.initialize(initializeCache);
   ...
   *          Var vSource
   * @param IN
   *          int n Intialize all numeric children of the group vSource by n
   * @see http://www.helsinki.fi/atk/unix/dec_manuals/cobv27ua/cobrm_028.htm#index_x_727
   *      for sample
   */
   ...
   *          Var vSource
   * @param IN
   *          int n Intialize all numeric edited children of the group vSource by
   *          n
   * @see http://www.helsinki.fi/atk/unix/dec_manuals/cobv27ua/cobrm_028.htm#index_x_727
   *      for sample
   */
   ...
   *          Var vSource
   * @param IN
   *          double d Intialize all numeric children of the group vSource by d
   * @see http://www.helsinki.fi/atk/unix/dec_manuals/cobv27ua/cobrm_028.htm#index_x_727
   *      for sample
   */
   ...
   *          Var vSource
   * @param IN
   *          String s Intialize all numeric children of the group vSource by d
   * @see http://www.helsinki.fi/atk/unix/dec_manuals/cobv27ua/cobrm_028.htm#index_x_727
   *      for sample
   */
   ...
   *          Var vSource
   * @param IN
   *          double d Intialize all numeric edited children of the group vSource
   *          by d
   * @see http://www.helsinki.fi/atk/unix/dec_manuals/cobv27ua/cobrm_028.htm#index_x_727
   *      for sample
   */
   ...
   *          Var vSource
   * @param IN
   *          String cs Intialize all alphanumeric children of the group vSource
   *          by cs
   * @see http://www.helsinki.fi/atk/unix/dec_manuals/cobv27ua/cobrm_028.htm#index_x_727
   *      for sample
   */
   ...
   *          Var vSource
   * @param IN
   *          String cs Intialize all alphanumeric edited children of the group
   *          vSource by cs
   * @see http://www.helsinki.fi/atk/unix/dec_manuals/cobv27ua/cobrm_028.htm#index_x_727
   *      for sample
   */
  ...
  /**
   * @param IN
   *          String cs: String to be concated with followings
   * @return Internal Concat object, that is used to accumulate the string
   *         concatenation
   */
  ...
  /**
   * @param IN
   *          Var var: variable whose vaue is to be concated with followings
   * @return Internal Concat object, that is used to accumulate the string
   *         concatenation
   */
   ...
   *          Var var: variable whose vaue is to be concated with followings
   * @param IN
   *          String csDelimiter: String used to find to leading string from var's
   ...
   *          Var var: variable whose vaue is to be concated with followings
   * @param IN
   *          Var varDelimiter: variable used to find to leading string from var's
   *          value to concatenate
   * @return Internal Concat object, that is used to accumulate the string
   *         concatenation
   */
  ...
  /**
   * @param IN
   *          String csVar: String whose vaue is to be concated with followings
   * @param IN
   *          String csDelimiter: String used to find to leading string from var's
   *          value to concatenate
   * @return Internal Concat object, that is used to accumulate the string
  ...
  /**
   * @param IN
   *          Var var: variable whose value, up to the first space char, is to be
   *          concated with followings
   * @return Internal Concat object, that is used to accumulate the string
   *         concatenation
   */
  ...
  /**
   * @param IN
   *          String cs: String whose value, up to the first space char, is to be
   *          concated with followings
   * @return Internal Concat object, that is used to accumulate the string
   *         concatenation
   */
  ...
  /**
   * @param IN
   *          Var var: var's value that is to be concated with followings one
   * @return Internal Concat object, that is used to accumulate the string
   *         concatenation
   */
  ...
  /**
   * @param IN
   *          String cs: String whose whole value, whatever it's end, is to be
   *          concated with followings
   * @return Internal Concat object, that is used to accumulate the string
   *         concatenation
   */
  ...
  /**
   * @param IN/OUT
   *          Map map Fill all forms of a map with low value
   */
   ...
   * @param IN
   *          Var var
   * @return a pointer on var Used internally
   */
  ...
  /**
   * @param Var
   *          v: String whose substring is to be filled with 0's
   * @param int
   *          nOffsetPosition Start position (begining at 1) of the substring to
   *          fill
   * @param int
   *          nNbChar Number of chars to fill Fill the substring of v with 0's,
   *          from position start up to nNbChar characters
   */
  ...
  /**
   * @param Var
   *          v: String whose substring is to be filled with space chars
   * @param int
   *          nOffsetPosition Start position (begining at 1) of the substring to
   *          fill
   * @param int
   *          nNbChar Number of chars to fill Fill the substring of v with spaces,
   *          from position start up to nNbChar characters
   */
  ...
  /**
   * @param Var
   *          v: String whose substring is to be filled with low value chars
   * @param int
   *          nOffsetPosition Start position (begining at 1) of the substring to
   *          fill
   * @param int
   *          nNbChar Number of chars to fill Fill the substring of v with low
   *          value, from position start up to nNbChar characters
   */
  ...
  /**
   * @param Var
   *          v: String whose substring is to be filled with high value chars
   * @param int
   *          nOffsetPosition Start position (begining at 1) of the substring to
   *          fill
   * @param int
   *          nNbChar Number of chars to fill Fill the substring of v with high
   *          value, from position start up to nNbChar characters
   */
   ...
   *          v: String whose substring is to be filled with space chars
   * @param Var
   *          varOffsetPosition: Start position (begining at 1) of the substring
   *          to fill
   * @param int
   *          nNbChar Number of chars to fill Fill the substring of v with spaces,
   *          from position start up to nNbChar characters
   ...
   * @param int
   *          nOffsetPosition: Start position into to source variable
   * @param int
   *          nNbChar: Number of char to extract form source variable
   * @param Var
   *          varValue: Source variable Fill the destination variable with the
   *          substring of source variable, starting from position
   *          nOffsetPosition, and containing up to nNbChar
   */
  ...
  protected void setSubString(VarAndEdit varDest, int nOffsetPosition, VarAndEdit varValue) {
    setSubString(varDest, nOffsetPosition, varDest.getVarDef()
                                                  .getLength(),
        varValue);
  }

  protected void setSubString(VarAndEdit varDest, VarAndEdit varOffsetPosition, VarAndEdit varValue) {
    setSubString(varDest, varOffsetPosition.getInt(), varDest.getVarDef()
                                                             .getLength(),
        varValue);
  }
   ...
   *          nOffsetPosition: Start position into to source variable
   * @param Var
   *          varNbChar: Number of char to extract form source variable
   * @param Var
   *          varValue: Source variable Fill the destination variable with the
   *          substring of source variable, starting from position
   *          nOffsetPosition, and containing up to nNbChar
  ...
  protected void setSubString(VarAndEdit varDest, int nOffsetPosition, Var varNbChar, VarAndEdit varValue) {
    if (IsSTCheck)
      Log.logFineDebug(
          "setSubString_V_n_V_V:" + varDest.getSTCheckValue() + ":" + nOffsetPosition + ":" + varNbChar.getSTCheckValue() + ":" + varValue.getSTCheckValue());
  ...
  protected void setSubString(VarAndEdit varDest, Var vOffsetPosition, Var varNbChar, Var varValue) {
    if (IsSTCheck)
      Log.logFineDebug("setSubString_V_V_V_V:" + varDest.getSTCheckValue() + ":" + vOffsetPosition.getSTCheckValue() + ":" + varNbChar.getSTCheckValue() + ":"
  ...
  protected void setSubString(VarAndEdit varDest, Var vOffsetPosition, MathBase nNbChar, Var varValue) {
    if (IsSTCheck)
      Log.logFineDebug("setSubString_V_V_M_V:" + varDest.getSTCheckValue() + ":" + vOffsetPosition.getSTCheckValue() + ":" + nNbChar.getSTCheckValue() + ":"
          + varValue.getSTCheckValue());
   ...
   *          varDest: Destination variable
   * @param int
   *          nOffsetPosition: Startposition into to source variable
   * @param Var
   *          varNbChar: Number of char to extract form source variable
   * @param String
   *          csValue: Source string Fill the destination variable with the
   *          substring of source variable, starting from position
   *          nOffsetPosition, and containing up to nNbChar
   */
  protected void setSubString(VarAndEdit varDest, Var nOffsetPosition, Var nNbChar, String csValue) {
    if (IsSTCheck)
      Log.logFineDebug(
          "setSubString_V_V_V_cs:" + varDest.getSTCheckValue() + ":" + nOffsetPosition.getSTCheckValue() + ":" + nNbChar.getSTCheckValue() + ":" + csValue);
  ...
  protected void setSubString(VarAndEdit varDest, MathBase nOffsetPosition, Var nNbChar, String csValue) {
    if (IsSTCheck)
      Log.logFineDebug(
          "setSubString_V_M_V_cs:" + varDest.getSTCheckValue() + ":" + nOffsetPosition.getSTCheckValue() + ":" + nNbChar.getSTCheckValue() + ":" + csValue);
   ...
   *          varDest: Destination variable
   * @param Var
   *          varOffsetPosition: Start position into to source variable
   * @param int
   *          nNbChar: Number of char to extract form source variable
   * @param String
   *          csValue: Source string Fill the destination variable with the
   ...
   *          varOffsetPosition: Start position into to source variable
   * @param Var
   *          varChar: Number of char to extract form source variable
   * @param String
   *          csValue: Source string Fill the destination variable with the
   *          substring of source variable, starting from position
   *          nOffsetPosition, and containing up to nNbChar
  ...
  protected void setSubString(VarAndEdit varDest, Var varOffsetPosition, int nNbChar, VarAndEdit varValue) {
    if (IsSTCheck)
      Log.logFineDebug(
          "setSubString_V_V_n_V" + varDest.getSTCheckValue() + ":" + varOffsetPosition.getSTCheckValue() + ":" + nNbChar + ":" + varValue.getSTCheckValue());
  ...
  /**
   * @return void Placeholder to identify the SLQCursor variable declaration. Can
   *         be omitted
   */
  ...
  /**
   * @param Var
   *          var: Source variable on which doing inspect replacing operations
   * @return InspectReplacing object, enabling options see class InspectReplacing
   */
  ...
  /**
   * @param Var
   *          var: Source variable on which doing inspect tallying operations
   * @return InspectTallying object, enabling options see class InspectTallying
   */
  ...
  /**
   * @param String
   *          csSource: Source string
   * @param int
   *          nStart: 1 based start position into source string
   * @param int
   *          nNbChars: Number of chars to extract
   * @return String Subtring of the source string, starting form position start,
   *         up to nNbChars chars
   */
   ...
   * @param int
   *          nNbChars: Number of chars to extract
   * @return String Subtring of varSource's string value, starting form position
   *         start, up to nNbChars chars
   */
   ...
   *          varSource: Source variable
   * @param Var
   *          varStart: 1 based start position into source string
   * @param int
   *          nNbChars: Number of chars to extract
   * @return String Subtring of varSource's string value, starting form position
   *         start, up to nNbChars chars
   ...
   *          nStart: 1 based start position into source string
   * @param Var
   *          varNbChars: Number of chars to extract
   * @return String Subtring of varSource's string value, starting form position
   *         start, up to nNbChars chars
   */
  ...
  protected int getSQLCode() {
    ...
    return m_BaseProgramManager.getSQLStatus()
                               .getSQLCode();
  }
  ...
  protected int getSQLDiagnosticCode(Var v) {
    ...
    int n = m_BaseProgramManager.getSQLStatus()
                                .getSQLDiagnosticCode(v.getInt());
    if (m_bUsedTempVarOrCStr)
      m_tempCache.resetTempIndex(v);
    return n;
  }
  ...
  protected boolean isSQLCode(int n) {
    ...
    int c = m_BaseProgramManager.getSQLStatus()
                                .getSQLCode();
    return c == n;
  }
  ...
  protected void resetSQLCode(int n) {
    ...
    m_BaseProgramManager.getSQLStatus()
                        .setSQLCode(n);
  }
   ...
   * @param Var
   *          var
   * @return Number of bytes ureserved for storage of the var, as defined in
   ...
   *          varDest: Destination variable
   * @param String
   *          csValue: Semantic context value Associates the semantic context
   *          value csVlaue, to the variable varDest. It is also associated to all
   *          variables that share it's position and length in storage buffer
   *          (redefines, ...)
   */
  ...
  /**
   * @param Var
   *          varSource: Source variable whose semantic value is to be found
  ...
  public void displayEnv(VarAndEdit var) {
    String csMessage = var.getDottedSignedString();
    m_BaseProgramManager.getEnv()
                        .setDisplay(csMessage);
    resetTempIndex(var);
  }

  protected String getLastCommandReturnCode() {
    /*
     * This EIB field contains the CICS response code returned after the function
     * requested by the last CICS command to be issued by the task has been
     * completed. For a normal response, this field contains hexadecimal zeros
     * (X'00'). For COBOL programs only, almost all of the information in this field
     * can be used within Var VNS4 = _02().picS9(4).value(4);ication programs by the
     * HANDLE CONDITION command. For COBOL: PIC X(6)
     */
    if (IsSTCheck)
  ...
  protected String getTime() {
    ...
    return m_BaseProgramManager.getEnv()
                               .getTime();
  }
  ...
  /**
   * @return String This EIB field contains the date at which the task is started
   *         (this field is updated by the ASKTIME command). The date is in packed
   *         decimal form (0CYYDDD+). Ex : 0099365 for 31/12/1999 or 0104001 for
   *         01/01/2004 For COBOL: PIC S9(7) COMP-3
   */
  ...
  protected int getConditionOccured() { // http://publib.boulder.ibm.com/infocenter/txen/topic/com.ibm.txseries510.doc/erzhai00148.htm#TBLZR009T4
    /*
     * EIBRESP This EIB field contains a 32-bit binary number corresponding to the
     * condition that has occurred. These numbers are listed in Table 26 (in
     * decimal) for the conditions that can occur on local requests during execution
     * of the commands described in CICS API command reference. For COBOL: PIC S9(8)
     * COMP
     */
    if (IsSTCheck)
  ...
  protected void setConditionOccured(Var v) { // http://publib.boulder.ibm.com/infocenter/txen/topic/com.ibm.txseries510.doc/erzhai00148.htm#TBLZR009T4
    /*
     * EIBRESP This EIB field contains a 32-bit binary number corresponding to the
     * condition that has occurred. These numbers are listed in Table 26 (in
     * decimal) for the conditions that can occur on local requests during execution
  ...
  protected void setConditionOccured(int n) { // http://publib.boulder.ibm.com/infocenter/txen/topic/com.ibm.txseries510.doc/erzhai00148.htm#TBLZR009T4
    /*
     * EIBRESP This EIB field contains a 32-bit binary number corresponding to the
     * condition that has occurred. These numbers are listed in Table 26 (in
     * decimal) for the conditions that can occur on local requests during execution
  ...
  protected int getTaskNumber() {
    /*
     * This EIB field contains the task number assigned to the task by CICS. This
     * number appears in trace entries generated while the task is in control. For
  ...
  protected String getTerminalID() { // PIC X(4)
    if (IsSTCheck)
  ...
  protected String getTransID() { // PIC X(4)
    if (IsSTCheck)
      Log.logFineDebug("getTransID:");

    return getCESM().getEnvironment()
   ...
   * @param Edit
   *          e: KeyPressed key
   * @return true if the last key hit is the same as key in parameter, false
   *         otherwise
   */
  ...
  /**
   * 
   * Forget last key pressed Internal usage only
   */
  ...
  /**
   * @param Var
   *          var Internal usage only Set the last key pressed as the value of var
   */
  ...
  /**
   * @param KeyPressed
   *          key Internal usage only * Set the last key pressed as key
   */
   ...
   * @param Edit
   *          e: KeyPressed key
   * @return false if the last key hit is the same as key in parameter, true
   *         otherwise
   */
  ...
  /**
   * Method: Var getCommAreaLength() Return the commarea length in a Var. It may
   * be 0 if no commera defined.
   * 
   * @param:
  ...
  /**
   * Method: setCommAreaLength Set the commarea length; Used internally
   * 
   * @param: int
   *           n
   * @return:
   ...
   *      ;
   */
  protected String getLastCICSCommandExecutedCode() { // PIC X(2)
    if (IsSTCheck)
  ...
  public void acceptEnv(Var varDest) {
    varDest.set(m_BaseProgramManager.getEnv()
                                    .getDisplayValue());
  }
}
