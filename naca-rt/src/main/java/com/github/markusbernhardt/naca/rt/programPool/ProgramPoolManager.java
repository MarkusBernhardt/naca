...
public class ProgramPoolManager // extends BaseOpenMBean
{
  ...
  public BaseProgram loadPooledProgramInstance(String csProgramName) {
    ...
    // if programInstance != null then we are inside a read lock, else no read lock
    // set

    // Double check programInstancesPool, as it may have been destroyed in jmx
    // thread (double check pattern)
  ...
  public void unloadAllPrograms(boolean bDoGCAfterEachProgramUnload) {
    ...
    if (colProgramInstancesPool != null) {
      // Create another ProgramInstancesPool container, as m_hashProgramInstancesPool
      // will be structurally modified in ProgramInstancesPool::unloadProgram() call
      int nNbEntries = 0;
  ...
  public void returnProgramInstanceToPool(BaseProgram program) {
    String csProgramName = program.getProgramManager()
                                  .getProgramName();
    ProgramInstancesPool programInstancesPool = m_hashProgramInstancesPool.get(csProgramName);
