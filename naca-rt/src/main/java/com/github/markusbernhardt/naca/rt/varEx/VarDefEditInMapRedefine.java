...
public class VarDefEditInMapRedefine extends VarDefEditInMapRedefineBase {
  ...
  void transfer(VarBufferPos bufferSource, VarAndEdit Dest) {
    ...
    // TempCacheLocator.getTLSTempCache().getProgramManager();
    //
    ...
    // int nPosAttributes = Dest.getBodyAbsolutePosition() -
    // Dest.getVarDef().getHeaderLength(); // Position of the attributes in the
    // buffer
    // for(int n=0; n<m_arrChildren.size(); n++)
    // {
    // EditInMapRedefine editDestChildTemplate =
    // (EditInMapRedefine)Dest.getEditChildAt(n+1);
    // EditInMapRedefine subItem =
    // editDestChildTemplate.allocOccursedItem(editDestChildTemplate.getVarDef());
    //
    // subItem.m_varDef.adjustSetting(editDestChildTemplate.getVarDef(),
    // Dest.getBodyAbsolutePosition()-7, 0, 1, null);
    //
  ...
  // void writeSubEditAndAttributes(VarBufferPos bufferDest,
  // VarDefEditInMapRedefine varSource, VarBufferPos bufferSource)
  // {
  // CStr cs = bufferSource.getStringAt(bufferSource.m_nAbsolutePosition,
  // m_nTotalSize);
  ...
  void write(VarBufferPos buffer, double d) {
    ...
    // Parag 9.c: A noninteger numeric literal or data item cannot be moved to an
  ...
  void write(VarBufferPos buffer, VarDefNumIntComp4 varSource, VarBufferPos bufferSource) {
    ...
    // @see
    // http://www.helsinki.fi/atk/unix/dec_manuals/cobv27ua/cobrm_029.htm#index_x_737
    // parag 11: When dest-item is alphanumeric or alphanumeric edited, alignment
    // and space-filling occur according to the Standard Alignment Rules.
    // If lit or src-item is signed numeric, the operational sign is not moved. If
    // the operational sign occupies a separate character position:
    // The sign character is not moved.
    // The size of lit or src-item is considered to be one less than its actual size
  ...
  // private int writeEditAndSubEditRightPadding(VarBufferPos buffer, CStr cs,
  // char cPad)
  // {
  // int nBodyPosStart = buffer.m_nAbsolutePosition;
  // int nBodyLength = m_nTotalSize;
  // return internalWriteRightPadding(buffer, nBodyPosStart, nBodyLength, cs,
  // cPad);
  // }
