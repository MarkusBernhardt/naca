...
import com.github.markusbernhardt.naca.rt.tempCache.CStrString;
...
public abstract class VarDefBuffer extends VarDefBase {
  ...
  protected void internalWriteJustifyRightPadding(VarBufferPos buffer, int nPosition, int nDestLength, String cs, char cPad) {
     ...
     * 
     * Data is moved from a sending data item to the right justified receiving data
     * item starting with the rightmost character of the sending data item. The
     * rightmost character is placed in the rightmost character of the receiving
     * data item. The next rightmost data item of the sending data item is then
     * moved to the next rightmost character of the receiving data item. This
     * process continues until either all of the sending data item has been moved,
     * or the receiving data item is full. Note that a space in the sending data
     * item is considered a valid character, no matter where it is within the
     * sending data item. That is, spaces are not stripped from the sending item,
     * even if they are in the rightmost positions of the sending item.
     * 
     * When a receiving data item is described using this clause, and the sending
     * data item is larger than the receiving item, the leftmost characters are
     * truncated.
     * 
  ...
  protected CStr getCStrRightPadded(int n, int nNbCharDest) {
    CStrNumber cs = TempCacheLocator.getTLSTempCache()
                                    .getCStrNumber();
  ...
  protected CStr getCStrRightPadded(CStr csIn, char cPad, int nNbCharDest) {
    CStrString csOut = TempCacheLocator.getTLSTempCache()
                                       .getCStrString();
  ...
  // protected void internalPhysicalWrite(VarBufferPos bufferDest, String
  // csSource)
  // {
  ...
  // void initializeItemAndChildren(ProgramManager programManager,
  ...
  void moveCorrespondingItemAndChildren(MoveCorrespondingEntryManager manager, SharedProgramInstanceData sharedProgramInstanceData,
      BaseProgramManager programManager, VarDefBase varDefDestGroup, int nSourceOffset, int nDestOffset) {
    if (m_arrChildren == null) // Final node
    {
  ...
  // void initializeUsingCache(VarBufferPos varBufferPos, InitializeCache
  // initializeCache) //, int nOffset)
  // {
  // initializeCache.applyItems(varBufferPos, varBufferPos.m_nAbsolutePosition);
  // //, nOffset);
  // }

  // void initializeItemAndChildren(VarBufferPos varBufferPos, InitializeManager
  // initializeManager, int nOffset)
  // {
  ...
  private void initializeItemAndChildren(TempCache cache, VarBufferPos varBufferPos, InitializeManager initializeManager, int nOffset, int nNbDimUsed,
  ...
  // private void tryInitialize(ProgramManager programManager, InitializeManager
  // initializeManager)
  // {
  ...
  // {
  // Assert("Child: '" + toString() + "' starts before it's parent variable: " +
  // m_varDefParent.toString() + "'"); // We are not inside parent range
  // return false;
  // }
  //
  // int nLastPosParent = nStartPosParent +
  // m_varDefParent.getHeaderLength()+m_varDefParent.getBodyLength() - 1;
  // int nLastPos = nStartPos + getHeaderLength()+getBodyLength() - 1;
  // if(nLastPos > nLastPosParent)
  // {
  // Assert("Child: '" + toString() + "' ends after it's parent variable ending:
  // '" + m_varDefParent.toString() + "'"); // We are not inside parent range
  // return false;
  // }
  // }
  ...
  // if(m_varDefParent != null)
  // {
  // boolean b = m_varDefParent.isChildKnown(this); // Check if our parent known
  // us
  // if(!b)
  // Assert("Child: '" + toString() + "' isn't correctly parented by: '" +
  // m_varDefParent.toString() + "'"); // We are not inside parent range
  // b = DEBUGCheckRangeWithinToParentRange(); // Chck if our range is
  // withinparent range
  // return b;
  // }
  // return true;
  ...
  // public boolean readFromFile(ProgramManager programManager, DataFile dataFile,
  // VarBufferPos bufferPos, boolean bConvertEbcdicToUnicode)
  // {
  // // Read the line
  // VarBase var = programManager.getVarFullName(this);
  ...
  // VarDefBuffer varDefChild = getChild(nChild);
  // if(varDefChild != null)
  // bEOF = varDefChild.readFromFile(programManager, dataFile, bufferPos,
  // bConvertEbcdicToUnicode);
  // }
  // }
