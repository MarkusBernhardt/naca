...
import com.github.markusbernhardt.naca.rt.mapSupport.MapFieldAttrFill;
...
import com.github.markusbernhardt.naca.rt.mapSupport.MapFieldAttrJustify;
...
public abstract class Edit extends VarAndEdit {
  ...
  public String getDottedSignedString() {
    String cs = m_varDef.getDottedSignedString(m_bufferPos)
                        .getAsString();
    return cs;
  }

  public String getDottedSignedStringAsSQLCol() {
    String cs = m_varDef.getDottedSignedStringAsSQLCol(m_bufferPos)
                        .getAsString();
    return cs;
  }
  ...
  /*
   * public String toString() { String cs = m_csFullName + "[" +
   * m_VarManager.getValueAbsoluteStartPosition() + "-" + getStorageSize() + "]:"
   * + "\"" + getString() + "\";" + m_FieldAttributes.toString();
   * 
   * return cs; }
   */
  ...
  protected void set(String csValue, boolean bWithPadding) {
    if (csValue != null && csValue.trim()
                                  .length() != 0) // Not an empty value
    {
      if (bWithPadding) {
        String csChar = "";
    ...
    // nPosition = m_VarManager.writeRepeatingCharAt(nPosition, ' ',
    // nNbLeftPadChar);
    // else if(m_FieldAttributes.isFillZero())
    // nPosition = m_VarManager.writeRepeatingCharAt(nPosition, '0',
    // nNbLeftPadChar);
    // }
    // }
    // }
  ...
  /*
   * public int encodeAndAppend(Var varDest, int nVarDestAbsStartposition, int
   * nTextLength) { //Custon serialization process uncompatible with direct
   * modification of header bytes form Cobol int nAttrEncoded =
   * m_FieldAttributes.getEncodedValue(); // Will use 4 char position char
   * cProgrammedSymbolSet = m_Flag.getEncodedValue(); // Will use 1 char //int
   * setEncodedEdit(int nAttributes, char cProgrammedSymbolSet, String csText, int
   * nVarDestAbsStartposition) int nSize =
   * varDest.m_VarManager.setEncodedEdit(nAttrEncoded, cProgrammedSymbolSet,
   * getString(), nVarDestAbsStartposition, nTextLength);
   * 
   * return nSize; }
   */
  ...
  public int decodeFromCharBuffer(InternalCharBuffer charBuffer, int nPos, int nDestLength) {
    ...
    // logCesm("edit decodeFromVar source="+varSource.getLoggableValue()+" to
    // edit="+getLoggableValue());
    return nPos + 7 + nDestLength;
  }
