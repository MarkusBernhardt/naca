...
import com.github.markusbernhardt.jlib.classLoader.CodeManager;
...
import com.github.markusbernhardt.jlib.misc.Mail;
import com.github.markusbernhardt.jlib.misc.MailService;
...
import com.github.markusbernhardt.naca.rt.asyncTasks.CAsynchronousTask;
...
import com.github.markusbernhardt.naca.rt.exceptions.CESMAbendException;
...
import com.github.markusbernhardt.naca.rt.exceptions.CannotOpenFileException;
import com.github.markusbernhardt.naca.rt.exceptions.DumpProgramException;
import com.github.markusbernhardt.naca.rt.exceptions.FileDescriptorNofFoundException;
import com.github.markusbernhardt.naca.rt.exceptions.InputFileNotFoundException;
import com.github.markusbernhardt.naca.rt.exceptions.TooManyCloseFileException;
import com.github.markusbernhardt.naca.rt.programPool.ProgramInstancesPool;
import com.github.markusbernhardt.naca.rt.programPool.ProgramPoolManager;
...
public abstract class BaseProgramLoader extends ProgramSequencer // ProgramSequencerExt
{
  ...
  public SharedProgramInstanceData forcePreloadSessionProgram(String csDefaultProgramName, int nNbInstanceToPreload) {
    ...
    if (currentProgram != null) {
      ...
      SharedProgramInstanceData s = currentProgram.getProgramManager()
      ...
      for (int n = 1; n < nNbInstanceToPreload; n++) {
        ...
        if (currentProgram2ndInstance != null && programInstancesPool != null) {
          boolean bNewProgramInstance = currentProgram2ndInstance.getProgramManager()
                                                                 .isNewProgramInstance();
          currentProgram2ndInstance.getProgramManager()
                                   .prepareCall(this, currentProgram, null, null, bNewProgramInstance);
  ...
  private BaseProgram LoadProgramInstance(String csProgram, boolean bUseProgramPool) {
    /*
     * String csProgramName = ResolveTransID(ProgId) ; if (csProgramName == null ||
     * csProgramName.equals("")) { csProgramName = ProgId ; }
     */
    csProgram = Character.toUpperCase(csProgram.charAt(0)) + csProgram.substring(1)
                                                                      .toLowerCase();
  ...
  // //ProgramInstance programInstance = new ProgramInstance(csProgramName,
  // program, true);
  // return program;
  // }
  // return null;
  ...
  private void runProgram(BaseEnvironment env, ArrayList<BaseCalledPrgPublicArgPositioned> arrCallerCallParam) throws Exception {
    TempCacheLocator.getTLSTempCache()
                    .setCurrentEnv(env);
    ...
    while (bContinue) {
      ...
      if (csProgramID.equals("")) // No current program
      {
        ...
        try {
          boolean bNewInstance = currentProgram.getProgramManager()
                                               .isNewProgramInstance();
          currentProgram.getProgramManager()
                        .prepareCall(this, currentProgram, arrCallerCallParam, env, bNewInstance);
          ms_lock.unlock();
        } catch (Exception e) {
        ...
        try {
          ...
          currentProgram.getProgramManager()
                        .mapCalledPrgReturnParameters(arrCallerCallParam);
