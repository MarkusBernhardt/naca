...
public class ProgramCopyOwner extends BaseCloseMBean {
  ...
  public void unloadCopy() {
    ...
    if (m_hashPrograms != null) {
      ...
      // if(m_hashPrograms.size() == 0)
      // Log.logNormal("unloadCopy; Correctly unload " + nNbPrograms + " programs
      // using copy "+m_csCopyName);
      // else
      // Log.logImportant("unloadCopy; ERROR: unload " + nNbPrograms + " programs
      // using copy "+m_csCopyName + " but " + m_hashPrograms.size() + " remains
      // uncorrectly loaded");
    } else
