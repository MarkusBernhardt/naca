...
import com.github.markusbernhardt.naca.rt.varEx.FileDescriptorOpenStatus;
...
public class FileManagerEntry extends CJMapObject {
  ...
  public String getPhysicalName(String csLogicalName, BaseSession baseSession) {
    ...
    // Log.logCritical("Environnement or Session ERROR: Logical File
    // \'"+m_csLogicalName + "\' has no physical definition");
    throw new FileDescriptorNofFoundException(csLogicalName, null);
    // "Environnement or Session ERROR: Logical File \'"+csLogicalName + "\' has no
    // physical definition"
  }
    ...
    if (checkCanOpen()) {
      ...
      if (csDdname != null && csDdname.equals(csLogicalName)) {
        if (baseSession.getDynamicAllocationInfo("SYSOUT") != null) {
          ...
          sb.append(StringUtil.rightPad(baseSession.getDynamicAllocationInfo("SYSOUT")
                                                   .substring(9),
              4, ' ')); // No de formulaire
