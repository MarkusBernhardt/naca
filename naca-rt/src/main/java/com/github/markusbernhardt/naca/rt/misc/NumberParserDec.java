...
public class NumberParserDec extends NumberParser {
  ...
  /*
   * public static Dec getAsDec(String cs) { boolean bPositive = true;
   * if(cs.length() > 0 && cs.charAt(0) == '-') { cs = cs.substring(1); bPositive
   * = false; } int nDot = cs.indexOf('.'); if(nDot >= 0) { String csInt =
   * cs.substring(0, nDot); String csDec = cs.substring(nDot+1); Dec dec = new
   * Dec(csInt, csDec); dec.setPositive(bPositive); return dec; } else { Dec dec =
   * new Dec(cs, "0"); dec.setPositive(bPositive); return dec; } }
   */
