...
public class ProgramInstancesPool extends BaseCloseMBean {
  ...
  synchronized public String getTime_ExecRunTime() {
    ...
    for (int n = 0; n < nNbInstances; n++) {
      ...
      long l = program.getProgramManager()
                      .getTimeRun();
      if (l < lMin)
        lMin = l;
      if (l > lMax)
  ...
  public BaseProgram getOrCreateUnusedInstance() {
    m_unloadProgramRWLock.readLock()
                         .lock(); // Get exclusive lock
  ...
  private synchronized BaseProgram createNewInstance() {
    ...
    if (obj != null) {
      ...
      // Program programInstance = new ProgramInstance(m_csProgramName, program,
      // true);
      return program;
    }
    return null;
  ...
  public void unloadProgram() {
    ...
    m_unloadProgramRWLock.writeLock()
                         .lock(); // Get exclusive lock
    ...
    m_unloadProgramRWLock.writeLock()
                         .unlock(); // Release exclusive lock; unlocking optinal thread waiting to obtain read lock
  ...
  private void doUnloadProgram() {
    ...
    while (m_stack.size() > 0) {
      BaseProgram program = m_stack.pop();
      program.getProgramManager()
             .unloadClassCode(); // Destroy the program's class
  ...
  public void returnProgram(BaseProgram program) {
    ...
    m_unloadProgramRWLock.readLock()
                         .unlock(); // Release read lock: the current thread do not own anymore the program instance
  }
