...
import com.github.markusbernhardt.jlib.log.StackStraceSupport;
...
public class Pic9Comp3BufferSupport extends BasePic9Comp3BufferSupport {
  ...
  static long keepRightMostDigits(VarDefBase varDef, long lOriginalValue, int nNbDigitsToKeep) {
    ...
    if (lOriginalValue < 0) {
      ...
      if (lValue > lPower10) // 1234 > 1000, when we want to keep only 3 digits for n, then returning only
                             // 234
      {
    ...
    if (lOriginalValue >= lPower10) // 1234 >= 1000, when we want to keep only 3 digits for n, then returning only
                                    // 234
    {
  ...
  private static void mailLogNumberTruncationError(VarDefBase varDef, long lOriginalValue, long lValue) {
    String csSimpleName = TempCacheLocator.getTLSTempCache()
                                          .getProgramManager()
                                          .getProgramName();
