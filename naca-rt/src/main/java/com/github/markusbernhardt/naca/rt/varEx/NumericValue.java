...
public class NumericValue {
  ...
  VarDefBuffer createVarDefFPacNum(VarDefBase varDefParent, DeclareType9 declareType9) {
    if (m_nComp == -3) // Comp-3 specified: 2 digits by char (1 by nibble), with the sign in the
                       // rightmost nibble
    {
      if (m_bSigned) {
  ...
  VarDefBuffer createVarDef(VarDefBase varDefParent, DeclareType9 declareType9) {
    if (m_nComp == 0) // No Comp-... specified: 1 char is a digit, except maybe the sign that may be
                      // embbed in the last char
    {
      if (m_nNbDigitDecimal == 0) {
    ...
    } else if (m_nComp == -3) // Comp-3 specified: 2 digits by char (1 by nibble), with the sign in the
                              // rightmost nibble
    {
      if (!m_bSigned) {
        if (m_nNbDigitDecimal == 0) {
